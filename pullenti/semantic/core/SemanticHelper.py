...
class SemanticHelper:
    ...
    def find_derivates(t : 'Token') -> typing.List['DerivateGroup']:
                ...
                if (isinstance(f, MorphWordForm)): 
                    if (cla is not None): 
                        if (((cla) & f.class0_).is_undefined): 
                            continue
                    res = Explanatory.find_derivates(Utils.ifNotNull((f).normal_full, (f).normal_case), True, None)
        ...
        if (isinstance(t, VerbPhraseItemToken)): 
            ...
            if (vpt.verb_morph is not None): 
                res = Explanatory.find_derivates(vpt.verb_morph.normal_case, True, t.morph.language)
                if (res is None or (len(res) == 0 and vpt.verb_morph.normal_full is not None and vpt.verb_morph.normal_case != vpt.verb_morph.normal_full)): 
                    res = Explanatory.find_derivates(vpt.verb_morph.normal_full, True, t.morph.language)
            return res
        if (isinstance(t, NumberToken)): 
            if ((t).value == "1"): 
                return Explanatory.find_derivates("ОДИН", True, MorphLang.RU)
    ...
    def try_create_links(master : 'MetaToken', slave : 'MetaToken', onto : 'ISemanticOnto'=None) -> typing.List['SemanticLink']:
        ...
        if ((npt1 is not None and sla2 is not None and sla2.morph.case_.is_genitive) and sla2.preposition is None): 
            if (npt1.noun.begin_token.get_morph_class_in_dictionary().is_personal_pronoun): 
                ...
                has_gen = False
                for r in res: 
                    if (r.question == ControlModelQuestion.BASE_GENETIVE): 
                        has_gen = True
                        break
                if (not has_gen): 
                    res.append(SemanticLink._new2938(True, npt1, sla2, 0.5, ControlModelQuestion.BASE_GENETIVE))
        if (onto is not None): 
            ...
            if (str2 is not None): 
                if (onto.check_link(str1, str2)): 
                    if (len(res) > 0): 
                        ...
                        res.append(SemanticLink._new2939(SemanticRole.STRONG, master, slave, 3))
        if (npt1 is not None): 
    ...
    def __try_create_inf(master : 'MetaToken', vpt2 : 'VerbPhraseToken', gr : 'DerivateGroup') -> typing.List['SemanticLink']:
        ...
        if (cit is not None and ControlModelQuestion.TO_DO in cit.links): 
            rol = cit.links[ControlModelQuestion.TO_DO]
        if (rol is not None): 
            res.append(SemanticLink._new2940((2 if rol != SemanticRole.COMMON else 1), ControlModelQuestion.TO_DO))
        return res
    ...
    def __try_create_noun(npt1 : 'NounPhraseToken', slave : 'MetaToken', gr : 'DerivateGroup') -> typing.List['SemanticLink']:
        ...
        if (ok): 
            if (len(res) == 0): 
                res.append(SemanticLink._new2941(ControlModelQuestion.BASE_GENETIVE, SemanticRole.PACIENT, True))
            for r in res: 
                r.rank += (4)
                if (r.role == SemanticRole.COMMON): 
                    r.role = SemanticRole.STRONG
    ...
    def __try_create_verb(vpt1 : 'VerbPhraseToken', slave : 'MetaToken', gr : 'DerivateGroup') -> typing.List['SemanticLink']:
        ...
        if (prep is None and morph_.case_.is_nominative and not vpt1.first_verb.is_participle): 
            ...
            if (ok): 
                cit00 = cit
                is_rev0 = is_rev1
                if (vpt1.first_verb != vpt1.last_verb and ((vpt1.first_verb.is_verb_reversive or vpt1.first_verb.is_verb_passive))): 
                    cit00 = (None)
                    is_rev0 = True
                ...
                addagent = False
                if (cit00 is None): 
                    sl = SemanticLink._new2942(True, (SemanticRole.PACIENT if is_rev0 else SemanticRole.AGENT), 1, ControlModelQuestion.BASE_NOMINATIVE, is_rev0)
                else: 
                    for kp in cit00.links.items(): 
                        if (kp[0].check(None, MorphCase.NOMINATIVE)): 
                            sl = SemanticLink._new2943(kp[1], 2, kp[0], is_rev0)
                ...
                if (sl is not None): 
                    ...
                    res.append(sl)
                    if (addagent): 
                        res.append(SemanticLink._new2944(SemanticRole.AGENT, sl.rank, sl.question))
        if (prep is None and is_rev1 and morph_.case_.is_instrumental): 
            no_instr = True
            cit00 = cit
            sl = None
            if (cit00 is None): 
                sl = SemanticLink._new2942(True, SemanticRole.AGENT, 1, ControlModelQuestion.BASE_INSTRUMENTAL, True)
            else: 
                for kp in cit00.links.items(): 
                    if (kp[0].check(None, MorphCase.INSTRUMENTAL)): 
                        sl = SemanticLink._new2944(kp[1], 2, kp[0])
                        if (sl.role == SemanticRole.AGENT): 
                            sl.is_passive = True
                        break
            if (sl is not None): 
                ...
                if (sla2.begin_char < vpt1.begin_char): 
                    sl.rank -= 0.5
                res.append(sl)
        if (prep is None and morph_.case_.is_dative and ((cit is None or not ControlModelQuestion.BASE_DATIVE in cit.links))): 
            sl = SemanticLink._new2947(cit is None, SemanticRole.STRONG, 1, ControlModelQuestion.BASE_DATIVE)
            ...
            if (ok): 
                if (len(res) == 0): 
                    ...
                    if (prep is None and is_rev1 and morph_.case_.is_nominative): 
                        ...
                        ok = True
                    if (ok): 
                        res.append(SemanticLink._new2948(SemanticRole.PACIENT, (ControlModelQuestion.BASE_NOMINATIVE if is_rev1 else ControlModelQuestion.BASE_ACCUSATIVE), True))
                else: 
                    for r in res: 
                        r.rank += (4)
                        if (r.role == SemanticRole.COMMON): 
    ...
    def __create_roles(cit : 'ControlModelItem', prep : str, cas : 'MorphCase', res : typing.List['SemanticLink'], ignore_nomin_case : bool=False, ignore_instr_case : bool=False) -> None:
        ...
        for li in cit.links.items(): 
            if (li[0].check(prep, cas)): 
                if (ignore_nomin_case and li[0].case_.is_nominative and li[0].preposition is None): 
                    continue
                if (ignore_instr_case and li[0].case_.is_instrumental and li[0].preposition is None): 
                    continue
                if (roles is None): 
                    roles = dict()
                q = li[0]
                r = li[1]
                if (q.is_abstract and q.subquestions is not None): 
                    for qq in q.subquestions: 
                        if (qq.check(prep, cas)): 
                            q = qq
