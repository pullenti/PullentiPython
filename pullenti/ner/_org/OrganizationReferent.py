...
class OrganizationReferent(Referent):
    ...
    def to_string(self, short_variant : bool, lang : 'MorphLang'=MorphLang(), lev : int=0) -> str:
        ...
        if (name is not None): 
            ...
            for s in self.slots: 
                if (s.type_name == OrganizationReferent.ATTR_TYPE): 
                    nam = (s.value if isinstance(s.value, str) else None)
        ...
        if (not short_variant): 
            ...
            kl = self.get_string_value(OrganizationReferent.ATTR_KLADR)
            if (kl is not None): 
            ...
            if (is_dep or out_own_in_name): 
                for ss in self.slots: 
                    if (ss.type_name == OrganizationReferent.ATTR_HIGHER and (isinstance(ss.value, Referent)) and (lev < 20)): 
                        hi = (ss.value if isinstance(ss.value, OrganizationReferent) else None)
                        if (hi is not None): 
                            tmp = list()
                            tmp.append(self)
                            ...
                            if (hi is not None): 
                                continue
                        print(';', end="", file=res)
                        print(" {0}".format((ss.value if isinstance(ss.value, Referent) else None).to_string(short_variant, lang, lev + 1)), end="", file=res, flush=True)
                        break
        if (res.tell() == 0): 
        ...
        for s in self.slots: 
            if (s.type_name == OrganizationReferent.ATTR_MISC): 
                if (isinstance(s.value, Referent)): 
                    r = (s.value if isinstance(s.value, Referent) else None)
                    if (r.type_name == "URI"): 
    ...
    def __correct_name(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            inoutarg2215 = RefOutArgWrapper(0)
            inoutres2216 = Utils.tryParseInt(name[0:0+name.find(' ')], inoutarg2215)
            i = inoutarg2215.value
            if (inoutres2216): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres2217 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres2217 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def add_name(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        ...
        inoutarg2218 = RefOutArgWrapper(0)
        s = self.__correct_name(name, inoutarg2218)
        num = inoutarg2218.value
        if (s is None): 
        ...
        if (t is not None and not t.chars.is_letter and BracketHelper.is_bracket(t, False)): 
            t = t.next0_
        if (((isinstance(t, TextToken)) and (s.find(' ') < 0) and len(s) > 3) and s == (t if isinstance(t, TextToken) else None).term): 
                ...
                if (snorm == s): 
                    if (self.__m_name_single_normal_real is None): 
                        ...
                        for ii in range(len(self.slots) - 1, -1, -1):
                            if (self.slots[ii].type_name == OrganizationReferent.ATTR_NAME and ((self.slots[ii].value if isinstance(self.slots[ii].value, str) else None)) != s): 
                                mt = Morphology.process(self.slots[ii].value if isinstance(self.slots[ii].value, str) else None, t.morph.language, None)
                                if (mt is not None and len(mt) == 1): 
    ...
    def add_name_str(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and len(typ.name) > len(typ.typ)): 
                num = 0
                inoutarg2219 = RefOutArgWrapper(0)
                s = self.__correct_name(typ.name, inoutarg2219)
                num = inoutarg2219.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    inoutarg2220 = RefOutArgWrapper(0)
                    ss = self.__correct_name(typ.name, inoutarg2220)
                    num = inoutarg2220.value
                    if (ss is not None): 
    ...
    def _types_contains(self, substr : str) -> bool:
        for s in self.slots: 
            if (s.type_name == OrganizationReferent.ATTR_TYPE): 
                if (substr in (s.value if isinstance(s.value, str) else None)): 
                    return True
        return False
    ...
    def add_type(self, typ : 'OrgItemTypeToken', final_add : bool=False) -> None:
        ...
        if (typ.geo is not None): 
            if (((isinstance(typ.geo.referent, GeoReferent))) and (typ.geo.referent if isinstance(typ.geo.referent, GeoReferent) else None).is_region and self.kind == OrganizationKind.STUDY): 
                pass
            else: 
    ...
    def owner(self) -> 'Referent':
        ...
        return (self.get_value(OrganizationReferent.ATTR_OWNER) if isinstance(self.get_value(OrganizationReferent.ATTR_OWNER), Referent) else None)
    
    @owner.setter
    def owner(self, value) -> 'Referent':
        self.add_slot(OrganizationReferent.ATTR_OWNER, (value if isinstance(value, Referent) else None), True, 0)
        return value
    ...
    def higher(self) -> 'OrganizationReferent':
        ...
        self.__m_parent = (self.get_value(OrganizationReferent.ATTR_HIGHER) if isinstance(self.get_value(OrganizationReferent.ATTR_HIGHER), OrganizationReferent) else None)
        ...
        oo = (sl.value if isinstance(sl.value, OrganizationReferent) else None)
        while oo is not None: 
            if (oo in li): 
                self.__m_parent = None
                return self.__m_parent
            li.append(oo)
            oo = (oo.get_value(OrganizationReferent.ATTR_HIGHER) if isinstance(oo.get_value(OrganizationReferent.ATTR_HIGHER), OrganizationReferent) else None)
        return self.__m_parent
    ...
    def _add_geo_object(self, r : object) -> bool:
        ...
        if (isinstance(r, GeoReferent)): 
            geo_ = (r if isinstance(r, GeoReferent) else None)
            for s in self.slots: 
                if (s.type_name == OrganizationReferent.ATTR_GEO and (isinstance(s.value, GeoReferent))): 
            ...
            if (isinstance((r if isinstance(r, ReferentToken) else None).get_referent(), GeoReferent)): 
                if (not self._add_geo_object((r if isinstance(r, ReferentToken) else None).get_referent())): 
                    return False
                self.add_ext_referent(r if isinstance(r, ReferentToken) else None)
                return True
            if (isinstance((r if isinstance(r, ReferentToken) else None).get_referent(), AddressReferent)): 
                return self._add_geo_object((r if isinstance(r, ReferentToken) else None).begin_token.get_referent())
        return False
    ...
    def can_be_general_for(self, obj : 'Referent') -> bool:
        ...
        geos2 = (obj if isinstance(obj, OrganizationReferent) else None)._geo_objects
        if (len(geos1) == 0 and len(geos2) > 0): 
            if (self.__check_eq_eponyms(obj if isinstance(obj, OrganizationReferent) else None)): 
                return False
            return True
        elif (len(geos1) == len(geos2)): 
            if (self.__check_eq_eponyms(obj if isinstance(obj, OrganizationReferent) else None)): 
                return False
            if (self.higher is not None and (obj if isinstance(obj, OrganizationReferent) else None).higher is not None): 
                if (self.higher.can_be_general_for((obj if isinstance(obj, OrganizationReferent) else None).higher)): 
                    return True
        return False
    ...
    def __can_be_equals(self, obj : 'Referent', ignore_geo_objects : bool, typ : 'EqualType', lev : int) -> bool:
            ...
            if (org0_.number != self.number): 
                ...
                for a in self.slots: 
                    if (a.type_name == OrganizationReferent.ATTR_TYPE): 
                        if (obj.find_slot(a.type_name, a.value, True) is not None or obj.find_slot(OrganizationReferent.ATTR_NAME, (a.value if isinstance(a.value, str) else None).upper(), True) is not None): 
                            eq_number = True
                            break
        if (typ == Referent.EqualType.DIFFERENTTEXTS): 
            if (self.number is not None or org0_.number is not None): 
            ...
            if (((eq_number or eq_eponyms)) and ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == Referent.EqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    inoutarg2222 = RefOutArgWrapper(False)
                    inoutres2223 = Utils.tryGetValue(org0_._name_vars, v[0], inoutarg2222)
                    b = inoutarg2222.value
                    if (not inoutres2223): 
                        continue
        ...
        if (self.find_slot(OrganizationReferent.ATTR_EPONYM, None, True) is not None or obj.find_slot(OrganizationReferent.ATTR_EPONYM, None, True) is not None): 
            if (typ == Referent.EqualType.FORMERGING and ((self.find_slot(OrganizationReferent.ATTR_EPONYM, None, True) is None or obj.find_slot(OrganizationReferent.ATTR_EPONYM, None, True) is None))): 
                pass
            else: 
                ok = False
                eps = self.eponyms
    ...
    def merge_slots(self, obj : 'Referent', merge_statistic : bool) -> None:
        ...
        own_obj = (obj if isinstance(obj, OrganizationReferent) else None).higher
        ...
        if ((obj if isinstance(obj, OrganizationReferent) else None)._is_from_global_ontos): 
    ...
    def _final_correction(self) -> None:
        ...
        if (len(typs) > 0 and LanguageHelper.is_latin(typs[0])): 
            ...
            if ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None and self.find_slot(OrganizationReferent.ATTR_GEO, None, True) is not None and self.find_slot(OrganizationReferent.ATTR_NUMBER, None, True) is None) and len(typs) > 0): 
                geo_ = (self.get_value(OrganizationReferent.ATTR_GEO) if isinstance(self.get_value(OrganizationReferent.ATTR_GEO), GeoReferent) else None)
                if (geo_ is not None): 
    ...
    def create_ontology_item_ex(self, min_len : int, only_names : bool=False, pure_names : bool=False) -> 'IntOntologyItem':
        ...
        for v in vars0_: 
            if (len(v) >= min_len): 
                if (pure_names): 
                    term = Termin()
                    term.init_by_normal_text(v, MorphLang())
                else: 
                    term = Termin(v)
