...
class NamedItemToken(MetaToken):
    ...
    @staticmethod
    def tryParseList(t : 'Token', loc_onto : 'IntOntologyCollection') -> typing.List['NamedItemToken']:
        ne = NamedItemToken.tryParse(t, loc_onto)
        if (ne is None): 
            return None
        res = list()
        res.append(ne)
    ...
    def tryParse(t : 'Token', loc_onto : 'IntOntologyCollection') -> 'NamedItemToken':
        ...
        if (isinstance(t, ReferentToken)): 
            ...
            if ((r.type_name == "PERSON" or r.type_name == "PERSONPROPERTY" or (isinstance(r, GeoReferent))) or r.type_name == "ORGANIZATION"): 
                return NamedItemToken._new1647(t, t, r, t.morph)
            return None
        ...
        if (typ is not None): 
            if (not ((isinstance(t, TextToken)))): 
                return None
            res = NamedItemToken._new1648(typ.begin_token, typ.end_token, typ.morph, typ.chars)
        ...
        if (nam is not None): 
            if (nam.begin_token.chars.is_all_lower): 
                return None
            res = NamedItemToken._new1648(nam.begin_token, nam.end_token, nam.morph, nam.chars)
            ...
            if (not t.is_whitespace_before and t.previous is not None): 
                ...
                if (t.next0_.isCharOf(",.;!?") and t.next0_.is_whitespace_after): 
                    pass
                else: 
                    ok = False
            if (ok): 
        ...
        if (adj is not None): 
            if (adj.morph.class0_.is_noun): 
                if (adj.end_token.isValue("ВОСТОК", None)): 
                    if (adj.begin_token == adj.end_token): 
                        return None
                    re = NamedItemToken._new1650(t, adj.end_token, adj.morph)
                    re.kind = NamedEntityKind.LOCATION
                    re.name_value = MiscHelper.getTextValue(t, adj.end_token, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
                    re.is_wellknown = True
                    return re
                return None
            if (adj.whitespaces_after_count > 2): 
                return None
            if ((isinstance(adj.end_token.next0_, ReferentToken)) and (isinstance(adj.end_token.next0_.getReferent(), GeoReferent))): 
                re = NamedItemToken._new1650(t, adj.end_token.next0_, adj.end_token.next0_.morph)
                re.kind = NamedEntityKind.LOCATION
                re.name_value = MiscHelper.getTextValue(t, adj.end_token.next0_, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
                re.is_wellknown = True
            ...
            if (res is not None and res.kind == NamedEntityKind.LOCATION): 
                s = adj.getNormalCaseText(MorphClass.ADJECTIVE, True, res.morph.gender, False)
                if (s is not None): 
            ...
            if (npt is not None and len(npt.adjectives) > 0): 
                ...
                if (test is not None and test.end_token == npt.end_token and test.type_value is not None): 
                    ...
                    for a in npt.adjectives: 
                        s = a.getNormalCaseText(MorphClass.ADJECTIVE, True, test.morph.gender, False)
                        if (tmp.tell() > 0): 
                            print(' ', end="", file=tmp)
                    ...
                    return test
        if ((BracketHelper.isBracket(t, True) and t.next0_ is not None and t.next0_.chars.is_letter) and not t.next0_.chars.is_all_lower): 
            br = BracketHelper.tryParse(t, BracketParseAttr.NO, 100)
            if (br is not None): 
                res = NamedItemToken(t, br.end_token)
                res.is_in_bracket = True
                res.name_value = MiscHelper.getTextValue(t, br.end_token, GetTextAttr.NO)
                nam = NamedItemToken.__m_names.tryParse(t.next0_, TerminParseAttr.NO)
    ...
    @staticmethod
    def _new1647(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Referent', _arg4 : 'MorphCollection') -> 'NamedItemToken':
    ...
    @staticmethod
    def _new1648(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : 'CharsInfo') -> 'NamedItemToken':
        res = NamedItemToken(_arg1, _arg2)
        res.morph = _arg3
        res.chars = _arg4
        return res
    ...
    @staticmethod
    def _new1650(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection') -> 'NamedItemToken':
        res = NamedItemToken(_arg1, _arg2)
        res.morph = _arg3
        return res