...
class PersonPropertyReferent(Referent):
    ...
    def __get_higher(self, lev : int) -> 'PersonPropertyReferent':
        hi = (self.get_value(PersonPropertyReferent.ATTR_HIGHER) if isinstance(self.get_value(PersonPropertyReferent.ATTR_HIGHER), PersonPropertyReferent) else None)
        if (hi is None): 
            return None
    ...
    def can_be_equals(self, obj : 'Referent', typ : 'EqualType') -> bool:
        pr = (obj if isinstance(obj, PersonPropertyReferent) else None)
        if (pr is None): 
            return False
        n1 = self.name
        n2 = pr.name
        ...
        if (self.find_slot(PersonPropertyReferent.ATTR_REF, None, True) is not None or pr.find_slot(PersonPropertyReferent.ATTR_REF, None, True) is not None): 
                ...
                if (isinstance(refs1[i], Referent)): 
                    for rr in refs2: 
                        if (isinstance(rr, Referent)): 
                            if ((rr if isinstance(rr, Referent) else None).can_be_equals(refs1[i] if isinstance(refs1[i], Referent) else None, typ)): 
                                noeq = False
                                eq = True
                                break
            i = 0
            ...
            while True:
                ...
                if (isinstance(refs2[i], Referent)): 
                    for rr in refs1: 
                        if (isinstance(rr, Referent)): 
                            if ((rr if isinstance(rr, Referent) else None).can_be_equals(refs2[i] if isinstance(refs2[i], Referent) else None, typ)): 
                                noeq = False
                                eq = True
                                break
            if (eq and not noeq): 
    ...
    def merge_slots(self, obj : 'Referent', merge_statistic : bool=True) -> None:
        nam = self.name
        nam1 = (obj if isinstance(obj, PersonPropertyReferent) else None).name
