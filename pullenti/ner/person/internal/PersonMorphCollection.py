...
class PersonMorphCollection:
    ...
    class PersonMorphVariant:
        ...
        @staticmethod
        def _new2566(_arg1 : str, _arg2 : 'MorphGender', _arg3 : str) -> 'PersonMorphVariant':
        ...
        if (gen == MorphGender.MASCULINE or gen == MorphGender.FEMINIE): 
            for it in self.items: 
                if (it.value == val and it.gender == gen): 
                    return
            self.items.append(PersonMorphCollection.PersonMorphVariant._new2566(val, gen, shortval))
            if (add_other_gender_var): 
                g0 = (MorphGender.MASCULINE if gen == MorphGender.FEMINIE else MorphGender.FEMINIE)
                v = Morphology.get_wordform(val, MorphBaseInfo._new212(MorphClass._new2547(True), g0))
                if (v is not None): 
                    self.items.append(PersonMorphCollection.PersonMorphVariant._new2566(v, g0, shortval))
        else: 
