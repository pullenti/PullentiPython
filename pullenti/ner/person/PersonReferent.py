...
class PersonReferent(Referent):
            ...
            if (isinstance(s.value, PersonPropertyReferent)): 
                ...
                for ss in pr.slots: 
                    if (ss.type_name == PersonPropertyReferent.ATTR_NAME): 
                        n = (ss.value if isinstance(ss.value, str) else None)
                        if (res is None): 
                            res = n
                        elif (len(res) > len(n)): 
                            res = n
            ...
            if (LanguageHelper.is_cyrillic_char(n[0])): 
                ...
                for sl in self.slots: 
                    if (sl.type_name == PersonReferent.ATTR_LASTNAME): 
                        ss = (sl.value if isinstance(sl.value, str) else None)
    ...
    def _add_fio_identity(self, last_name : 'PersonMorphCollection', first_name : 'PersonMorphCollection', middle_name : object) -> None:
        ...
        if (first_name is not None): 
            ...
            if (isinstance(middle_name, str)): 
                ...
                mm = (middle_name if isinstance(middle_name, PersonMorphCollection) else None)
    ...
    def can_be_equals(self, obj : 'Referent', typ : 'EqualType') -> bool:
        p = (obj if isinstance(obj, PersonReferent) else None)
        if (p is None): 
            return False
        for a in self.slots: 
            if (a.type_name == PersonReferent.ATTR_IDENTITY): 
                for aa in p.slots: 
                    if (aa.type_name == a.type_name): 
                        if (PersonReferent._del_surname_end(a.value if isinstance(a.value, str) else None) == PersonReferent._del_surname_end(aa.value if isinstance(aa.value, str) else None)): 
                            return True
    ...
    def can_be_general_for(self, obj : 'Referent') -> bool:
        ...
        for s in self.slots: 
            if (s.type_name == PersonReferent.ATTR_FIRSTNAME): 
                if (PersonReferent.__is_initial(s.value if isinstance(s.value, str) else None)): 
                    name_inits += 1
                else: 
                    name_fulls += 1
            ...
            if (self.slots[i].type_name == PersonReferent.ATTR_ATTR and isinstance(self.slots[i].value, PersonPropertyReferent)): 
                if ((self.slots[i].value if isinstance(self.slots[i].value, PersonPropertyReferent) else None).tag is not None): 
                    ...
                    if (pr is not None and pr.general_referent is None): 
                        pr.general_referent = (self.slots[i].value if isinstance(self.slots[i].value, PersonPropertyReferent) else None)
                    del self.slots[i]
