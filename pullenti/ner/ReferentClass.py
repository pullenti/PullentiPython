...
class ReferentClass:
    ...
    def add_feature(self, attr_name : str, attr_caption : str, low_bound : int=0, up_bound : int=0) -> 'Feature':
        ...
        """
        res = Feature._new2926(attr_name, attr_caption, low_bound, up_bound)
    ...
    def find_feature(self, name_ : str) -> 'Feature':
        ...
        """
        wrapres2927 = RefOutArgWrapper(None)
        inoutres2928 = Utils.tryGetValue(self.__m_attrs, name_, wrapres2927)
        res = wrapres2927.value
        if (not inoutres2928): 
            return None
        else: 
            return res
