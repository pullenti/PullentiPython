...
class NounPhraseItem(MetaToken):
    ...
    def is_pronoun(self) -> bool:
        return self.begin_token.morph.class0.is_pronoun
    
    @property
    def is_personal_pronoun(self) -> bool:
        return self.begin_token.morph.class0.is_personal_pronoun
    
    @property
    def is_verb(self) -> bool:
        """ Это признак причастия """
        return self.begin_token.morph.class0.is_verb
    
    @property
    def is_adverb(self) -> bool:
        return self.begin_token.morph.class0.is_adverb
    ...
    def __corr_chars(self, str0 : str, keep : bool) -> str:
        ...
        for it in self.morph.items: 
            ...
            if (single_number and v.single_number_value is not None): 
                if (mc is not None and ((gender == MorphGender.NEUTER or gender == MorphGender.FEMINIE)) and mc.is_adjective): 
                    bi = MorphBaseInfo._new1430(MorphClass(mc), gender, MorphNumber.SINGULAR, MorphCase.NOMINATIVE, self.morph.language)
                    str0 = Morphology.get_wordform(v.single_number_value, bi)
                    if (str0 is not None): 
                        res = str0
                else: 
            ...
            if (v.normal_value[0].isdigit() and mc is not None and mc.is_adjective): 
                ...
                inoutres1432 = Utils.tryParseInt(v.normal_value, inoutarg1431)
                val = inoutarg1431.value
                if (inoutres1432): 
                    str0 = NumberHelper.get_number_adjective(val, gender, (MorphNumber.SINGULAR if single_number else MorphNumber.PLURAL))
                    if (str0 is not None): 
                        res = str0
                        if (v.undef_coef == 0): 
                            break
    ...
    def try_parse(t : 'Token', items : typing.List['NounPhraseItem'], attrs : 'NounPhraseParseAttr') -> 'NounPhraseItem':
        ...
        if (isinstance(t, TextToken)): 
            ...
            if (str0[len(str0) - 1] == 'А' or str0[len(str0) - 1] == 'О'): 
                    ...
                    if (isinstance(wf, MorphWordForm) and (wf if isinstance(wf, MorphWordForm) else None).is_in_dictionary): 
                        if (wf.class0.is_verb): 
                            ...
                            if (not mc.is_noun and ((attrs & NounPhraseParseAttr.IGNOREPARTICIPLES)) == NounPhraseParseAttr.NO): 
                                if (not LanguageHelper.ends_with_ex(str0, "ОГО", "ЕГО", None, None)): 
                                    return None
                            has_legal_verb = True
                        if (wf.class0.is_adverb): 
                            if (t.next0 is None or not t.next0.is_hiphen): 
                                if ((str0 == "ВСЕГО" or str0 == "ДОМА" or str0 == "НЕСКОЛЬКО") or str0 == "МНОГО" or str0 == "ПОРЯДКА"): 
                                    pass
                                else: 
                                    return None
            ...
            if (mc0.is_proper_surname and not t.chars.is_all_lower): 
                for wf in t.morph.items: 
                    if (wf.class0.is_proper_surname and wf.number != MorphNumber.PLURAL): 
            ...
            if (mc0.is_proper_name and not t.chars.is_all_lower): 
                for wff in t.morph.items: 
                    ...
                    if (wf.normal_case == "ГОР"): 
                        continue
                    if (wf.class0.is_proper_name and wf.is_in_dictionary): 
            ...
            if (mc1 == MorphClass.VERB): 
                return None
            if ((((attrs & NounPhraseParseAttr.IGNOREPARTICIPLES)) == NounPhraseParseAttr.IGNOREPARTICIPLES and t.morph.class0.is_verb and not t.morph.class0.is_noun) and not t.morph.class0.is_proper): 
                for wf in t.morph.items: 
                    if (wf.class0.is_verb): 
        ...
        for k in range(2):
            ...
            if (k == 0): 
                if ((((isinstance(t0, TextToken))) and t0.next0 is not None and t0.next0.is_hiphen) and t0.next0.next0 is not None): 
                    if (not t0.is_whitespace_after and not t0.morph.class0.is_pronoun): 
                        if (not t0.next0.is_whitespace_after): 
                            t = t0.next0.next0
                        elif (t0.next0.next0.chars.is_all_lower and LanguageHelper.ends_with((t0 if isinstance(t0, TextToken) else None).term, "О")): 
                            t = t0.next0.next0
            it = NounPhraseItem._new1433(t0, t, _can_be_surname)
            can_be_prepos = False
            for v in t.morph.items: 
                wf = (v if isinstance(v, MorphWordForm) else None)
                if (v.class0.is_preposition): 
                    can_be_prepos = True
                if (v.class0.is_adjective or ((v.class0.is_pronoun and not v.class0.is_personal_pronoun)) or ((v.class0.is_noun and isinstance(t, NumberToken)))): 
                    if (NounPhraseItem.try_accord_variant(items, (0 if items is None else len(items)), v)): 
                        ...
                        if (isinstance(t, TextToken)): 
                            ...
                            if (v.class0.is_noun and t.morph.class0.is_adjective): 
                                ok = False
                            elif (t.morph.class0.is_noun and ((attrs & NounPhraseParseAttr.PARSENUMERICASADJECTIVE)) == NounPhraseParseAttr.NO): 
                                ok = False
                        if (ok): 
                ...
                if (isinstance(t, NumberToken)): 
                    pass
                elif (v.class0.is_noun or ((wf is not None and wf.normal_case == "САМ"))): 
                    can_be_noun_ = True
                elif (v.class0.is_personal_pronoun): 
                    if (items is None or len(items) == 0): 
                        ...
                        if (len(items) == 1): 
                            if (items[0].can_be_adj_for_personal_pronoun): 
                                can_be_noun_ = True
                elif ((v.class0.is_pronoun and ((items is None or len(items) == 0 or ((len(items) == 1 and items[0].can_be_adj_for_personal_pronoun)))) and wf is not None) and ((((wf.normal_case == "ТОТ" or wf.normal_full == "ТО" or wf.normal_case == "ТО") or wf.normal_case == "ЭТО" or wf.normal_case == "ВСЕ") or wf.normal_case == "ЧТО" or wf.normal_case == "КТО"))): 
                    if (wf.normal_case == "ВСЕ"): 
                        if (t.next0 is not None and t.next0.is_value("РАВНО", None)): 
                            return None
                    can_be_noun_ = True
                elif (wf is not None and ((Utils.ifNotNull(wf.normal_full, wf.normal_case))) == "КОТОРЫЙ"): 
                    return None
                elif (v.class0.is_proper and isinstance(t, TextToken)): 
                    if (t.length_char > 4): 
                        can_be_noun_ = True
                if (can_be_noun_): 
                    if (NounPhraseItem.try_accord_variant(items, (0 if items is None else len(items)), v)): 
            ...
            if (can_be_prepos and it.can_be_noun): 
                if (items is not None and len(items) > 0): 
                    ...
                    npt1 = NounPhraseHelper.try_parse(t.next0, Utils.valToEnum(NounPhraseParseAttr.PARSEPRONOUNS | NounPhraseParseAttr.PARSEVERBS, NounPhraseParseAttr), 0)
                    if (npt1 is not None): 
            ...
            if (it.can_be_noun or it.can_be_adj or k == 1): 
                ...
                return it
            if (t0 == t): 
                if (t0.is_value("БИЗНЕС", None) and t0.next0 is not None and t0.next0.chars == t0.chars): 
                    t1 = t0.next0
                    continue
                return it
        return None
    ...
    @staticmethod
    def _new1433(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NounPhraseItem':
