    ...
    def __calc_rank_and_value(self, min_newlines_count : int) -> bool:
        ...
        while True:
            ...
            if (isinstance(t, NumberToken)): 
                if ((t if isinstance(t, NumberToken) else None).typ == NumberSpellingType.WORDS): 
                    words += 1
                    if (t.chars.is_all_upper): 
                        up_words += 1
                else: 
            ...
            if (pat is not None): 
                if (t.is_newline_before): 
                    if (not pat.morph.case.is_undefined and not pat.morph.case.is_nominative): 
                        pass
            ...
            if (oitt is not None): 
                if (oitt.morph.number != MorphNumber.PLURAL and not oitt.is_doubt_root_word): 
                    if (not oitt.morph.case.is_undefined and not oitt.morph.case.is_nominative): 
                        words += 1
                        if (t.chars.is_all_upper): 
                            up_words += 1
                    else: 
