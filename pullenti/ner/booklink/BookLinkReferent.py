...
class BookLinkReferent(Referent):
    ...
    @property
    def year(self) -> int:
        wrapyear397 = RefOutArgWrapper(0)
        inoutres398 = Utils.tryParseInt(Utils.ifNotNull(self.get_string_value(BookLinkReferent.ATTR_YEAR), ""), wrapyear397)
        year_ = wrapyear397.value
        if (inoutres398): 
            return year_
        else: 
            return 0
