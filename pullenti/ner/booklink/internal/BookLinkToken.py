...
class BookLinkToken(MetaToken):
    ...
    def try_parse_author(t : 'Token', prev_pers_template : 'FioTemplateType'=FioTemplateType.UNDEFINED) -> 'BookLinkToken':
        ...
        if (rtp is not None): 
            if (rtp.data is None): 
                re = BookLinkToken._new360(t, (t if rtp == t else rtp.end_token), BookLinkTyp.PERSON, rtp.referent)
            else: 
        ...
        if (((t.is_value("И", None) or t.is_value("ET", None))) and t.next0_ is not None): 
            if (t.next0_.is_value("ДРУГИЕ", None) or t.next0_.is_value("ДР", None) or t.next0_.is_value("AL", None)): 
                res = BookLinkToken._new362(t, t.next0_, BookLinkTyp.ANDOTHERS)
    ...
    def __try_parse(t : 'Token', lev : int) -> 'BookLinkToken':
        ...
        if (t.is_char('[')): 
            ...
            if (br is not None): 
                if ((isinstance(br.end_token.previous, NumberToken)) and (br.length_char < 30)): 
                    return BookLinkToken._new363(t, br.end_token, BookLinkTyp.NUMBER, MiscHelper.get_text_value(br.begin_token.next0_, br.end_token.previous, GetTextAttr.NO))
        t0 = t
        if (isinstance(t, ReferentToken)): 
            ...
            if (isinstance(t.get_referent(), GeoReferent)): 
                return BookLinkToken._new360(t, t, BookLinkTyp.GEO, t.get_referent())
            if (isinstance(t.get_referent(), DateReferent)): 
                ...
                if (len(dr.slots) == 1 and dr.year > 0): 
                    return BookLinkToken._new363(t, t, BookLinkTyp.YEAR, str(dr.year))
            ...
            if (isinstance(t.get_referent(), OrganizationReferent)): 
                ...
                if (org0_.kind == OrganizationKind.PRESS): 
                    return BookLinkToken._new360(t, t, BookLinkTyp.PRESS, org0_)
            if (isinstance(t.get_referent(), UriReferent)): 
                ...
                if ((uri.scheme == "http" or uri.scheme == "https" or uri.scheme == "ftp") or uri.scheme is None): 
                    return BookLinkToken._new360(t, t, BookLinkTyp.URL, uri)
            ...
            if (typ_ == BookLinkTyp.ELECTRONRES): 
                ...
                while True:
                    ...
                    if (isinstance(tt.get_referent(), UriReferent)): 
                        return BookLinkToken._new360(t, tt, BookLinkTyp.ELECTRONRES, tt.get_referent())
                    break
        if (t.is_char('/')): 
            res = BookLinkToken._new363(t, t, BookLinkTyp.DELIMETER, "/")
        ...
        if ((isinstance(t, NumberToken)) and (t).int_value is not None and (t).typ == NumberSpellingType.DIGIT): 
            res = BookLinkToken._new363(t, t, BookLinkTyp.NUMBER, str((t).value))
            val = (t).int_value
        ...
        if (isinstance(t, TextToken)): 
            ...
            if (((((((term == "СТР" or term == "C" or term == "С") or term == "ТОМ" or term == "T") or term == "Т" or term == "P") or term == "PP" or term == "V") or term == "VOL" or term == "S") or term == "СТОР" or t.is_value("PAGE", None)) or t.is_value("СТРАНИЦА", "СТОРІНКА")): 
                ...
                if (isinstance(tt, NumberToken)): 
                    res = BookLinkToken._new362(t, tt, BookLinkTyp.PAGERANGE)
                    tt0 = tt
                    tt1 = tt
                    tt = tt.next0_
                    first_pass2934 = True
            ...
            if ((term == "M" or term == "М" or term == "СПБ") or term == "K" or term == "К"): 
                if (t.next0_ is not None and t.next0_.is_char_of(":;")): 
                    re = BookLinkToken._new362(t, t.next0_, BookLinkTyp.GEO)
                    return re
                if (t.next0_ is not None and t.next0_.is_char_of(".")): 
                    res = BookLinkToken._new362(t, t.next0_, BookLinkTyp.GEO)
            ...
            if (term == "ПЕР" or term == "ПЕРЕВ" or term == "ПЕРЕВОД"): 
                ...
                if (tt.next0_ is not None and ((tt.next0_.is_value("C", None) or tt.next0_.is_value("С", None)))): 
                    ...
                    if (tt.next0_ is None or tt.whitespaces_after_count > 2): 
                        return None
                    re = BookLinkToken._new362(t, tt.next0_, BookLinkTyp.TRANSLATE)
                    return re
            if (term == "ТАМ" or term == "ТАМЖЕ"): 
                res = BookLinkToken._new362(t, t, BookLinkTyp.TAMZE)
            ...
            if (isinstance(no, NumberToken)): 
                return BookLinkToken._new362(t, no, BookLinkTyp.N)
            if (((term == "B" or term == "В")) and (isinstance(t.next0_, NumberToken)) and (isinstance(t.next0_.next0_, TextToken))): 
                ...
                if (((term2 == "Т" or term2 == "T" or term2.startswith("ТОМ")) or term2 == "TT" or term2 == "ТТ") or term2 == "КН" or term2.startswith("КНИГ")): 
                    return BookLinkToken._new362(t, t.next0_.next0_, BookLinkTyp.VOLUME)
        if (t.is_char('(')): 
            if (((isinstance(t.next0_, NumberToken)) and (t.next0_).int_value is not None and t.next0_.next0_ is not None) and t.next0_.next0_.is_char(')')): 
                ...
                if (num > 1900 and num <= 2040): 
                    if (num <= datetime.datetime.now().year): 
                        return BookLinkToken._new363(t, t.next0_.next0_, BookLinkTyp.YEAR, str(num))
    ...
    def check_link_before(t0 : 'Token', num : str) -> bool:
        ...
        if (t0.previous is not None and (isinstance(t0.previous.get_referent(), BookLinkRefReferent))): 
            wrapnn383 = RefOutArgWrapper(0)
            inoutres384 = Utils.tryParseInt(Utils.ifNotNull((t0.previous.get_referent()).number, ""), wrapnn383)
            nn = wrapnn383.value
            if (inoutres384): 
                if (str((nn + 1)) == num): 
                    return True
    ...
    def check_link_after(t1 : 'Token', num : str) -> bool:
        ...
        if (t1.is_newline_after): 
            ...
            if (bbb is not None and bbb.typ == BookLinkTyp.NUMBER): 
                wrapnn385 = RefOutArgWrapper(0)
                inoutres386 = Utils.tryParseInt(Utils.ifNotNull(bbb.value, ""), wrapnn385)
                nn = wrapnn385.value
                if (inoutres386): 
                    if (str((nn - 1)) == num): 
                        return True
    ...
    def initialize() -> None:
        ...
        tt = Termin._new135("ТЕКСТ", BookLinkTyp.NAMETAIL)
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new135("ЭЛЕКТРОННЫЙ РЕСУРС", BookLinkTyp.ELECTRONRES)
        ...
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new135("РЕЖИМ ДОСТУПА", BookLinkTyp.MISC)
        tt.add_variant("РЕЖИМ ДОСТУПУ", False)
        tt.add_variant("AVAILABLE", False)
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new135("МОНОГРАФИЯ", BookLinkTyp.TYPE)
        tt.add_variant("МОНОГРАФІЯ", False)
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new135("УЧЕБНОЕ ПОСОБИЕ", BookLinkTyp.TYPE)
        ...
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new136("ПІД РЕДАКЦІЄЮ", BookLinkTyp.EDITORS, MorphLang.UA)
        ...
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new135("СОСТАВИТЕЛЬ", BookLinkTyp.SOSTAVITEL)
        tt.add_abridge("СОСТ.")
        BookLinkToken.__m_termins.add(tt)
        tt = Termin._new136("УКЛАДАЧ", BookLinkTyp.SOSTAVITEL, MorphLang.UA)
        tt.add_abridge("УКЛ.")
        BookLinkToken.__m_termins.add(tt)
        for s in ["Политиздат", "Прогресс", "Мысль", "Просвещение", "Наука", "Физматлит", "Физматкнига", "Инфра-М", "Питер", "Интеллект", "Аспект пресс", "Аспект-пресс", "АСВ", "Радиотехника", "Радио и связь", "Лань", "Академия", "Академкнига", "URSS", "Академический проект", "БИНОМ", "БВХ", "Вильямс", "Владос", "Волтерс Клувер", "Wolters Kluwer", "Восток-Запад", "Высшая школа", "ГЕО", "Дашков и К", "Кнорус", "Когито-Центр", "КолосС", "Проспект", "РХД", "Статистика", "Финансы и статистика", "Флинта", "Юнити-дана"]: 
            BookLinkToken.__m_termins.add(Termin._new135(s.upper(), BookLinkTyp.PRESS))
        tt = Termin._new135("ИЗДАТЕЛЬСТВО", BookLinkTyp.PRESS)
    ...
    @staticmethod
    def _new360(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BookLinkTyp', _arg4 : 'Referent') -> 'BookLinkToken':
    ...
    @staticmethod
    def _new361(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BookLinkTyp', _arg4 : 'ReferentToken') -> 'BookLinkToken':
        res = BookLinkToken(_arg1, _arg2)
        res.typ = _arg3
        res.tok = _arg4
        return res
    ...
    @staticmethod
    def _new362(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BookLinkTyp') -> 'BookLinkToken':
        res = BookLinkToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new363(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BookLinkTyp', _arg4 : str) -> 'BookLinkToken':
        res = BookLinkToken(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        return res