...
class UriItemToken(MetaToken):
    ...
    def attachisocontent(t0 : 'Token', spec_chars : str) -> 'UriItemToken':
        ...
        if (txt.tell() == 0): 
            return None
        return UriItemToken._new2366(t0, t1, Utils.toStringStringIO(txt))
    ...
    def __attach_uri_content(t0 : 'Token', chars_ : str, can_be_whitespaces : bool=False) -> 'UriItemToken':
                ...
                if (t.previous.is_hiphen): 
                    ...
                    while True:
                        ...
                        if (not ((isinstance(tt1, TextToken)))): 
                            break
                        term1 = (tt1 if isinstance(tt1, TextToken) else None).term
        ...
        i = 0
        while i < txt.tell(): 
            if (Utils.getCharAtStringIO(txt, i).isalnum()): 
                break
            i += 1
        if (i >= txt.tell()): 
            return dom
        ...
        if (Utils.compareStrings(tmp, "WWW", True) == 0): 
            return None
        res = UriItemToken._new2366(t0, t1, Utils.toStringStringIO(txt))
        return res
    ...
    def attach_domain_name(t0 : 'Token', check_ : bool, can_be_whitspaces : bool) -> 'UriItemToken':
        ...
        while True:
            ...
            if (isinstance(t, NumberToken)): 
                ...
                print(nt.get_source_text(), end="", file=txt)
                t1 = t
                if (nt.typ == NumberSpellingType.DIGIT and nt.value >= 0 and (nt.value < 256)): 
                    ip_count += 1
                else: 
                    is_ip = False
                continue
            ...
            if (tt is None): 
                break
            src = (tt if isinstance(tt, TextToken) else None).term
            ch = src[0]
            if (not ch.isalpha()): 
                if ((ch) not in ".-_"): 
                    break
        ...
        if (check_): 
            ...
            if (not ok): 
                return None
        return UriItemToken._new2366(t0, t1, Utils.toStringStringIO(txt).lower())
    ...
    def attach_mail_users(t1 : 'Token') -> typing.List['UriItemToken']:
        ...
        if (txt.tell() == 0): 
            return None
        res = list()
        res.append(UriItemToken._new2366(t0, t1, Utils.toStringStringIO(txt).lower()))
        return res
    ...
    def attach_url(t0 : 'Token') -> 'UriItemToken':
        ...
        t1 = srv.end_token
        if (t1.next0 is not None and t1.next0.is_char(':') and isinstance(t1.next0.next0, NumberToken)): 
            t1 = t1.next0.next0
            print(":{0}".format((t1 if isinstance(t1, NumberToken) else None).value), end="", file=txt, flush=True)
        t = t1.next0
        while t is not None: 
            ...
            if (t.is_whitespace_after): 
                t1 = t
                break
            dat = UriItemToken.__attach_uri_content(t.next0, ".-_+%", False)
            if (dat is None): 
                t1 = t
                break
            t1 = dat.end_token
            ...
            t = t.next0
        if ((t1.next0 is not None and t1.next0.is_char('?') and not t1.next0.is_whitespace_after) and not t1.is_whitespace_after): 
            dat = UriItemToken.__attach_uri_content(t1.next0.next0, ".-_+%=&", False)
            if (dat is not None): 
                t1 = dat.end_token
                print("?{0}".format(dat.value), end="", file=txt, flush=True)
        if ((t1.next0 is not None and t1.next0.is_char('#') and not t1.next0.is_whitespace_after) and not t1.is_whitespace_after): 
    ...
    def attachisbn(t0 : 'Token') -> 'UriItemToken':
        ...
        while True:
            ...
            if (isinstance(t, NumberToken)): 
                nt = (t if isinstance(t, NumberToken) else None)
                if (nt.typ != NumberSpellingType.DIGIT or not nt.morph.class0.is_undefined): 
                    break
        ...
        i = 0
        while i < txt.tell(): 
            if (Utils.getCharAtStringIO(txt, i).isdigit()): 
                dig += 1
            i += 1
        if (dig < 7): 
            return None
    ...
    def attachbbk(t0 : 'Token') -> 'UriItemToken':
        ...
        while True:
            ...
            if (tt.is_char(',')): 
                break
            if (tt.is_char('(')): 
                if (not ((isinstance(tt.next0, NumberToken)))): 
                    break
    ...
    @staticmethod
    def _new2366(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'UriItemToken':
