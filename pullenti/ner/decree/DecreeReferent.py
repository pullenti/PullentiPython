    ...
    def __allYears(self) -> typing.List[int]:
        ...
        for s in self.slots: 
            if (s.type_name == DecreeReferent.ATTR_DATE): 
                ...
                if (i == 4): 
                    str0_ = str0_[0:0+4]
                wrapi1101 = RefOutArgWrapper(0)
                inoutres1102 = Utils.tryParseInt(str0_, wrapi1101)
                i = wrapi1101.value
                if (inoutres1102): 
                    res.append(i)
        return res
    ...
    def _checkCorrection(self, noun_is_doubtful : bool) -> bool:
        ...
        if (((("ОПРЕДЕЛЕНИЕ" in typ_ or "РЕШЕНИЕ" in typ_ or "ПОСТАНОВЛЕНИЕ" in typ_) or "ПРИГОВОР" in typ_ or "ВИЗНАЧЕННЯ" in typ_) or "РІШЕННЯ" in typ_ or "ПОСТАНОВА" in typ_) or "ВИРОК" in typ_): 
            if (self.number is not None): 
                if (self.findSlot(DecreeReferent.ATTR_DATE, None, True) is not None or self.findSlot(DecreeReferent.ATTR_SOURCE, None, True) is not None): 
                    return True
