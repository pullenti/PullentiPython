...
class DecreeAnalyzer(Analyzer):
    ...
    class ThisDecree(MetaToken):
        ...
        def try_attach_back(t : 'Token', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (ukaz is not None): 
                if (base_typ is not None and base_typ.value is not None and (("ДОГОВОР" in base_typ.value or "ДОГОВІР" in base_typ.value))): 
                    return DecreeAnalyzer.ThisDecree._new1077(ukaz, ukaz, True, base_typ.value)
            return None
        ...
        def try_attach(dtok : 'PartToken', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (not ((isinstance(tt, TextToken)))): 
                if ((isinstance(tt, ReferentToken)) and (isinstance(tt.get_referent(), DecreeReferent))): 
                    return DecreeAnalyzer.ThisDecree._new1078(t, tt, (tt.get_referent() if isinstance(tt.get_referent(), DecreeReferent) else None))
                return None
        ...
        @staticmethod
        def _new1077(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : str) -> 'ThisDecree':
        ...
        @staticmethod
        def _new1078(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeReferent') -> 'ThisDecree':
            res = DecreeAnalyzer.ThisDecree(_arg1, _arg2)
            res.real = _arg3
            return res
    ...
    def images(self) -> typing.List[tuple]:
        ...
        from pullenti.ner.decree.internal.MetaDecreeChangeValue import MetaDecreeChangeValue
        res = dict()
        res[MetaDecree.DECREE_IMAGE_ID] = ResourceHelper.get_bytes("decree.png")
        res[MetaDecree.STANDADR_IMAGE_ID] = ResourceHelper.get_bytes("decreestd.png")
        res[MetaDecreePart.PART_IMAGE_ID] = ResourceHelper.get_bytes("part.png")
        res[MetaDecreePart.PART_LOC_IMAGE_ID] = ResourceHelper.get_bytes("document_into.png")
        res[MetaDecree.PUBLISH_IMAGE_ID] = ResourceHelper.get_bytes("publish.png")
        res[MetaDecreeChange.IMAGE_ID] = ResourceHelper.get_bytes("decreechange.png")
        res[MetaDecreeChangeValue.IMAGE_ID] = ResourceHelper.get_bytes("decreechangevalue.png")
        return res
    ...
    def process(self, kit : 'AnalysisKit') -> None:
            ...
            if (dts is None or len(dts) == 0 or ((len(dts) == 1 and dts[0].typ == DecreeToken.ItemType.TYP))): 
                ...
                if (rt0 is not None): 
                    ...
                    if (mt is not None): 
                        ...
                        mt = (rt0.tag if isinstance(rt0.tag, MetaToken) else None)
                        if ((mt) is not None): 
                            if (aliases is not None): 
                                term = Termin()
                                term.init_by(mt.begin_token, mt.end_token.previous, rt0.referent, False)
                ...
                if (rts is not None): 
                    ...
                    while i < len(rts): 
                        ...
                        if (isinstance(rtt.referent, DecreePartReferent)): 
                            (rtt.referent if isinstance(rtt.referent, DecreePartReferent) else None).owner = (ad.register_referent((rtt.referent if isinstance(rtt.referent, DecreePartReferent) else None).owner) if isinstance(ad.register_referent((rtt.referent if isinstance(rtt.referent, DecreePartReferent) else None).owner), DecreeReferent) else None)
            ...
            if (rtli is not None): 
                ...
                while ii < len(rtli): 
                    ...
                    if (mt is not None): 
                        ...
                        mt = (rt.tag if isinstance(rt.tag, MetaToken) else None)
                        if ((mt) is not None): 
                            if (aliases is not None): 
                                term = Termin()
                                term.init_by(mt.begin_token, mt.end_token.previous, rt.referent, False)
                ...
                if (dts[0].chars.is_capital_upper and not dts[0].is_doubtful): 
                    ...
                    if (base_typ is not None and dts[0].ref is not None): 
                        drr = (dts[0].ref.get_referent() if isinstance(dts[0].ref.get_referent(), DecreeReferent) else None)
                        if (drr is not None): 
                ...
                for i in range(len(li) - 1, 0, -1):
                    if (li[i].typ == li[i - 1].typ): 
                        if (li[i].date is not None and li[i - 1].date is None): 
                            li[i - 1].add_slot(DecreeReferent.ATTR_DATE, li[i].get_value(DecreeReferent.ATTR_DATE), False, 0)
                else: i = 0
                i = 0
                while i < (len(li) - 1): 
                    if (li[i].typ == li[i + 1].typ): 
        ...
        while True:
            ...
            if (rts is not None): 
                for kp in rts: 
                    dpr_list = (kp.tag if isinstance(kp.tag, list) else None)
                    if (dpr_list is None): 
                        continue
                    i = 0
                    while i < len(dpr_list): 
                            ...
                            while len(change_stack) > 0:
                                if (dr._is_all_items_less_level(change_stack[0], False)): 
                                    if (isinstance(change_stack[0], DecreePartReferent)): 
                                        dr._add_high_level_info(change_stack[0] if isinstance(change_stack[0], DecreePartReferent) else None)
                                    break
                                if (isinstance(change_stack[0], DecreePartReferent)): 
                                    del change_stack[0]
                        if (last_change is not None and len(last_change.owners) > 0): 
                            dr0 = (last_change.owners[0] if isinstance(last_change.owners[0], DecreePartReferent) else None)
                        ...
                        dr = (ad.register_referent(dr) if isinstance(ad.register_referent(dr), DecreePartReferent) else None)
                        if (dprs is None): 
                            dprs = list()
                        dprs.append(dr)
                        if (i == 0): 
            ...
            if (dcht is not None and has_start_change > 0): 
                ...
                if (dcht.typ == DecreeChangeTokenTyp.STARTMULTU): 
                    ...
                    if (len(change_stack) > 0 or dcht.decree is not None): 
                        root_change = (ad.register_referent(DecreeChangeReferent._new1066(DecreeChangeKind.CONTAINER)) if isinstance(ad.register_referent(DecreeChangeReferent._new1066(DecreeChangeKind.CONTAINER)), DecreeChangeReferent) else None)
                ...
                if (dprs is not None and len(dprs) > 0): 
                    ...
                    if (chrt is None and expire_regime): 
                        chrt = list()
                        dcr = DecreeChangeReferent._new1066(DecreeChangeKind.EXPIRE)
                    ...
                    if (dcht.part_typ != PartToken.ItemType.UNDEFINED): 
                        for ss in change_stack: 
                            if (isinstance(ss, DecreePartReferent)): 
                                if ((ss if isinstance(ss, DecreePartReferent) else None)._is_all_items_over_this_level(dcht.part_typ)): 
                                    ee = True
                ...
                if (chrt is not None): 
                    for rt in chrt: 
                        ...
                        if (isinstance(rt.referent, DecreeChangeReferent)): 
                            last_change = (rt.referent if isinstance(rt.referent, DecreeChangeReferent) else None)
                            if (dprs is not None): 
                                ii = 0
                                while ii < (len(dprs) - 1): 
    ...
    def _try_attach_approved(t : 'Token', ad : 'AnalyzerData') -> 'ReferentToken':
        ...
        while True:
            ...
            if (DecreeToken.is_keyword(tt, False) is not None and tt.chars.is_capital_upper): 
                break
            rt0 = DecreeAnalyzer.__try_attach_approved(tt, ad)
            if (rt0 is not None): 
                t1 = tt.previous
    ...
    @staticmethod
    def __try_attach_approved(t : 'Token', ad : 'AnalyzerData') -> 'ReferentToken':
        from pullenti.ner.geo.GeoReferent import GeoReferent
        from pullenti.ner.decree.internal.DecreeToken import DecreeToken
        from pullenti.ner.date.DateReferent import DateReferent
        from pullenti.ner.decree.DecreeReferent import DecreeReferent
        ...
        olev = None
        lev = 0
        inoutarg1070 = RefOutArgWrapper(None)
        inoutres1071 = Utils.tryGetValue(kit.misc_data, "dovr", inoutarg1070)
        olev = inoutarg1070.value
        if (not inoutres1071): 
            lev = 1
        ...
        try: 
            ...
            if (rt is None): 
                ...
                while ii < len(dts): 
                    if (dts[ii].typ == DecreeToken.ItemType.NUMBER): 
                        has_num += 1
                    elif ((dts[ii].typ == DecreeToken.ItemType.DATE and dts[ii].ref is not None and (isinstance(dts[ii].ref.referent, DateReferent))) and (dts[ii].ref.referent if isinstance(dts[ii].ref.referent, DateReferent) else None).dt is not None): 
                        has_date += 1
            ...
            if (((rt is None and len(dts) == 1 and dts[0].typ == DecreeToken.ItemType.DATE) and dts[0].ref is not None and (isinstance(dts[0].ref.referent, DateReferent))) and (dts[0].ref.referent if isinstance(dts[0].ref.referent, DateReferent) else None).dt is not None): 
                dr = DecreeReferent()
                dr._add_date(dts[0])
                rt = list()
                rt.append(ReferentToken(dr, dts[0].begin_token, dts[len(dts) - 1].end_token))
    ...
    @staticmethod
    def __get_decree(t : 'Token') -> 'DecreeReferent':
        from pullenti.ner.ReferentToken import ReferentToken
        from pullenti.ner.decree.DecreeReferent import DecreeReferent
        from pullenti.ner.decree.DecreePartReferent import DecreePartReferent
        ...
        if (isinstance(r, DecreeReferent)): 
            return (r if isinstance(r, DecreeReferent) else None)
        if (isinstance(r, DecreePartReferent)): 
            return (r if isinstance(r, DecreePartReferent) else None).owner
        return None
    
    @staticmethod
    def __check_other_typ(t : 'Token', first : bool) -> 'Token':
        from pullenti.ner.decree.internal.DecreeToken import DecreeToken
    ...
    def __try_attach_pulishers(self, dts : typing.List['DecreeToken']) -> typing.List['ReferentToken']:
        ...
        while True:
            ...
            if (pt is not None): 
                ...
                if (pub_part is None): 
                    pub_part = DecreePartReferent._new1072(pub)
    ...
    def _try_attach_parts(parts : typing.List['PartToken'], base_typ : 'DecreeToken', _def_owner : 'Referent') -> typing.List['MetaToken']:
        ...
        if (parts[len(parts) - 1].typ != PartToken.ItemType.SUBPROGRAM and parts[len(parts) - 1].typ != PartToken.ItemType.ADDAGREE): 
            ...
            if (this_dec is not None): 
                if ((isinstance(_def_owner, DecreeReferent)) and (((_def_owner if isinstance(_def_owner, DecreeReferent) else None).typ0 == this_dec.typ or parts[0].typ == PartToken.ItemType.APPENDIX))): 
                    pass
                else: 
                    _def_owner = (None)
                ...
                for p in parts: 
                    if (p.typ == PartToken.ItemType.PART): 
                        ...
                        if (isinstance(_def_owner, DecreePartReferent)): 
                            if ((_def_owner if isinstance(_def_owner, DecreePartReferent) else None).clause is not None): 
                                has_clause = True
                        if (not has_clause): 
            ...
            is_program = True
        def_owner = (_def_owner if isinstance(_def_owner, DecreeReferent) else None)
        if (isinstance(_def_owner, DecreePartReferent)): 
            def_owner = (_def_owner if isinstance(_def_owner, DecreePartReferent) else None).owner
        res = list()
        has_prefix = False
        ...
        for j in range(len(res) - 1, 0, -1):
            d0 = (res[j].referent if isinstance(res[j].referent, DecreePartReferent) else None)
            d = (res[j - 1].referent if isinstance(res[j - 1].referent, DecreePartReferent) else None)
        ...
        if (owner is None and this_dec is not None): 
            ...
            if (tt0.previous is not None): 
                owner = (tt0.previous.get_referent() if isinstance(tt0.previous.get_referent(), DecreeReferent) else None)
                if ((owner) is not None): 
        ...
        if ((this_dec is None or not this_dec.has_this_ref)): 
            ...
            while True:
                ...
                if (brack_level > 0): 
                    continue
                dr = DecreeAnalyzer.__get_decree(tt)
                if (dr is not None and dr.kind != DecreeKind.PUBLISHER): 
                    ...
                    if (dr.kind == DecreeKind.CONTRACT): 
                        if (is_add_agree): 
                            ...
                            continue
                    if (this_dec is not None): 
                        dpr = (tt.get_referent() if isinstance(tt.get_referent(), DecreePartReferent) else None)
                        if (this_dec.typ == dr.typ or this_dec.typ == dr.typ0): 
                            ...
                            for r in res: 
                                dpr0 = (r.referent if isinstance(r.referent, DecreePartReferent) else None)
                        ...
                        if (owner is None): 
                            coef_before = 0
                        elif (owner == DecreeAnalyzer.__get_decree(tt)): 
                            coef_before = 0
                    bt = tt
                    break
                if (dr is not None): 
                    continue
                dpr2 = (tt.get_referent() if isinstance(tt.get_referent(), DecreePartReferent) else None)
                if (dpr2 is not None): 
                    bt = tt
                    break
            ...
            while True:
                ...
                if (dr is not None): 
                    continue
                tte2 = DecreeAnalyzer.__check_other_typ(tt, tt == tt0)
                if (tte2 is not None): 
                    at = tte2
                    break
            ...
            if (owner is None): 
                if (at is not None): 
                    owner = DecreeAnalyzer.__get_decree(at)
                    if (isinstance(at, TextToken)): 
                        if (isinstance(at.tag, str)): 
                            loc_typ = (at.tag if isinstance(at.tag, str) else None)
                        else: 
                            loc_typ = (at if isinstance(at, TextToken) else None).lemma
                elif (bt is not None): 
                    owner = DecreeAnalyzer.__get_decree(bt)
                    owner_paer = (bt.get_referent() if isinstance(bt.get_referent(), DecreePartReferent) else None)
                ...
                if (owner_paer is not None and loc_typ is None): 
                    loc_typ = owner_paer.local_typ
            if (((bt is not None and len(parts) == 1 and parts[0].typ == PartToken.ItemType.DOCPART) and (isinstance(bt.get_referent(), DecreePartReferent)) and (bt.get_referent() if isinstance(bt.get_referent(), DecreePartReferent) else None).clause is not None) and len(res) == 1 and owner == (bt.get_referent() if isinstance(bt.get_referent(), DecreePartReferent) else None).owner): 
                for s in res[0].referent.slots: 
                    if (s.type_name == DecreePartReferent.ATTR_DOCPART): 
                        s.type_name = DecreePartReferent.ATTR_PART
                (res[0].referent if isinstance(res[0].referent, DecreePartReferent) else None)._add_high_level_info(bt.get_referent() if isinstance(bt.get_referent(), DecreePartReferent) else None)
        if (owner is None): 
            if (this_dec is None and loc_typ is None): 
                if ((len(parts) == 1 and len(parts[0].values) == 1 and parts[0].typ == PartToken.ItemType.APPENDIX) and parts[0].begin_token.chars.is_capital_upper): 
                    ...
                    return None
            for r in res: 
                dr = (r.referent if isinstance(r.referent, DecreePartReferent) else None)
                if (this_dec is not None): 
        ...
        if (is_program): 
            for i in range(len(res) - 1, -1, -1):
                pa = (res[i].referent if isinstance(res[i].referent, DecreePartReferent) else None)
        ...
        while i < len(res): 
            ...
            while j < len(res): 
                if (res[j].begin_token != res[i].begin_token): 
                    ...
                    li.append(res[j].referent if isinstance(res[j].referent, DecreePartReferent) else None)
                j += 1
            if (j < len(res)): 
        ...
        if ((len(dts) == 2 and dts[0].typ == DecreeToken.ItemType.TYP and dts[0].typ_kind == DecreeKind.STANDARD) and dts[1].typ == DecreeToken.ItemType.NUMBER): 
            ...
            while ttt is not None: 
                ...
                if (nu is None or nu.typ != DecreeToken.ItemType.NUMBER): 
                    break
                dr1 = DecreeReferent._new1079(dec.typ)
                dr1._add_number(nu)
