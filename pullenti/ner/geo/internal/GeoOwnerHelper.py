...
class GeoOwnerHelper:
    ...
    @staticmethod
    def _can_be_higher(rhi : 'Token', rlo : 'Token') -> bool:
        from pullenti.ner.geo.GeoReferent import GeoReferent
        ...
        if (rlo.previous is not None and rlo.previous.morph.class0.is_preposition): 
            if (rlo.previous.morph.language.is_ua): 
                if ((rlo.previous.is_value("У", None) and not rlo.morph.case.is_dative and not rlo.morph.case.is_prepositional) and not rlo.morph.case.is_undefined): 
                    return False
                if (rlo.previous.is_value("З", None) and not rlo.morph.case.is_genitive and not rlo.morph.case.is_undefined): 
                    return False
            else: 
                if ((rlo.previous.is_value("В", None) and not rlo.morph.case.is_dative and not rlo.morph.case.is_prepositional) and not rlo.morph.case.is_undefined): 
                    return False
                if (rlo.previous.is_value("ИЗ", None) and not rlo.morph.case.is_genitive and not rlo.morph.case.is_undefined): 
                    return False
