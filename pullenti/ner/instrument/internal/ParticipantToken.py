...
class ParticipantToken(MetaToken):
    ...
    def try_attach(t : 'Token', p1 : 'InstrumentParticipant'=None, p2 : 'InstrumentParticipant'=None, is_contract : bool=False) -> 'ParticipantToken':
        ...
        if (p1 is None and p2 is None and is_contract): 
            r1 = t.get_referent()
            if ((r1 is not None and t.next0 is not None and t.next0.is_comma_and) and isinstance(t.next0.next0, ReferentToken)): 
                r2 = t.next0.next0.get_referent()
                if (r1.type_name == r2.type_name): 
                    ttt = t.next0.next0.next0
                    refs = list()
                    refs.append(r1)
                    refs.append(r2)
                    first_pass2785 = True
                    while True:
                        ...
                        if ((ttt.is_comma_and and ttt.next0 is not None and ttt.next0.get_referent() is not None) and ttt.next0.get_referent().type_name == r1.type_name): 
                            ttt = ttt.next0
                    ...
                    while True:
                        ...
                        if (not (ttt is not None)): break
                        if (ttt.is_comma or ttt.morph.class0.is_preposition): 
                            continue
                        ...
                        if (npt is not None and npt.noun.is_value("СТОРОНА", None) and npt.morph.number != MorphNumber.SINGULAR): 
                            re = ParticipantToken._new1404(t, npt.end_token, ParticipantToken.Kinds.NAMEDASPARTS)
                            re.parts = refs
                            return re
                        break
            if (isinstance(r1, OrganizationReferent) or isinstance(r1, PersonReferent)): 
                ...
                while ttt is not None and (ttt.end_char < t.end_char): 
                    if (ttt.is_char('(')): 
                        ...
                        has_named = True
                    elif ((ttt.is_comma or ttt.morph.class0.is_preposition or ttt.is_hiphen) or ttt.is_char(':')): 
                        pass
                        ...
                        if (npt is None): 
                            break
                        if (has_br): 
                            if (npt.end_token.next0 is None or not npt.end_token.next0.is_char(')')): 
                                break
                        if (not has_named): 
                            if (ParticipantToken._m_ontology.try_parse(ttt, TerminParseAttr.NO) is None): 
                                break
                        re = ParticipantToken._new1404(t, t, ParticipantToken.Kinds.NAMEDAS)
                ...
                while True:
                    ...
                    if (not (ttt is not None)): break
                    if (isinstance(ttt, NumberToken) and isinstance(ttt.next0, TextToken) and (ttt.next0 if isinstance(ttt.next0, TextToken) else None).term == "СТОРОНЫ"): 
                        ttt = ttt.next0
                        end_side = ttt
                        if (ttt.next0 is not None and ttt.next0.is_comma): 
                            ttt = ttt.next0
                        if (ttt.next0 is not None and ttt.next0.is_and): 
                            break
                    ...
                    if (BracketHelper.can_be_start_of_sequence(ttt, False, False)): 
                        ...
                        if (npt is not None): 
                            ...
                            if (isinstance(ttt, MetaToken)): 
                                ttok = ParticipantToken._m_ontology.try_parse((ttt if isinstance(ttt, MetaToken) else None).begin_token, TerminParseAttr.NO)
                            if (ttok is not None): 
                                ...
                                if (brr is not None): 
                                    continue
                                break
                        if (BracketHelper.can_be_end_of_sequence(ttt.next0, True, None, False)): 
                            ttt = ttt.next0
                        if (brr is not None): 
                            if (ttt.next0 is None): 
                                ttt = brr.end_token
                                continue
                            ttt = ttt.next0
                        if (not has_named and typ22 is None): 
                            if (ParticipantToken._m_ontology.try_parse(npt.begin_token, TerminParseAttr.NO) is None): 
                                break
                        re = ParticipantToken._new1404(t, ttt, ParticipantToken.Kinds.NAMEDAS)
                        re.typ = (Utils.ifNotNull(typ22, (npt.get_normal_case_text(MorphClass(), True, MorphGender.UNDEFINED, False) if npt is not None else None)))
                        re.parts = list()
                        re.parts.append(r1)
                        return re
                    elif (ttt.is_value("ЗАРЕГИСТРИРОВАННЫЙ", None) or ttt.is_value("КАЧЕСТВО", None) or ttt.is_value("ПРОЖИВАЮЩИЙ", None)): 
                        pass
                        ...
                        npt = NounPhraseHelper.try_parse(ttt, NounPhraseParseAttr.NO, 0)
                        if (npt is not None): 
                            if (npt.noun.is_value("МЕСТО", None) or npt.noun.is_value("ЖИТЕЛЬСТВО", None)): 
                                ttt = npt.end_token
                                continue
                ...
                if (end_side is not None or ((add_refs is not None and t.previous is not None and t.previous.is_and))): 
                    re = ParticipantToken._new1404(t, Utils.ifNotNull(end_side, t), ParticipantToken.Kinds.NAMEDAS)
                    re.typ = None
                    re.parts = list()
                    re.parts.append(r1)
                    ...
                    return re
            too = ParticipantToken._m_ontology.try_parse(t, TerminParseAttr.NO)
            if (too is not None and too.termin.tag is not None and too.termin.canonic_text != "СТОРОНА"): 
                tt1 = too.end_token.next0
                if (tt1 is not None): 
                ...
                if (isinstance(tt1, ReferentToken)): 
                    ...
                    if (isinstance(r1, PersonReferent) or isinstance(r1, OrganizationReferent)): 
                        re = ParticipantToken._new1404(t, tt1, ParticipantToken.Kinds.NAMEDAS)
        ...
        if (BracketHelper.can_be_start_of_sequence(tt, False, False) and tt.next0 is not None): 
            br = True
            tt = tt.next0
        term1 = None
        term2 = None
        ...
        while True:
            ...
            if (tt.morph.class0.is_preposition and typ_ is not None): 
                continue
            ...
            if (BracketHelper.is_bracket(tt, False)): 
                continue
            tok = ParticipantToken._m_ontology.try_parse(tt, TerminParseAttr.NO)
            if (tok is None): 
        ...
        if (BracketHelper.can_be_end_of_sequence(t1.next0, False, None, False)): 
            ...
            if (not t.is_whitespace_before and BracketHelper.can_be_start_of_sequence(t.previous, False, False)): 
                t = t.previous
        elif (BracketHelper.can_be_start_of_sequence(t, False, False) and BracketHelper.can_be_end_of_sequence(t1.next0, True, t, True)): 
            t1 = t1.next0
        if (br and t1.next0 is not None and BracketHelper.can_be_end_of_sequence(t1.next0, False, None, False)): 
            t1 = t1.next0
        res = ParticipantToken._new1409(t, t1, (ParticipantToken.Kinds.NAMEDAS if named else ParticipantToken.Kinds.PURE), typ_)
        if (t.is_char(':')): 
            res.begin_token = t.next0
        return res
    ...
    def try_attach_to_exist(t : 'Token', p1 : 'InstrumentParticipant', p2 : 'InstrumentParticipant') -> 'ReferentToken':
        ...
        if (rt.end_token.next0 is not None and rt.end_token.next0.is_char(':')): 
            rt1 = ParticipantToken.try_attach_requisites(rt.end_token.next0.next0, p, (p2 if p == p1 else p1), False)
            if (rt1 is not None): 
                rt1.begin_token = rt.begin_token
                return rt1
            rt.end_token = rt.end_token.next0
        while rt.end_token.next0 is not None and isinstance(rt.end_token.next0.get_referent(), OrganizationReferent):
            org = (rt.end_token.next0.get_referent() if isinstance(rt.end_token.next0.get_referent(), OrganizationReferent) else None)
            if (rt.referent.find_slot(None, org, True) is not None): 
                rt.end_token = rt.end_token.next0
                continue
            break
        return rt
            ...
            if (t.whitespaces_before_count > 25 and not is_in_tab_cell): 
                ...
                if (isinstance(t.get_referent(), PersonReferent) or isinstance(t.get_referent(), OrganizationReferent)): 
                    if (not cur._contains_ref(t.get_referent())): 
                        break
            if ((t.is_char_of(";:,.") or t.is_hiphen or t.morph.class0.is_preposition) or t.morph.class0.is_conjunction): 
                continue
            if (t.is_char_of("_/\\")): 
                spec_chars += 1
            ...
            if (t.is_value("ОФИС", None)): 
                if (BracketHelper.can_be_start_of_sequence(t.next0, True, False)): 
                    ...
                    if (br is not None): 
                        t = br.end_token
                        continue
                if (isinstance(t.next0, TextToken) and not t.next0.chars.is_all_lower): 
                    t = t.next0
                continue
            r = t.get_referent()
    ...
    def attach_first(self, p : 'InstrumentParticipant', min_char : int, max_char : int) -> 'ReferentToken':
        ...
        while True:
            ...
            r = t.get_referent()
            if ((((isinstance(r, OrganizationReferent) or isinstance(r, PhoneReferent) or isinstance(r, PersonReferent)) or isinstance(r, PersonPropertyReferent) or isinstance(r, AddressReferent)) or isinstance(r, UriReferent) or isinstance(r, PersonIdentityReferent)) or isinstance(r, BankDataReferent)): 
                if (((isinstance(r, PersonPropertyReferent) and t.next0 is not None and t.next0.is_comma) and isinstance(t.next0.next0, ReferentToken) and isinstance(t.next0.next0.get_referent(), PersonReferent)) and not t.next0.is_newline_after): 
                    pe = (t.next0.next0.get_referent() if isinstance(t.next0.next0.get_referent(), PersonReferent) else None)
            ...
            if (tt is not None): 
                ...
                t = rt.end_token
                continue
            if (t.is_value("В", None) and t.next0 is not None and t.next0.is_value("ЛИЦО", None)): 
                t = t.next0
                continue
            if (t.is_value("ОТ", None) and t.next0 is not None and t.next0.is_value("ИМЯ", None)): 
                t = t.next0
                continue
            if (t.is_value("ПО", None) and t.next0 is not None and t.next0.is_value("ПОРУЧЕНИЕ", None)): 
                t = t.next0
                continue
            if (t.is_newline_before): 
                break
            ...
            if (t.is_and and t.previous is not None and t.previous.is_comma): 
                break
            if (t.is_and and t.next0.get_referent() is not None): 
                if (isinstance(t.next0.get_referent(), OrganizationReferent)): 
                    break
                pe = (t.next0.get_referent() if isinstance(t.next0.get_referent(), PersonReferent) else None)
                if (pe is not None): 
                    has_ip = False
                    for s in pe.slots: 
    ...
    def __try_attach_contract_ground(t : 'Token', ip : 'InstrumentParticipant', can_be_passport : bool=False) -> 'Token':
        ...
        while True:
            ...
            if (not (t is not None)): break
            if (t.is_char(',') or t.morph.class0.is_preposition): 
                continue
            if (t.is_char('(')): 
    ...
    def initialize() -> None:
        ...
        from pullenti.ner.core.Termin import Termin
        if (ParticipantToken._m_ontology is not None): 
            return
        ParticipantToken._m_ontology = TerminCollection()
        for s in ["АРЕНДОДАТЕЛЬ", "АРЕНДАТОР", "СУБАРЕНДАТОР", "НАЙМОДАТЕЛЬ", "НАНИМАТЕЛЬ", "АГЕНТ", "ПРИНЦИПАЛ", "ПРОДАВЕЦ", "ПОКУПАТЕЛЬ", "ЗАКАЗЧИК", "ИСПОЛНИТЕЛЬ", "ПОСТАВЩИК", "ПОДРЯДЧИК", "СУБПОДРЯДЧИК", "СТОРОНА", "ЛИЦЕНЗИАР", "ЛИЦЕНЗИАТ", "СТРАХОВЩИК", "СТРАХОВАТЕЛЬ", "ПРОВАЙДЕР", "АБОНЕНТ", "ЗАСТРОЙЩИК", "УЧАСТНИК ДОЛЕВОГО СТРОИТЕЛЬСТВА", "КЛИЕНТ", "ЗАЕМЩИК", "УПРАВЛЯЮЩИЙ"]: 
            ParticipantToken._m_ontology.add(Termin._new118(s, ParticipantToken._m_ontology))
        t = Termin._new118("ГЕНПОДРЯДЧИК", ParticipantToken._m_ontology)
        t.add_variant("ГЕНЕРАЛЬНЫЙ ПОДРЯДЧИК", False)
        ParticipantToken._m_ontology.add(t)
        t = Termin._new118("ЗАИМОДАТЕЛЬ", ParticipantToken._m_ontology)
    ...
    @staticmethod
    def _new1318(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'ParticipantToken':
        res = ParticipantToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new1404(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Kinds') -> 'ParticipantToken':
        res = ParticipantToken(_arg1, _arg2)
        res.kind = _arg3
        return res
    
    @staticmethod
    def _new1409(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Kinds', _arg4 : str) -> 'ParticipantToken':
        res = ParticipantToken(_arg1, _arg2)
        res.kind = _arg3
        res.typ = _arg4
        return res