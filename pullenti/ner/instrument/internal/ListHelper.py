...
class ListHelper:
    ...
    class LineToken(MetaToken):
        ...
        def correctBeginToken(self) -> None:
            ...
            elif ((self.number > 0 and self.begin_token.next0_ is not None and self.begin_token.next0_.isChar(')')) and self.begin_token.next0_.next0_ is not None): 
                self.begin_token = self.begin_token.next0_.next0_
        
        def __str__(self) -> str:
            return "{0}: {1}".format(("LISTITEM" if self.is_list_item else "TEXT"), self.getSourceText())
        ...
        def parse(t : 'Token', max_char : int, prev : 'LineToken') -> 'LineToken':
            ...
            while True:
                ...
                if (t.isChar(':')): 
                    if (res.is_newline_before and res.begin_token.isValue("ПРИЛОЖЕНИЕ", "ДОДАТОК")): 
                ...
                if (t.isChar(';')): 
                    if (not t.is_whitespace_after): 
                        pass
                    if (t.previous is not None and (isinstance(t.previous.getReferent(), DecreeReferent))): 
                        if (not t.is_whitespace_after): 
                            continue
                        if (t.next0_ is not None and (isinstance(t.next0_.getReferent(), DecreeReferent))): 
                            continue
                ...
                if (t.is_newline_before and t != res.begin_token): 
                    next0__ = True
                    if (t.previous.is_comma or t.previous.is_and or t.isCharOf("(")): 
                        next0__ = False
            ...
            elif (res.begin_token.next0_ is not None and ((res.begin_token.next0_.isChar(')') or ((prev is not None and ((prev.is_list_item or prev.is_list_head))))))): 
        ...
        @staticmethod
        def parseList(t : 'Token', max_char : int, prev : 'LineToken') -> typing.List['LineToken']:
            ...
            if ((len(res) < 2) and not res[0].is_list_item): 
                if ((prev is not None and prev.is_list_item and res[0].end_token.isChar('.')) and not res[0].begin_token.chars.is_capital_upper): 
            ...
            while j < len(res): 
                if (not res[j].is_list_item): 
                    if (res[j - 1].is_list_item and res[j].end_token.isChar('.')): 
                        if (res[j].begin_token.getSourceText() == res[i].begin_token.getSourceText() or res[j].begin_token.chars.is_all_lower): 
    ...
    def analyze(res : 'FragToken') -> None:
        ...
        if (len(res.children) == 0): 
            ...
            if (((ki == InstrumentKind.CHAPTER or ki == InstrumentKind.CLAUSE or ki == InstrumentKind.CONTENT) or ki == InstrumentKind.ITEM or ki == InstrumentKind.SUBITEM) or ki == InstrumentKind.CLAUSEPART or ki == InstrumentKind.INDENTION): 
                tmp = list()
                tmp.append(res)
                ListHelper.__analizeListItems(tmp, 0)
            return
        ...
        while True:
            ...
            if (res.children[i].kind == InstrumentKind.INDENTION and ((res.children[i].end_token.isCharOf(":;") or ((((i + 1) < len(res.children)) and res.children[i + 1].kind == InstrumentKind.EDITIONS and res.children[i + 1].end_token.isCharOf(":;")))))): 
                cou = 1
                list_bullet = chr(0)
                j = (i + 1)
                first_pass3019 = True
                while True:
                    ...
                    if (ch.kind != InstrumentKind.INDENTION): 
                        break
                    if (ch.end_token.isCharOf(";") or ((((j + 1) < len(res.children)) and res.children[j + 1].kind == InstrumentKind.EDITIONS and res.children[j + 1].end_token.isChar(';')))): 
                        cou += 1
                        if ((isinstance(ch.begin_token, TextToken)) and not ch.chars.is_letter): 
                            list_bullet = ch.kit.getTextCharacter(ch.begin_char)
                        continue
                    if (ch.end_token.isCharOf(".")): 
                        ...
                        break
                    if (ch.end_token.isCharOf(":")): 
                        if ((ord(list_bullet)) != 0 and ch.begin_token.isChar(list_bullet)): 
                            tt = ch.begin_token.next0_
                            while tt is not None and (tt.end_char < ch.end_char): 
                                if (tt.previous.isChar('.') and MiscHelper.canBeStartOfSentence(tt)): 
                                    ch2 = FragToken._new1274(tt, ch.end_token, InstrumentKind.INDENTION, ch.number)
                ...
                if (cou < 3): 
                    i = j
                    continue
                if ((i > 0 and not res.children[i].end_token.isChar(':') and res.children[i - 1].kind2 == InstrumentKind.UNDEFINED) and res.children[i - 1].end_token.isChar(':')): 
                ...
                while True:
                    ...
                    elif (((i + 1) < j) and res.children[i + 1].kind == InstrumentKind.EDITIONS and res.children[i + 1].end_token.isChar(':')): 
                        ch.kind2 = InstrumentKind.LISTHEAD
                    else: 
        ...
        while i < len(res.children): 
            ...
            if (len(res.children[i].children) > 0): 
                ...
                co = ListHelper.__analizeListItems(res.children, i)
                if (co > 0): 
    ...
    def __analizeListItems(chi : typing.List['FragToken'], ind : int) -> int:
        ...
        if (res.has_changes and res.multiline_changes_value is not None): 
            ...
            cit = FragToken._new1257(ci.begin_token, ci.end_token, InstrumentKind.CITATION)
            ...
            if (BracketHelper.isBracket(cit.end_token.next0_, True)): 
                ...
                if (cit.end_token.next0_ is not None and cit.end_token.next0_.isCharOf(";.")): 
                    cit.end_token = cit.end_token.next0_
            ...
            if (isinstance(ci.tag, DecreeChangeReferent)): 
                ...
                if (dcr.value is not None and len(dcr.value.new_items) > 0): 
                    ...
                    if (((i)) > 0): 
                        mnem = mnem[0:0+i]
                    cit_kind = PartToken._getInstrKindByTyp(PartToken._getTypeByAttrName(mnem))
            ...
            if (cit_kind != InstrumentKind.UNDEFINED and cit_kind != InstrumentKind.APPENDIX): 
                ...
                sub = FragToken.createDocument(ci.begin_token, ci.end_char, cit_kind)
        ...
        if (res.kind == InstrumentKind.CONTENT): 
            ...
            while j < len(chi): 
                if (chi[j].kind == InstrumentKind.CONTENT): 
                    lines2 = ListHelper.LineToken.parseList(chi[j].begin_token, chi[j].end_char, lines[len(lines) - 1])
                    if (lines2 is None or (len(lines2) < 1)): 
                        break
                    if (not lines2[0].is_list_item): 
                        if ((len(lines2) > 1 and lines2[1].is_list_item and lines2[0].end_token.isCharOf(":")) and not lines2[0].begin_token.chars.is_capital_upper): 
        ...
        if (len(lines) > 2): 
            ...
            if ((not last.is_list_item and last.end_token.isChar('.') and last2.is_list_item) and last2.end_token.isChar(';')): 
        ...
        while i < (len(lines) - 1): 
            if (not lines[i].is_list_item and not lines[i + 1].is_list_item): 
                if (((i + 2) < len(lines)) and lines[i + 2].is_list_item and lines[i + 1].end_token.isChar(':')): 
                    pass
                else: 
                ...
                if (not ok): 
                    ...
                    while i < len(lines): 
                        if (not lines[i].is_list_item): 
                            ...
                            lines[i].number = 0
                        i += 1
                if (cou > 3 and lines[i0].begin_token.getSourceText() != lines[i0 + 1].begin_token.getSourceText() and lines[i0 + 1].begin_token.getSourceText() == lines[i0 + 2].begin_token.getSourceText()): 
        ...
        for li in lines: 
            ...
            ch = FragToken._new1274(li.begin_token, li.end_token, (InstrumentKind.LISTITEM if li.is_list_item else InstrumentKind.CONTENT), li.number)
            if (ch.kind == InstrumentKind.CONTENT and ch.end_token.isChar(':')): 
                ch.kind = InstrumentKind.LISTHEAD
            res.children.append(ch)
            chli = Utils.asObjectOrNull(li.tag, list)
            if (chli is not None): 
                for lt in chli: 
                    ch.children.append(FragToken._new1257(lt.begin_token, lt.end_token, InstrumentKind.LISTITEM))
                if (ch.begin_char < ch.children[0].begin_char): 
                    ch.children.insert(0, FragToken._new1257(ch.begin_token, ch.children[0].begin_token.previous, InstrumentKind.CONTENT))
        return ret
            ...
            if ((lines[i].typ == InstrToken1.Types.LINE and len(lines[i].numbers) == 0 and lines[i].begin_token.isValue("ПРИЛОЖЕНИЯ", "ДОДАТОК")) and len(lines[i + 1].numbers) > 0 and lines[i].end_token.isChar(':')): 
                ...
                while True:
                    ...
                    if (len(lines[i].numbers) == 0): 
                        ...
                        wrapnn1461 = RefOutArgWrapper(0)
                        inoutres1462 = Utils.tryParseInt(lines[i].numbers[0], wrapnn1461)
                        nn = wrapnn1461.value
                        if (inoutres1462): 
                            num = (nn + 1)
    ...
    def correctIndex(lines : typing.List['InstrToken1']) -> None:
        ...
        while i < len(lines): 
            if (lines[i].typ == lines[0].typ): 
                if (ListHelper.__canBeEquals(lines[i], lines[0])): 
                    break
                else: 
        ...
        if (len(index) == len(content) and len(index) > 2): 
            if ((ind_text * 10) < con_text): 
                lines[0] = InstrToken1._new1463(lines[0].begin_token, lines[cind - 1].end_token, True, InstrToken1.Types.INDEX)
        ...
        if (len(i1.numbers) > 0 and len(i2.numbers) > 0): 
            ...
            while i < len(i1.numbers): 
                if (i1.numbers[i] != i2.numbers[i]): 
                    return False
                i += 1
        if (not MiscHelper.canBeEqualsEx(i1.value, i2.value, Utils.valToEnum((CanBeEqualsAttrs.IGNORENONLETTERS) | (CanBeEqualsAttrs.IGNOREUPPERCASE), CanBeEqualsAttrs))): 
            return False
        return True