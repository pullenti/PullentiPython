...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                wrapnn2557 = RefOutArgWrapper(None)
                inoutres2558 = Utils.tryGetValue(tn.children, dig, wrapnn2557)
                nn = wrapnn2557.value
                if (not inoutres2558): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            wrapnn2559 = RefOutArgWrapper(None)
            inoutres2560 = Utils.tryGetValue(nod.children, dig, wrapnn2559)
            nn = wrapnn2559.value
            if (not inoutres2560): 
                break
