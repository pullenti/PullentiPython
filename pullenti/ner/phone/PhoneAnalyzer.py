...
class PhoneAnalyzer(Analyzer):
    ...
    class PhoneAnalizerData(AnalyzerData):
        ...
        def register_referent(self, referent : 'Referent') -> 'Referent':
            ...
            phone_ = (referent if isinstance(referent, PhoneReferent) else None)
            if (phone_ is None): 
                return None
            key = phone_.number
            if (len(key) >= 10): 
        ...
        while True:
            ...
            while tt is not None: 
                if (isinstance(tt.get_referent(), PhoneReferent)): 
                    prev_phone = (tt.get_referent() if isinstance(tt.get_referent(), PhoneReferent) else None)
                    break
                elif (tt.is_char(')')): 
                    ttt = tt.previous
                    cou = 0
            ...
            if (rts is None): 
                ...
                if ((ki == PhoneKind.UNDEFINED and prev_phone is not None and not is_pref) and prev_phone.kind != PhoneKind.MOBILE): 
                    ki = prev_phone.kind
                for rt in rts: 
                    ph = (rt.referent if isinstance(rt.referent, PhoneReferent) else None)
                    if (ki != PhoneKind.UNDEFINED): 
                        ph.kind = ki
                    else: 
    ...
    def __get_next_phone(self, t : 'Token', lev : int) -> 'PhoneReferent':
        ...
        if (rt is None): 
            return None
        return (rt.referent if isinstance(rt.referent, PhoneReferent) else None)
