...
class DefinitionAnalyzer(Analyzer):
        ...
        if (kit.ontology is not None): 
            ...
            for it in kit.ontology.items: 
                if (isinstance(it.referent, DefinitionReferent)): 
                    ...
                    if (not termin in oh): 
                        oh[termin] = True
                        onto.add(Termin._new1083(termin, termin))
    ...
    def try_attach(self, t : 'Token', glos_regime : bool, onto : 'TerminCollection', max_char : int) -> typing.List['ReferentToken']:
        ...
        if (t.is_hiphen or t.is_char_of(":") or ((can_next_sent and t.is_char('.')))): 
            if (isinstance(t.next0_, TextToken) and (t.next0_ if isinstance(t.next0_, TextToken) else None).term == "ЭТО"): 
                ok = 2
                t = t.next0_.next0_
            elif (glos_regime): 
                ok = 2
                ...
                if (npt is not None and npt.morph.case.is_nominative): 
                    ok = 2
                    t = tt
                elif ((tt is not None and tt.morph.case.is_nominative and tt.morph.class0_.is_verb) and tt.morph.class0_.is_adjective): 
                    ok = 2
                    t = tt
            else: 
        ...
        if (nam is None): 
            return None
        if (l0 == l1 and l0.morph.class0_.is_adjective and l0.morph.case.is_instrumental): 
            if (t is not None and t.is_value("ТАКОЙ", None)): 
                ...
                if (npt is not None and npt.morph.case.is_nominative): 
                    ...
                    nam = "{0} {1}".format(str0_, npt.get_normal_case_text(MorphClass(), False, MorphGender.UNDEFINED, False))
    ...
    def __try_attach_end(self, t : 'Token', onto : 'TerminCollection', max_char : int) -> typing.List['ReferentToken']:
        ...
        if ((((l1.end_char - l0.end_char)) * 2) > ((r1.end_char - r0.end_char))): 
            return None
        dr = DefinitionReferent._new1084(DefinitionKind.DEFINITION)
    ...
    def __try_attach_misc_token(t : 'Token') -> 'MetaToken':
        ...
        if (t.is_value("КАК", None)): 
            ...
            while True:
                ...
                break
            if (t1 is not None): 
                res = MetaToken._new825(t, t1, MiscHelper.get_text_value(t, t1, GetTextAttr.KEEPQUOTES))
        ...
        if (npt is not None): 
            if (DefinitionAnalyzer.__m_misc_first_words.try_parse(npt.noun.begin_token, TerminParseAttr.NO) is not None): 
                res = MetaToken._new825(t, npt.end_token, npt.get_normal_case_text(MorphClass(), True, MorphGender.UNDEFINED, False))
        ...
        if (t.is_value("В", None)): 
            ...
            if (npt is not None): 
                if (npt.noun.is_value("СМЫСЛ", None)): 
                    res = MetaToken._new825(t, npt.end_token, MiscHelper.get_text_value(t, npt.end_token, GetTextAttr.NO))
                    res.morph.class0_ = MorphClass.NOUN
                    return res
        return None
