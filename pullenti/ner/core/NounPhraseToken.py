...
class NounPhraseToken(MetaToken):
        ...
        if (self.adverbs is not None and len(self.adverbs) > 0): 
            ...
            if (len(self.adjectives) > 0): 
                ...
                while j < len(self.adjectives): 
                    s = self.adjectives[j].getNormalCaseText((MorphClass.ADJECTIVE) | MorphClass.PRONOUN, single_number, gender, keep_chars)
                    ...
                    while i < len(self.adverbs): 
                        if (self.adverbs[i].begin_char < self.adjectives[j].begin_char): 
                            print("{0} ".format(self.adjectives[i].getNormalCaseText(MorphClass.ADVERB, False, MorphGender.UNDEFINED, False)), end="", file=res, flush=True)
                        else: 
                            break
                        i += 1
                    j += 1
            ...
            for t in self.adjectives: 
                s = t.getNormalCaseText((MorphClass.ADJECTIVE) | MorphClass.PRONOUN, single_number, gender, keep_chars)
                print("{0} ".format(Utils.ifNotNull(s, "?")), end="", file=res, flush=True)
        r = None
    ...
    def getNormalCaseTextWithoutAdjective(self, adj_index : int) -> str:
        res = io.StringIO()
        i = 0
        while i < len(self.adjectives): 
            if (i != adj_index): 
                s = self.adjectives[i].getNormalCaseText((MorphClass.ADJECTIVE) | MorphClass.PRONOUN, False, MorphGender.UNDEFINED, False)
                print("{0} ".format(Utils.ifNotNull(s, "?")), end="", file=res, flush=True)
            i += 1
