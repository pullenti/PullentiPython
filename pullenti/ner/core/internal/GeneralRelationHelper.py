...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new464(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new464(r, ad)
                all_refs.append(nod)
                r.tag = nod
                wrapsi467 = RefOutArgWrapper(None)
                inoutres468 = Utils.tryGetValue(all0_, a.name, wrapsi467)
                si = wrapsi467.value
                if (not inoutres468): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    li = [ ]
                    wrapli465 = RefOutArgWrapper(None)
                    inoutres466 = Utils.tryGetValue(si, s, wrapli465)
                    li = wrapli465.value
                    if (not inoutres466): 
                        li = list()
                        si[s] = li
