...
class NounPhraseHelper:
    ...
    @staticmethod
    def try_parse(t : 'Token', typ : 'NounPhraseParseAttr'=NounPhraseParseAttr.NO, max_char_pos : int=0, noun : 'MetaToken'=None) -> 'NounPhraseToken':
        ...
        res = _NounPraseHelperInt.try_parse(t, typ, max_char_pos, Utils.asObjectOrNull(noun, NounPhraseItem))
        if (res is not None): 
            if ((((typ) & (NounPhraseParseAttr.PARSEPREPOSITION))) != (NounPhraseParseAttr.NO)): 
                if (res.begin_token == res.end_token and t.morph.class0_.is_preposition): 
                    ...
                    if (prep is not None): 
                        res2 = _NounPraseHelperInt.try_parse(t.next0_, typ, max_char_pos, Utils.asObjectOrNull(noun, NounPhraseItem))
                        if (res2 is not None): 
        ...
        if ((((typ) & (NounPhraseParseAttr.PARSEPREPOSITION))) != (NounPhraseParseAttr.NO)): 
            ...
            if (prep is not None and (prep.whitespaces_after_count < 3)): 
                res = _NounPraseHelperInt.try_parse(prep.end_token.next0_, typ, max_char_pos, Utils.asObjectOrNull(noun, NounPhraseItem))
                if (res is not None): 
