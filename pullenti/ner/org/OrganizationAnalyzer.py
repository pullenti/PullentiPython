...
class OrganizationAnalyzer(Analyzer):
    ...
    class OrgAnalyzerData(AnalyzerDataWithOntology):
        ...
        def register_referent(self, referent : 'Referent') -> 'Referent':
            ...
            if (isinstance(referent, OrganizationReferent)): 
                (referent if isinstance(referent, OrganizationReferent) else None)._final_correction()
            ...
            if (not self.large_text_regim and isinstance(res, OrganizationReferent) and ((res == referent or len(res.slots) != slots))): 
                ioi = (res if isinstance(res, OrganizationReferent) else None).create_ontology_item_ex(3, True, False)
                if (ioi is not None): 
                    self.loc_orgs.add_item(ioi)
                names = (res if isinstance(res, OrganizationReferent) else None)._get_pure_names()
    ...
    def images(self) -> typing.List['java.util.Map.Entry']:
        from pullenti.ner.org.internal.MetaOrganization import MetaOrganization
        res = dict()
        res[Utils.enumToString(OrgProfile.UNIT)] = ResourceHelper.get_bytes("dep.png")
        res[Utils.enumToString(OrgProfile.UNION)] = ResourceHelper.get_bytes("party.png")
        res[Utils.enumToString(OrgProfile.COMPETITION)] = ResourceHelper.get_bytes("festival.png")
        res[Utils.enumToString(OrgProfile.HOLDING)] = ResourceHelper.get_bytes("holding.png")
        res[Utils.enumToString(OrgProfile.STATE)] = ResourceHelper.get_bytes("gov.png")
        res[Utils.enumToString(OrgProfile.FINANCE)] = ResourceHelper.get_bytes("bank.png")
        res[Utils.enumToString(OrgProfile.EDUCATION)] = ResourceHelper.get_bytes("study.png")
        res[Utils.enumToString(OrgProfile.SCIENCE)] = ResourceHelper.get_bytes("science.png")
        res[Utils.enumToString(OrgProfile.INDUSTRY)] = ResourceHelper.get_bytes("factory.png")
        res[Utils.enumToString(OrgProfile.TRADE)] = ResourceHelper.get_bytes("trade.png")
        res[Utils.enumToString(OrgProfile.POLICY)] = ResourceHelper.get_bytes("politics.png")
        res[Utils.enumToString(OrgProfile.JUSTICE)] = ResourceHelper.get_bytes("justice.png")
        res[Utils.enumToString(OrgProfile.ENFORCEMENT)] = ResourceHelper.get_bytes("gov.png")
        res[Utils.enumToString(OrgProfile.ARMY)] = ResourceHelper.get_bytes("military.png")
        res[Utils.enumToString(OrgProfile.SPORT)] = ResourceHelper.get_bytes("sport.png")
        res[Utils.enumToString(OrgProfile.RELIGION)] = ResourceHelper.get_bytes("church.png")
        res[Utils.enumToString(OrgProfile.MUSIC)] = ResourceHelper.get_bytes("music.png")
        res[Utils.enumToString(OrgProfile.MEDIA)] = ResourceHelper.get_bytes("media.png")
        res[Utils.enumToString(OrgProfile.PRESS)] = ResourceHelper.get_bytes("press.png")
        res[Utils.enumToString(OrgProfile.HOTEL)] = ResourceHelper.get_bytes("hotel.png")
        res[Utils.enumToString(OrgProfile.MEDICINE)] = ResourceHelper.get_bytes("medicine.png")
        res[Utils.enumToString(OrgProfile.TRANSPORT)] = ResourceHelper.get_bytes("train.png")
        res[Utils.enumToString(OrganizationKind.BANK)] = ResourceHelper.get_bytes("bank.png")
        res[Utils.enumToString(OrganizationKind.CULTURE)] = ResourceHelper.get_bytes("culture.png")
        res[Utils.enumToString(OrganizationKind.DEPARTMENT)] = ResourceHelper.get_bytes("dep.png")
        res[Utils.enumToString(OrganizationKind.FACTORY)] = ResourceHelper.get_bytes("factory.png")
        res[Utils.enumToString(OrganizationKind.GOVENMENT)] = ResourceHelper.get_bytes("gov.png")
        res[Utils.enumToString(OrganizationKind.MEDICAL)] = ResourceHelper.get_bytes("medicine.png")
        res[Utils.enumToString(OrganizationKind.PARTY)] = ResourceHelper.get_bytes("party.png")
        res[Utils.enumToString(OrganizationKind.STUDY)] = ResourceHelper.get_bytes("study.png")
        res[Utils.enumToString(OrganizationKind.FEDERATION)] = ResourceHelper.get_bytes("federation.png")
        res[Utils.enumToString(OrganizationKind.CHURCH)] = ResourceHelper.get_bytes("church.png")
        res[Utils.enumToString(OrganizationKind.MILITARY)] = ResourceHelper.get_bytes("military.png")
        res[Utils.enumToString(OrganizationKind.AIRPORT)] = ResourceHelper.get_bytes("avia.png")
        res[Utils.enumToString(OrganizationKind.FESTIVAL)] = ResourceHelper.get_bytes("festival.png")
        res[MetaOrganization.ORG_IMAGE_ID] = ResourceHelper.get_bytes("org.png")
        return res
    
    def create_referent(self, type0 : str) -> 'Referent':
        from pullenti.ner.org.OrganizationReferent import OrganizationReferent
        if (type0 == OrganizationReferent.OBJ_TYPENAME): 
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        ad = (kit.get_analyzer_data(self) if isinstance(kit.get_analyzer_data(self), OrganizationAnalyzer.OrgAnalyzerData) else None)
        ...
        while step < steps: 
            ...
            while t is not None: 
                ...
                if (step > 0 and isinstance(t, ReferentToken) and isinstance(t.get_referent(), OrganizationReferent)): 
                    mt = OrganizationAnalyzer._check_alias_after(t if isinstance(t, ReferentToken) else None, t.next0)
                    if (mt is not None): 
                        if (ad is not None): 
                            term = Termin()
                ...
                while True:
                    ...
                    emb = False
                    for rt in rts: 
                        if (not (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)._check_correction()): 
                            continue
                        ...
                        if (rt.begin_char <= t.begin_char): 
                            t = rt
                    if ((len(rts) == 1 and t == rts[0] and isinstance(t.next0, ReferentToken)) and isinstance(t.next0.get_referent(), OrganizationReferent)): 
                        org0 = (rts[0].referent if isinstance(rts[0].referent, OrganizationReferent) else None)
                        org1 = (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None)
                        if (org1.higher is None and OrgOwnershipHelper.can_be_higher(org0, org1, False) and not OrgOwnershipHelper.can_be_higher(org1, org0, False)): 
                            rtt = (t.next0 if isinstance(t.next0, ReferentToken) else None)
                            kit.debed_token(rtt)
                            org1.higher = org0
                            rt1 = ReferentToken._new695(ad.register_referent(org1), t, rtt.end_token, t.next0.morph)
                            kit.embed_token(rt1)
                            t = rt1
                ...
                if (isinstance(t, ReferentToken) and isinstance(t.get_referent(), OrganizationReferent)): 
                    rt0 = (t if isinstance(t, ReferentToken) else None)
        ...
        while True:
            ...
            org_ = (t.get_referent() if isinstance(t.get_referent(), OrganizationReferent) else None)
            if (org_ is None): 
                continue
            t1 = t.next0
            if (((t1 is not None and t1.is_char('(') and t1.next0 is not None) and isinstance(t1.next0.get_referent(), OrganizationReferent) and t1.next0.next0 is not None) and t1.next0.next0.is_char(')')): 
                org0 = (t1.next0.get_referent() if isinstance(t1.next0.get_referent(), OrganizationReferent) else None)
                if (org0 == org_ or org_.higher == org0): 
                    rt1 = ReferentToken._new695(org_, t, t1.next0.next0, t.morph)
                    kit.embed_token(rt1)
                    t = rt1
                    t1 = t.next0
                elif (org_.higher is None and OrgOwnershipHelper.can_be_higher(org0, org_, False) and not OrgOwnershipHelper.can_be_higher(org_, org0, False)): 
                    org_.higher = org0
                    rt1 = ReferentToken._new695(org_, t, t1.next0.next0, t.morph)
                    kit.embed_token(rt1)
                    t = rt1
            ...
            if (t1 is not None): 
                if (t1.is_char_of(",;") or t1.is_hiphen): 
                    ...
                    of_tok = (t1 if isinstance(t1, TextToken) else None)
                    t1 = t1.next0
            if (t1 is None): 
                break
            org1 = (t1.get_referent() if isinstance(t1.get_referent(), OrganizationReferent) else None)
            if (org1 is None): 
                continue
            if (of_tok is None): 
                if (org_.higher is None and not OrgOwnershipHelper.can_be_higher(org1, org_, False)): 
                    continue
            if (org_.higher is not None): 
                if (not org_.higher.can_be_equals(org1, Referent.EqualType.WITHINONETEXT)): 
                    continue
            list0.clear()
            list0.append(t if isinstance(t, ReferentToken) else None)
            list0.append(t1 if isinstance(t1, ReferentToken) else None)
            if (of_tok is not None and org_.higher is None): 
                t2 = t1.next0
                while t2 is not None: 
                    if ((isinstance(t2, TextToken) and (t2 if isinstance(t2, TextToken) else None).term == of_tok.term and t2.next0 is not None) and isinstance(t2.next0.get_referent(), OrganizationReferent)): 
                        t2 = t2.next0
                        if (org1.higher is not None): 
                            if (not org1.higher.can_be_equals(t2.get_referent(), Referent.EqualType.WITHINONETEXT)): 
                                break
                        list0.append(t2 if isinstance(t2, ReferentToken) else None)
                        org1 = (t2.get_referent() if isinstance(t2.get_referent(), OrganizationReferent) else None)
                    else: 
                        break
                    t2 = t2.next0
            rt0 = list0[len(list0) - 1]
            for i in range(len(list0) - 2, -1, -1):
                org_ = (list0[i].referent if isinstance(list0[i].referent, OrganizationReferent) else None)
                org1 = (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)
                if (org_.higher is None): 
                    org_.higher = org1
                    org_ = (ad.register_referent(org_) if isinstance(ad.register_referent(org_), OrganizationReferent) else None)
                rt = ReferentToken(org_, list0[i], rt0)
                kit.embed_token(rt)
                t = rt
                rt0 = rt
        owners = dict()
        ...
        while True:
            ...
            if (hi is None): 
                continue
            for ty in org_.types: 
                li = [ ]
                inoutarg2013 = RefOutArgWrapper(None)
                inoutres2014 = Utils.tryGetValue(owners, ty, inoutarg2013)
                li = inoutarg2013.value
                if (not inoutres2014): 
                    li = list()
                    owners[ty] = li
                childs = None
                if (not hi in li): 
                    ...
                    childs = (hi.tag if isinstance(hi.tag, list) else None)
                if (childs is not None and not org_ in childs): 
                    childs.append(org_)
        owns = list()
        t = kit.first_token
        first_pass2822 = True
        while True:
            ...
            if (not (t is not None)): break
            org_ = (t.get_referent() if isinstance(t.get_referent(), OrganizationReferent) else None)
            if (org_ is None or org_.higher is not None): 
                continue
            owns.clear()
            for ty in org_.types: 
                li = [ ]
                inoutarg2015 = RefOutArgWrapper(None)
                inoutres2016 = Utils.tryGetValue(owners, ty, inoutarg2015)
                li = inoutarg2015.value
                if (not inoutres2016): 
                    continue
                for h in li: 
                    if (not h in owns): 
            ...
            if (len(owns) != 1): 
                continue
            if (OrgOwnershipHelper.can_be_higher(owns[0], org_, True)): 
                childs = (owns[0].tag if isinstance(owns[0].tag, list) else None)
                if (childs is None): 
                    continue
                has_num = False
                has_geo = False
                for oo in childs: 
                    ...
                    if (oo.find_slot(OrganizationReferent.ATTR_NUMBER, None, True) is not None): 
                        has_num = True
                if (has_num != ((org_.find_slot(OrganizationReferent.ATTR_NUMBER, None, True) is not None))): 
                    continue
                if (has_geo != ((org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is not None))): 
                    continue
                org_.higher = owns[0]
    ...
    def _check_alias_after(rt : 'ReferentToken', t : 'Token') -> 'MetaToken':
        ...
        while t is not None:
            if (not ((isinstance(t, TextToken)))): 
                ...
                t = t.next0
            elif (t.morph.class0.is_preposition or t.morph.class0.is_misc or t.is_value("ИМЕНОВАТЬ", None)): 
                t = t.next0
            else: 
                break
        if (t is None): 
        ...
        if ((' ') not in nam): 
            ...
            while tt is not None and tt.end_char <= rt.end_char: 
                if (tt.is_value(mt.tag if isinstance(mt.tag, str) else None, None)): 
                    return mt
                tt = tt.next0
            return None
        return mt
    ...
    def __try_attach_orgs(self, t : 'Token', ad : 'OrgAnalyzerData', step : int) -> typing.List['ReferentToken']:
        ...
        if (t.chars.is_latin_letter and MiscHelper.is_eng_article(t)): 
            res11 = self.__try_attach_orgs(t.next0, ad, step)
            ...
            if (typ.root is None or not typ.root.is_pure_prefix): 
                if (((typ.morph.number & MorphNumber.PLURAL)) != MorphNumber.UNDEFINED): 
                    ...
                    if (t1.next0 is not None and t1.next0.is_char(',')): 
                        t1 = t1.next0
                        ok1 = True
                        if (t1.next0 is not None and t1.next0.is_value("КАК", None)): 
                            t1 = t1.next0
                        else: 
                            ok = False
                    if (t1.next0 is not None and t1.next0.is_value("КАК", None)): 
                    ...
                    if (ok): 
                        if (not ok1 and typ.coef > 0): 
                            ok1 = True
                        if (ok1): 
                            rt = self.__try_attach_org(t1.next0, ad, OrganizationAnalyzer.AttachType.MULTIPLE, typ, False, 0, -1)
                    if (rt is not None): 
                        self.__do_post_analyze(rt, ad)
                        res = list()
                        res.append(rt)
                        org_ = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
                        if (ok1): 
                            rt.begin_token = t
        ...
        if (typ is not None and ((typ.is_dep or typ.can_be_dep_before_organization))): 
            ...
            if (rt is None): 
                rt = self.__try_attach_org(typ.end_token.next0, ad, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, 0, -1)
            ...
            if (s.startswith("ГУ") and len(s) > 3 and ((len(s) > 4 or s == "ГУВД"))): 
                ...
                if (rt is not None): 
                    ...
                    if (rt0 is not None): 
                        ...
                        (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None).higher = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
                        li2 = list()
                        self.__do_post_analyze(rt0, ad)
                        li2.append(rt0)
                        return li2
            elif (((((((((s[0] == 'У' and len(s) > 3 and tt.get_morph_class_in_dictionary().is_undefined)) or s == "ОВД" or s == "РОВД") or s == "ОМВД" or s == "ОСБ") or s == "УПФ" or s == "УФНС") or s == "ИФНС" or s == "ИНФС") or s == "УВД" or s == "УФМС") or s == "ОУФМС" or s == "УФК") or s == "УФССП"): 
                ...
                tt.max_length = max0
                if (rt is not None): 
                    org1 = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
                    if (len(org1._geo_objects) == 0 and rt.end_token.next0 is not None): 
                        g = (rt.end_token.next0.get_referent() if isinstance(rt.end_token.next0.get_referent(), GeoReferent) else None)
                    ...
                    rt0 = self.__try_attach_dep_before_org(typ, rt)
                    if (rt0 is not None): 
                        org0 = (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)
                        org0.add_profile(OrgProfile.UNIT)
                        if (org0.number is None and not tt.is_newline_after): 
                            num = OrgItemNumberToken.try_attach(tt.next0, True, typ)
                            if (num is not None): 
                        ...
                        if (rt0.referent.find_slot(OrganizationReferent.ATTR_GEO, None, True) is None): 
                            geo_ = self.__is_geo(rt0.end_token.next0, False)
                            if ((geo_) is not None): 
                                if ((rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)._add_geo_object(geo_)): 
                                    rt0.end_token = self.__get_geo_end_token(geo_, rt0.end_token.next0)
                            elif (rt0.end_token.whitespaces_after_count < 3): 
                                nam = OrgItemNameToken.try_attach(rt0.end_token.next0, None, False, True)
                                if (nam is not None): 
                                    geo_ = self.__is_geo(nam.end_token.next0, False)
                                    if ((geo_) is not None): 
                                        if ((rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)._add_geo_object(geo_)): 
                                            rt0.end_token = self.__get_geo_end_token(geo_, nam.end_token.next0)
                                        (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None).add_name(nam.value, True, None)
                        if (len(rt0.referent.slots) > 2): 
                            if (tt.previous is not None and ((tt.previous.morph.class0.is_adjective and not tt.previous.morph.class0.is_verb)) and tt.whitespaces_before_count == 1): 
                                adj = Morphology.get_wordform(tt.previous.get_source_text().upper(), MorphBaseInfo._new2017(MorphClass.ADJECTIVE, gen, tt.previous.morph.language))
                                if (adj is not None and not adj.startswith("УПОЛНОМОЧ") and not adj.startswith("ОПЕРУПОЛНОМОЧ")): 
                                    ...
                                    if (rt0.begin_token.previous is not None and rt0.begin_token.previous.is_hiphen and rt0.begin_token.previous.previous is not None): 
                                        ...
                                        if (tt0.chars == rt0.begin_token.chars and isinstance(tt0, TextToken)): 
                                            adj = (tt0 if isinstance(tt0, TextToken) else None).term
                                            if (tt0.morph.class0.is_adjective and not tt0.morph.contains_attr("неизм.", MorphClass())): 
                                                adj = Morphology.get_wordform(adj, MorphBaseInfo._new2017(MorphClass.ADJECTIVE, gen, tt0.morph.language))
                    ...
                    if (nam is not None): 
                        ...
                        while ttt is not None: 
                            ...
                            if (not nam.is_std_tail): 
                                ...
                                if (ei is not None): 
                                    org_.add_type_str(ei.full_value)
                                    if (ei.short_value is not None): 
                                        org_.add_type_str(ei.short_value)
                            ttt = ttt.next0
                        org_.add_name(Utils.toStringStringIO(name_), True, None)
                else: 
                    br = BracketHelper.try_parse(t, BracketParseAttr.NO, 100)
                    if (br is not None): 
                        rt11 = self.__try_attach_org(t.next0, ad, OrganizationAnalyzer.AttachType.NORMAL, None, False, 0, -1)
                        if (rt11 is not None and ((rt11.end_token == br.end_token.previous or rt11.end_token == br.end_token))): 
                            rt11.begin_token = t
                            rt11.end_token = br.end_token
                            rt = rt11
                            org_ = (rt11.referent if isinstance(rt11.referent, OrganizationReferent) else None)
                        else: 
                            org_.add_name(MiscHelper.get_text_value_of_meta_token(br, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE), True, None)
                            org_.add_name(MiscHelper.get_text_value_of_meta_token(br, GetTextAttr.NO), True, br.begin_token.next0)
                            if (br.begin_token.next0 == br.end_token.previous and br.begin_token.next0.get_morph_class_in_dictionary().is_undefined): 
                                for wf in br.begin_token.next0.morph.items: 
                                    if (wf.case.is_genitive and isinstance(wf, MorphWordForm)): 
                                        org_.add_name((wf if isinstance(wf, MorphWordForm) else None).normal_case, True, None)
                ...
                if (br is not None): 
                    ...
                    if (t1 is not None and (t1.whitespaces_before_count < 3)): 
                        ...
                        if ((typ) is not None and typ.root is not None and typ.root.typ == OrgItemTermin.Types.PREFIX): 
                            ...
                            if (ok): 
                                org_ = OrganizationReferent()
                                rt = ReferentToken(org_, t, typ.end_token)
                                org_.add_type(typ, False)
                                nam = MiscHelper.get_text_value(br.begin_token.next0, br.end_token.previous, GetTextAttr.NO)
                                org_.add_name(nam, True, None)
                                rt11 = self.__try_attach_org(br.begin_token.next0, ad, OrganizationAnalyzer.AttachType.NORMAL, None, False, 0, -1)
                                if (rt11 is not None and rt11.end_char <= typ.end_char): 
                                    org_.merge_slots(rt11.referent, True)
            if (rt is None): 
                return None
        self.__do_post_analyze(rt, ad)
        if (step > 0): 
            mt = OrganizationAnalyzer._check_alias_after(rt, rt.end_token.next0)
            if (mt is not None): 
                if (ad is not None): 
                    term = Termin()
        ...
        if (tt1 is not None and tt1.is_comma_and): 
            if (BracketHelper.can_be_start_of_sequence(tt1.next0, True, False)): 
                if (BracketHelper.can_be_end_of_sequence(rt.end_token, True, None, False)): 
                    ...
                    while True:
                        ...
                        if (not ttt.is_comma_and): 
                            break
                        if (not BracketHelper.can_be_start_of_sequence(ttt.next0, True, False)): 
                            break
                        br = BracketHelper.try_parse(ttt.next0, BracketParseAttr.NO, 100)
                        if (br is None): 
                            break
                        add_typ = False
                        rt1 = self.__try_attach_org_(ttt.next0.next0, ttt.next0.next0, ad, None, True, OrganizationAnalyzer.AttachType.NORMAL, None, False, 0)
                        if (rt1 is None or (rt1.end_char < (br.end_char - 1))): 
                            add_typ = True
                            rt1 = self.__try_attach_org_(ttt.next0, ttt.next0, ad, None, True, OrganizationAnalyzer.AttachType.HIGH, None, False, 0)
                        if (rt1 is None or (rt1.end_char < (br.end_char - 1))): 
                            break
                        li.append(rt1)
                        org1 = (rt1.referent if isinstance(rt1.referent, OrganizationReferent) else None)
                        if (typ is not None): 
                            ok = True
                        if (len(org1.types) == 0): 
                            add_typ = True
                            ...
                            if (s is not None): 
                                ...
                                for n in org1.names: 
                                    if (s.startswith(n)): 
                                        ...
                                        break
                                if (not ex): 
                                    org1.add_name(s, True, br.begin_token.next0)
                        if (ttt.is_and): 
                            ok = True
                            break
                        ttt = rt1.end_token
    ...
    def __corr_brackets(rt : 'ReferentToken') -> bool:
        from pullenti.ner.core.BracketHelper import BracketHelper
        if (not BracketHelper.can_be_start_of_sequence(rt.begin_token.previous, True, False) or not BracketHelper.can_be_end_of_sequence(rt.end_token.next0, True, None, False)): 
            return False
    ...
    def __do_post_analyze(self, rt : 'ReferentToken', ad : 'OrgAnalyzerData') -> None:
        ...
        from pullenti.ner.geo.GeoReferent import GeoReferent
        from pullenti.ner.core.MiscHelper import MiscHelper
        if (rt.morph.case.is_undefined): 
            if (not rt.begin_token.chars.is_all_upper): 
                npt1 = NounPhraseHelper.try_parse(rt.begin_token, NounPhraseParseAttr.NO, 0)
                if (npt1 is None): 
                    npt1 = NounPhraseHelper.try_parse(rt.begin_token.next0, NounPhraseParseAttr.NO, 0)
                if (npt1 is not None): 
                    rt.morph = npt1.morph
        o = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
        if ((rt.kit.ontology is not None and o.ontology_items is None and o.higher is None) and o._m_temp_parent_org is None): 
            ...
            if (ot is not None and len(ot) == 1 and isinstance(ot[0].referent, OrganizationReferent)): 
                oo = (ot[0].referent if isinstance(ot[0].referent, OrganizationReferent) else None)
                ...
                for sl in o.slots: 
                    if (isinstance(sl.value, Referent)): 
                        ...
                        if (not ext): 
                            continue
                        rr = (sl.value if isinstance(sl.value, Referent) else None).clone()
                        rr.occurrence.clear()
                        o.upload_slot(sl, rr)
                        ...
                        for sss in rr.slots: 
                            if (isinstance(sss.value, Referent)): 
                                rrr = (sss.value if isinstance(sss.value, Referent) else None).clone()
                                ...
                                (sl.value if isinstance(sl.value, Referent) else None).add_ext_referent(rt_ex2)
        if (o.higher is None and o._m_temp_parent_org is None): 
            if (isinstance(rt.begin_token.previous, ReferentToken) and isinstance(rt.begin_token.previous.get_referent(), OrganizationReferent)): 
                oo = (rt.begin_token.previous.get_referent() if isinstance(rt.begin_token.previous.get_referent(), OrganizationReferent) else None)
                if (OrgOwnershipHelper.can_be_higher(oo, o, False)): 
                    o._m_temp_parent_org = oo
            if (o._m_temp_parent_org is None and isinstance(rt.end_token.next0, ReferentToken) and isinstance(rt.end_token.next0.get_referent(), OrganizationReferent)): 
                oo = (rt.end_token.next0.get_referent() if isinstance(rt.end_token.next0.get_referent(), OrganizationReferent) else None)
                if (OrgOwnershipHelper.can_be_higher(oo, o, False)): 
                    o._m_temp_parent_org = oo
            if (o._m_temp_parent_org is None): 
                rt1 = self.__try_attach_org(rt.end_token.next0, None, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, 0, -1)
                if (rt1 is not None and rt.end_token.next0 == rt1.begin_token): 
                    if (OrgOwnershipHelper.can_be_higher(rt1.referent if isinstance(rt1.referent, OrganizationReferent) else None, o, False)): 
                        o._m_temp_parent_org = (rt1.referent if isinstance(rt1.referent, OrganizationReferent) else None)
        if (rt.end_token.next0 is None): 
            return
        OrganizationAnalyzer.__corr_brackets(rt)
        if (rt.begin_token.previous is not None and rt.begin_token.previous.morph.class0.is_adjective and (rt.whitespaces_before_count < 2)): 
            if (len((rt.referent if isinstance(rt.referent, OrganizationReferent) else None)._geo_objects) == 0): 
                geo_ = self.__is_geo(rt.begin_token.previous, True)
                if (geo_ is not None): 
                    if ((rt.referent if isinstance(rt.referent, OrganizationReferent) else None)._add_geo_object(geo_)): 
                        rt.begin_token = rt.begin_token.previous
        ...
        while True:
            ...
            if (not (ttt is not None)): break
            if (ttt.is_char_of(",;") or ttt.morph.class0.is_preposition): 
                continue
            if (ttt.is_char(')')): 
                if (br): 
                    te = ttt
            ...
            if (rr is not None): 
                ...
                if (rr.type_name == "URI"): 
                    ...
                    refs.append(rr)
                    te = ttt
                    if (ttt.next0 is not None and ttt.next0.is_char('(')): 
                        brrr = BracketHelper.try_parse(ttt.next0, BracketParseAttr.NO, 100)
                        if (brrr is not None): 
                            ttt = brrr.end_token
                    continue
                elif (rr == rt.referent): 
            ...
            if (isinstance(ttt, TextToken)): 
                ...
                if (npt is not None): 
                    if ((npt.end_token.is_value("ДАТА", None) or npt.end_token.is_value("РЕГИСТРАЦИЯ", None) or npt.end_token.is_value("ЛИЦО", None)) or npt.end_token.is_value("ЮР", None) or npt.end_token.is_value("АДРЕС", None)): 
                        ...
                        keyword_ = True
                        continue
                if (ttt.is_value("REGISTRATION", None) and ttt.next0 is not None and ttt.next0.is_value("NUMBER", None)): 
                    ...
                    while True:
                        ...
                        if (isinstance(tt3, TextToken)): 
                            print((tt3 if isinstance(tt3, TextToken) else None).term, end="", file=tmp)
                    ...
                    continue
                if ((ttt.is_value("REGISTERED", None) and ttt.next0 is not None and ttt.next0.is_value("IN", None)) and isinstance(ttt.next0.next0, ReferentToken) and isinstance(ttt.next0.next0.get_referent(), GeoReferent)): 
                    rt.referent.add_slot(OrganizationReferent.ATTR_MISC, ttt.next0.next0.get_referent(), False, 0)
                    ttt = ttt.next0.next0
                    rt.end_token = ttt
                    continue
                if (br): 
                    ...
                    if (otyp is not None and (ttt.whitespaces_before_count < 2) and otyp.geo is None): 
                        ...
                        if (not OrgItemTypeToken.is_types_antagonisticoo(o, or1) and otyp.end_token.next0 is not None and otyp.end_token.next0.is_char(')')): 
                            o.add_type(otyp, False)
                            ttt = otyp.end_token
                            rt.end_token = ttt
                            if (br and ttt.next0 is not None and ttt.next0.is_char(')')): 
                                rt.end_token = ttt.next0
                                break
                            continue
            keyword_ = False
            errs -= 1
        ...
        if (te is not None and len(refs) > 0 and ((te.is_char(')') or has_inn or has_ok > 0))): 
            for rr in refs: 
                if (rr.type_name == OrganizationAnalyzer.__geoname): 
                    (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)._add_geo_object(rr)
                else: 
        ...
        if ((rt.whitespaces_before_count < 2) and isinstance(rt.begin_token.previous, TextToken) and rt.begin_token.previous.chars.is_all_upper): 
            term = (rt.begin_token.previous if isinstance(rt.begin_token.previous, TextToken) else None).term
            for s in o.slots: 
                if (isinstance(s.value, str)): 
                    a = MiscHelper.get_abbreviation(s.value if isinstance(s.value, str) else None)
    ...
    def __try_attach_org_by_alias(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        t0 = t
        br = False
        if (t0.next0 is not None and BracketHelper.can_be_start_of_sequence(t0, True, False)): 
            t = t0.next0
            br = True
        ...
        if (ad is not None): 
            ...
            if (tok is not None): 
                rt0 = ReferentToken(tok.termin.tag if isinstance(tok.termin.tag, Referent) else None, t0, tok.end_token)
                if (br): 
                    if (BracketHelper.can_be_end_of_sequence(tok.end_token.next0, True, None, False)): 
                        rt0.end_token = tok.end_token.next0
                    else: 
                        return None
                return rt0
        if (not br): 
            ...
            if (t.chars.is_latin_letter): 
                if (t.next0 is not None and t.next0.chars.is_latin_letter): 
                    return None
            elif (t.next0 is not None and ((t.next0.chars.is_cyrillic_letter or not t.next0.chars.is_all_lower))): 
                return None
        elif (not BracketHelper.can_be_end_of_sequence(t.next0, True, None, False)): 
            return None
        cou = 0
        ttt = t.previous
        first_pass2826 = True
        while True:
            ...
            org00 = (ttt.get_referent() if isinstance(ttt.get_referent(), OrganizationReferent) else None)
            if (org00 is None): 
                continue
            for n in org00.names: 
                ...
                if (ii > 0): 
                    str0 = n[0 : (ii)]
                if (t.is_value(str0, None)): 
                    if (ad is not None): 
                        ad.aliases.add(Termin._new118(str0, org00))
    ...
    def __attach_middle_attributes(self, org_ : 'OrganizationReferent', t : 'Token') -> 'Token':
        from pullenti.ner.org.internal.OrgItemNumberToken import OrgItemNumberToken
    ...
    def __is_geo(self, t : 'Token', can_be_adjective : bool=False) -> object:
        ...
        if (t is None): 
            return None
        if (t.is_value("В", None) and t.next0 is not None): 
            t = t.next0
        r = t.get_referent()
        if (r is not None): 
            if (r.type_name == OrganizationAnalyzer.__geoname): 
                if (t.whitespaces_before_count <= 15 or t.morph.case.is_genitive): 
                    return r
    ...
    def __get_geo_end_token(self, geo_ : object, t : 'Token') -> 'Token':
        ...
        if (isinstance(geo_, ReferentToken)): 
            if (isinstance((geo_ if isinstance(geo_, ReferentToken) else None).get_referent(), AddressReferent)): 
                return t.previous
            return (geo_ if isinstance(geo_, ReferentToken) else None).end_token
        elif (t is not None and t.next0 is not None and t.morph.class0.is_preposition): 
            return t.next0
        else: 
            return t
    
    def __attach_tail_attributes(self, org_ : 'OrganizationReferent', t : 'Token', ad : 'OrgAnalyzerData', attach_for_new_org : bool, attach_typ : 'AttachType', is_global : bool=False) -> 'Token':
        from pullenti.ner.Referent import Referent
        from pullenti.ner.org.OrganizationReferent import OrganizationReferent
        from pullenti.ner.ReferentToken import ReferentToken
        from pullenti.ner.geo.GeoReferent import GeoReferent
        ...
        can_has_geo = True
        if (not can_has_geo): 
            if (org_._types_contains("комитет") or org_._types_contains("академия") or org_._types_contains("инспекция")): 
                can_has_geo = True
        first_pass2828 = True
        while True:
            ...
            if (not (t is not None)): break
            if (((t.is_value("ПО", None) or t.is_value("В", None) or t.is_value("IN", None))) and t.next0 is not None): 
                if (attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                    break
                if (not can_has_geo): 
                    break
                r = self.__is_geo(t.next0, False)
                if (r is None): 
                    break
                if (not org_._add_geo_object(r)): 
                    break
                t1 = self.__get_geo_end_token(r, t.next0)
                t = t1
                continue
            if (t.is_value("ИЗ", None) and t.next0 is not None): 
                if (attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                    break
                if (not can_has_geo): 
                    break
                ...
                t = t1
                continue
            if (can_has_geo and org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is None and not t.is_newline_before): 
                ...
                if (t.is_char('(')): 
                    r = self.__is_geo(t.next0, False)
                    if (isinstance(r, ReferentToken) and (r if isinstance(r, ReferentToken) else None).end_token.next0 is not None and (r if isinstance(r, ReferentToken) else None).end_token.next0.is_char(')')): 
                        if (not org_._add_geo_object(r)): 
                            break
                        t1 = (r if isinstance(r, ReferentToken) else None).end_token.next0
                        t = t1
                        continue
                    if (isinstance(r, GeoReferent) and t.next0.next0 is not None and t.next0.next0.is_char(')')): 
                        if (not org_._add_geo_object(r)): 
                            break
                        t1 = t.next0.next0
                        t = t1
            ...
            if (isinstance(t.get_referent(), GeoReferent) and (t.whitespaces_before_count < 2)): 
                if (org_.find_slot(OrganizationReferent.ATTR_GEO, t.get_referent(), True) is not None): 
                    t1 = t
                    continue
            if (((t.is_value("ПРИ", None) or t.is_value("В", None))) and t.next0 is not None and isinstance(t.next0, ReferentToken)): 
                r = t.next0.get_referent()
                if (isinstance(r, OrganizationReferent)): 
                    if (t.is_value("В", None) and not OrgOwnershipHelper.can_be_higher(r if isinstance(r, OrganizationReferent) else None, org_, False)): 
                        pass
                    else: 
                        org_.higher = (r if isinstance(r, OrganizationReferent) else None)
                        t1 = t.next0
                        t = t1
                        continue
            if (t.chars.is_latin_letter and (t.whitespaces_before_count < 2)): 
                ...
                if (has_latin_name): 
                    ...
                    if (eng is not None): 
                        org_.add_type_str(eng.full_value)
                        if (eng.short_value is not None): 
                            org_.add_type_str(eng.short_value)
                        t1 = eng.end_token
                        t = t1
                        continue
            re = self.__is_geo(t, False)
            if (re is None and t.is_char(',')): 
                re = self.__is_geo(t.next0, False)
            if (re is not None): 
                if (attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                    if ((not can_has_geo and ki != OrganizationKind.BANK and ki != OrganizationKind.FEDERATION) and not "университет" in org_.types): 
                        break
                    if (not org_._add_geo_object(re)): 
                        break
                    if (t.is_char(',')): 
                        t = t.next0
            ...
            if (t.is_char('(')): 
                ...
                if (br is None): 
                    break
                if (t.next0 is not None and t.next0.get_referent() is not None): 
                    if (t.next0.next0 != br.end_token): 
                        break
                    r = t.next0.get_referent()
                    if (r.type_name == OrganizationAnalyzer.__geoname): 
                        if (not org_._add_geo_object(r)): 
                    ...
                    if (isinstance(r, OrganizationReferent) and not is_global): 
                        if (not attach_for_new_org and not org_.can_be_equals(r, Referent.EqualType.WITHINONETEXT)): 
                            break
                        org_.merge_slots(r, True)
                        t1 = br.end_token
                        t = t1
                        continue
                    break
                if (not is_global): 
                    if (attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                        ...
                        if (typ is not None and typ.end_token == br.end_token.previous and not typ.is_dep): 
                            ...
                            if (typ.name is not None): 
                                org_.add_type_str(typ.name.lower())
                            t1 = br.end_token
                            t = t1
                            continue
                    rte = OrgItemEngItem.try_attach_org(br.begin_token, False)
                    if (rte is not None): 
                        if (org_.can_be_equals(rte.referent, Referent.EqualType.FORMERGING)): 
                            org_.merge_slots(rte.referent, True)
                            t1 = rte.end_token
                            t = t1
                            continue
                        ...
                        for s in org_.slots: 
                            if (s.type_name == OrganizationReferent.ATTR_NAME): 
                                if (MiscHelper.can_be_equal_cyr_and_latss(nam, s.value)): 
                                    org_.add_name(nam, True, br.begin_token.next0)
                                    eq = True
                                    break
                        if (eq): 
                            t1 = br.end_token
                    ...
                    if (tt0 is not None): 
                        if (tt0.is_value("РАНЕЕ", None)): 
                            old_name = True
                            tt0 = tt0.next0
                        elif (tt0.morph.class0.is_adjective and tt0.next0 is not None and ((tt0.next0.is_value("НАЗВАНИЕ", None) or tt0.next0.is_value("НАИМЕНОВАНИЕ", None)))): 
                            old_name = True
                            tt0 = tt0.next0.next0
                    ...
                    if (rt is None): 
                        break
                    if (not org_.can_be_equals(rt.referent, Referent.EqualType.FORMERGING)): 
                        break
                    if (rt.end_token != br.end_token.previous): 
                        break
                    if (not attach_for_new_org and not org_.can_be_equals(rt.referent, Referent.EqualType.WITHINONETEXT)): 
                        break
                    if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL): 
                        if (not old_name and not OrganizationReferent.can_be_second_definition(org_, (rt.referent if isinstance(rt.referent, OrganizationReferent) else None))): 
                            break
                        typ = OrgItemTypeToken.try_attach(t.next0, True, None)
                        if (typ is not None and typ.is_douter_org): 
                            break
                    org_.merge_slots(rt.referent, True)
                    t1 = br.end_token
                    t = t1
                    continue
                break
                ...
                if (nam is not None): 
                    org_.add_name(nam, True, br.begin_token.next0)
                rt1 = self.__try_attach_org(t.next0, ad, OrganizationAnalyzer.AttachType.HIGH, None, True, 0, -1)
                if (rt1 is not None and rt1.end_token.next0 == br.end_token): 
                    org_.merge_slots(rt1.referent, True)
                    t1 = br.end_token
                    t = t1
            else: 
                break
    ...
    def __correct_owner_before(self, res : 'ReferentToken') -> None:
        ...
        if (res is None): 
            return
        if ((res.referent if isinstance(res.referent, OrganizationReferent) else None).kind == OrganizationKind.PRESS): 
            if (res.begin_token.is_value("КОРРЕСПОНДЕНТ", None) and res.begin_token != res.end_token): 
                res.begin_token = res.begin_token.next0
        org_ = (res.referent if isinstance(res.referent, OrganizationReferent) else None)
        if (org_.higher is not None): 
            return
        hi_before = None
        cou_before = 0
        ...
        while True:
            ...
            if (t.is_char(',')): 
                ...
                return
            if (t.is_referent): 
                hi_before = (t.get_referent() if isinstance(t.get_referent(), OrganizationReferent) else None)
                if ((hi_before) is not None): 
                    t0 = t
            break
        if (t0 is None): 
            return
        if (not OrgOwnershipHelper.can_be_higher(hi_before, org_, False)): 
            return
        if (OrgOwnershipHelper.can_be_higher(org_, hi_before, False)): 
            return
        hi_after = None
        cou_after = 0
        ...
        while True:
            ...
            if (t.is_char(',') or t.is_value("ПРИ", None)): 
                ...
                continue
            if (t.is_referent): 
                hi_after = (t.get_referent() if isinstance(t.get_referent(), OrganizationReferent) else None)
                break
            rt = self.__try_attach_org(t, None, OrganizationAnalyzer.AttachType.NORMAL, None, False, 0, -1)
            if (rt is not None): 
                hi_after = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
            break
        if (hi_after is not None): 
            if (OrgOwnershipHelper.can_be_higher(hi_after, org_, False)): 
                if (cou_before >= cou_after): 
                    return
        if (org_.kind == hi_before.kind and org_.kind != OrganizationKind.UNDEFINED): 
            if (org_.kind != OrganizationKind.DEPARTMENT & org_.kind != OrganizationKind.GOVENMENT): 
                return
        org_.higher = hi_before
        res.begin_token = t0
    ...
    def __check_ownership(self, t : 'Token') -> 'ReferentToken':
        ...
        while t is not None: 
            ...
            if (tt is not None and tt.morph.class0.is_preposition): 
                if (tt.is_value("ПРИ", None)): 
                    ...
                    break
                tt = tt.next0
            if ((tt is not None and tt.is_char('(') and isinstance(tt.next0, ReferentToken)) and tt.next0.next0 is not None and tt.next0.next0.is_char(')')): 
                br = True
                tt = tt.next0
            if (isinstance(tt, ReferentToken)): 
                org2 = (tt.get_referent() if isinstance(tt.get_referent(), OrganizationReferent) else None)
                if (org2 is not None): 
                    ok = OrgOwnershipHelper.can_be_higher(org2, org_, False)
                    if (always or ok): 
                        ok = True
                    elif (OrgOwnershipHelper.can_be_higher(org2, org_, True)): 
                        t0 = t.previous
                        ...
                        if (rt is not None and rt.referent.type_name == "PERSONPROPERTY" and rt.morph.number == MorphNumber.SINGULAR): 
                            ok = True
                    if (ok and ((org_.higher is None or org_.higher.can_be_equals(org2, Referent.EqualType.WITHINONETEXT)))): 
                        org_.higher = org2
                        if (br): 
                            tt = tt.next0
                        if (org_.higher == org2): 
                            if (res is None): 
                                res = ReferentToken(org_, t, tt)
                            else: 
                                res.end_token = tt
                            t = tt
                            if (len(org_._geo_objects) == 0): 
                                ttt = t.next0
                                if (ttt is not None and ttt.is_value("В", None)): 
                                    ttt = ttt.next0
                                if (self.__is_geo(ttt, False) is not None): 
                                    org_._add_geo_object(ttt)
                                    res.end_token = ttt
                                    t = ttt
                            org_ = org2
                            continue
                    if (org_.higher is not None and org_.higher.higher is None and OrgOwnershipHelper.can_be_higher(org2, org_.higher, False)): 
                        org_.higher.higher = org2
                        res = ReferentToken(org_, t, tt)
                        if (br): 
                            res.end_token = tt.next0
                        return res
                    if ((org_.higher is not None and org2.higher is None and OrgOwnershipHelper.can_be_higher(org_.higher, org2, False)) and OrgOwnershipHelper.can_be_higher(org2, org_, False)): 
                        org2.higher = org_.higher
                        org_.higher = org2
                        res = ReferentToken(org_, t, tt)
                        if (br): 
                            res.end_token = tt.next0
        ...
        if (res is not None): 
            return res
        if (org_.kind == OrganizationKind.DEPARTMENT and org_.higher is None and org_._m_temp_parent_org is None): 
            cou = 0
            tt = tt0.previous
            first_pass2831 = True
            while True:
                ...
                if ((cou) > 100): 
                    break
                org0 = (tt.get_referent() if isinstance(tt.get_referent(), OrganizationReferent) else None)
                if (org0 is None): 
                    continue
                tmp = list()
                while org0 is not None: 
                    if (OrgOwnershipHelper.can_be_higher(org0, org_, False)): 
                        org_.higher = org0
                        break
    ...
    def process_ontology_item(self, begin : 'Token') -> 'ReferentToken':
        ...
        if (rt is not None): 
            r = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
            if (r.higher is None and rt.end_token.next0 is not None): 
                h = (rt.end_token.next0.get_referent() if isinstance(rt.end_token.next0.get_referent(), OrganizationReferent) else None)
                if (h is not None): 
        ...
        if (Utils.isNullOrEmpty(name_)): 
            return None
        org_ = OrganizationReferent()
        org_.add_name(name_, True, begin)
        return ReferentToken(org_, begin, et)
    ...
    def initialize() -> None:
        ...
        if (OrganizationAnalyzer.__m_inited): 
            return
        OrganizationAnalyzer.__m_inited = True
        try: 
    ...
    def __try_attach_politic_party(self, t : 'Token', ad : 'OrgAnalyzerData', only_abbrs : bool=False) -> 'ReferentToken':
        ...
        while True:
            ...
            if (t.is_hiphen): 
                if (prev_toks is None): 
                    return None
                continue
            tokn = OrganizationAnalyzer.__m_politic_names.try_parse(t, TerminParseAttr.NO)
            if (tokn is not None): 
                if (not t.chars.is_all_lower): 
                    break
                t1 = tokn.end_token
            tok = OrganizationAnalyzer.__m_politic_prefs.try_parse(t, TerminParseAttr.NO)
            if (tok is None): 
            ...
            if (tok.termin.tag is None): 
                coef += 1
                prev_words += 1
            elif (tok.morph.class0.is_adjective): 
                coef += 0.5
            t = tok.end_token
            if (t.end_char > t1.end_char): 
                t1 = t
        ...
        if (t.is_value("ПАРТИЯ", None) or t.is_value("ФРОНТ", None) or t.is_value("ГРУППИРОВКА", None)): 
            ...
            root = t1
            is_fraction = True
            if (t.next0 is not None and isinstance(t.next0.get_referent(), OrganizationReferent)): 
                coef += 2
            else: 
                return None
        br = None
        name_tok = OrganizationAnalyzer.__m_politic_names.try_parse(t, TerminParseAttr.NO)
        if ((name_tok) is not None): 
            ...
            if (BracketHelper.can_be_start_of_sequence(t, True, False) and (br) is not None): 
                name_tok = OrganizationAnalyzer.__m_politic_names.try_parse(t.next0, TerminParseAttr.NO)
                if ((name_tok) is not None): 
                    coef += 1.5
                elif (only_abbrs): 
                    return None
                elif (t.next0 is not None and t.next0.is_value("О", None)): 
                    return None
                else: 
                    tt = t.next0
                    while tt is not None and tt.end_char <= br.end_char: 
                        tok2 = OrganizationAnalyzer.__m_politic_prefs.try_parse(tt, TerminParseAttr.NO)
                        if (tok2 is not None and tok2.termin.tag is None): 
                            if (tok2.termin.tag2 is None): 
                                is_politic = True
                            coef += 0.5
                            words_after += 1
                        elif (OrganizationAnalyzer.__m_politic_suffs.try_parse(tt, TerminParseAttr.NO) is not None): 
                            coef += 0.5
                            words_after += 1
                ...
                while True:
                    ...
                    if (tt.morph.class0.is_preposition): 
                        ...
                        continue
                    if (tt.is_and): 
                        npt2 = NounPhraseHelper.try_parse(tt.next0, NounPhraseParseAttr.REFERENTCANBENOUN, 0)
                        if (npt2 is not None and OrganizationAnalyzer.__m_politic_suffs.try_parse(npt2.end_token, TerminParseAttr.NO) is not None and npt2.end_token.chars == tt.previous.chars): 
                            continue
                        break
                    ...
                    while ttt is not None and ttt.end_char <= npt.end_char: 
                        tok2 = OrganizationAnalyzer.__m_politic_prefs.try_parse(ttt, TerminParseAttr.NO)
                        if (tok2 is not None and tok2.termin.tag is None): 
                            if (tok2.termin.tag2 is None): 
                                is_politic = True
                            co += 0.5
                            words_after += 1
                        elif (OrganizationAnalyzer.__m_politic_suffs.try_parse(ttt, TerminParseAttr.NO) is not None): 
                            co += 0.5
                            words_after += 1
                    ...
                    if (co == 0): 
                        if (not npt.morph.case.is_genitive): 
                            break
                        last_suf = OrganizationAnalyzer.__m_politic_suffs.try_parse(tt.previous, TerminParseAttr.NO)
        ...
        while True:
            ...
            if (not ((isinstance(tt, TextToken)))): 
                org1 = (tt.get_referent() if isinstance(tt.get_referent(), OrganizationReferent) else None)
            ...
            if (tt.morph.class0.is_preposition or tt.morph.class0.is_conjunction): 
                continue
            if (OrganizationAnalyzer.__m_politic_prefs.try_parse(tt, TerminParseAttr.NO) is not None): 
                coef += 0.5
        ...
        if (name_tok is not None): 
            is_politic = True
        if (is_fraction): 
            org_.add_profile(OrgProfile.POLICY)
            org_.add_profile(OrgProfile.UNIT)
        elif (is_politic): 
            org_.add_profile(OrgProfile.POLICY)
            org_.add_profile(OrgProfile.UNION)
        else: 
            org_.add_profile(OrgProfile.UNION)
        if (name_tok is not None): 
            is_politic = True
            org_.add_name(name_tok.termin.canonic_text, True, None)
            if (name_tok.termin.additional_vars is not None): 
                for v in name_tok.termin.additional_vars: 
                    org_.add_name(v.canonic_text, True, None)
            if (name_tok.termin.acronym is not None): 
                geo1 = (name_tok.termin.tag if isinstance(name_tok.termin.tag, GeoReferent) else None)
                if (geo1 is None): 
                    org_.add_name(name_tok.termin.acronym, True, None)
            ...
            if (typ is not None): 
                if (br is None): 
                    ...
                    if (t2.end_char > root.end_char): 
                        nam = "{0} {1}".format(typ, MiscHelper.get_text_value(root.next0, t2, GetTextAttr.NO))
                        org_.add_name(nam, True, None)
                if (len(org_.names) == 0 and typ1 != root): 
                    org_.add_name(typ, True, None)
                else: 
                    org_.add_type_str(typ.lower())
            if (is_fraction and isinstance(t1.next0, ReferentToken)): 
                org_.add_type_str("фракция")
                t1 = t1.next0
                org_.higher = (t1.get_referent() if isinstance(t1.get_referent(), OrganizationReferent) else None)
                if (t1.next0 is not None and t1.next0.is_value("В", None) and isinstance(t1.next0.next0, ReferentToken)): 
                    oo = (t1.next0.next0.get_referent() if isinstance(t1.next0.next0.get_referent(), OrganizationReferent) else None)
                    if (oo is not None and oo.kind == OrganizationKind.GOVENMENT): 
                        t1 = t1.next0.next0
                        org_.add_slot(OrganizationReferent.ATTR_MISC, oo, False, 0)
                    elif (isinstance(t1.next0.next0.get_referent(), GeoReferent)): 
                        t1 = t1.next0.next0
                        org_.add_slot(OrganizationReferent.ATTR_MISC, t1.get_referent(), False, 0)
        if (geo_ is not None): 
            org_._add_geo_object(geo_)
        elif (isinstance(t1.get_referent(), GeoReferent)): 
            org_._add_geo_object(t1.get_referent())
        return ReferentToken(org_, t0, t1)
    
    @staticmethod
    def __init_politic() -> None:
        from pullenti.ner.core.TerminCollection import TerminCollection
        from pullenti.ner.core.Termin import Termin
        OrganizationAnalyzer.__m_politic_prefs = TerminCollection()
        for s in ["либеральный", "либерал", "лейбористский", "демократический", "коммунистрический", "большевистский", "социальный", "социал", "национал", "националистическая", "свободный", "радикальный", "леворадикальный", "радикал", "революционная", "левый", "правый", "социалистический", "рабочий", "трудовой", "республиканский", "народный", "аграрный", "монархический", "анархический", "прогрессивый", "прогрессистский", "консервативный", "гражданский", "фашистский", "марксистский", "ленинский", "маоистский", "имперский", "славянский", "анархический", "баскский", "конституционный", "пиратский", "патриотический", "русский"]: 
            OrganizationAnalyzer.__m_politic_prefs.add(Termin(s.upper()))
        for s in ["объединенный", "всероссийский", "общероссийский", "христианский", "независимый", "альтернативный"]: 
            OrganizationAnalyzer.__m_politic_prefs.add(Termin._new2020(s.upper(), s))
        for s in ["политический", "правящий", "оппозиционный", "запрешенный", "террористический", "запрещенный", "экстремистский"]: 
            OrganizationAnalyzer.__m_politic_prefs.add(Termin._new118(s.upper(), s))
        for s in ["активист", "член", "руководство", "лидер", "глава", "демонстрация", "фракция", "съезд", "пленум", "террорист", "парламент", "депутат", "парламентарий", "оппозиция", "дума", "рада"]: 
            OrganizationAnalyzer.__m_politic_prefs.add(Termin._new120(s.upper(), s, s))
        OrganizationAnalyzer.__m_politic_suffs = TerminCollection()
        for s in ["коммунист", "социалист", "либерал", "республиканец", "националист", "радикал", "лейборист", "анархист", "патриот", "консерватор", "левый", "правый", "новый", "зеленые", "демократ", "фашист", "защитник", "труд", "равенство", "прогресс", "жизнь", "мир", "родина", "отечество", "отчизна", "республика", "революция", "революционер", "народовластие", "фронт", "сила", "платформа", "воля", "справедливость", "преображение", "преобразование", "солидарность", "управление", "демократия", "народ", "гражданин", "предприниматель", "предпринимательство", "бизнес", "пенсионер", "христианин"]: 
            OrganizationAnalyzer.__m_politic_suffs.add(Termin(s.upper()))
        for s in ["реформа", "свобода", "единство", "развитие", "освобождение", "любитель", "поддержка", "возрождение", "независимость"]: 
            OrganizationAnalyzer.__m_politic_suffs.add(Termin._new118(s.upper(), s))
        OrganizationAnalyzer.__m_politic_names = TerminCollection()
        for s in ["Республиканская партия", "Демократическая партия;Демпартия", "Христианско демократический союз;ХДС", "Свободная демократическая партия;СвДП", "ЯБЛОКО", "ПАРНАС", "ПАМЯТЬ", "Движение против нелегальной иммиграции;ДПНИ", "НАЦИОНАЛ БОЛЬШЕВИСТСКАЯ ПАРТИЯ;НБП", "НАЦИОНАЛЬНЫЙ ФРОНТ;НАЦФРОНТ", "Национальный патриотический фронт;НПФ", "Батькивщина;Батькiвщина", "НАРОДНАЯ САМООБОРОНА", "Гражданская платформа", "Народная воля", "Славянский союз", "ПРАВЫЙ СЕКТОР", "ПЕГИДА;PEGIDA", "Венгерский гражданский союз;ФИДЕС", "БЛОК ЮЛИИ ТИМОШЕНКО;БЮТ", "Аль Каида;Аль Каеда;Аль Кайда;Al Qaeda;Al Qaida", "Талибан;движение талибан", "Бригады мученников Аль Аксы", "Хезболла;Хезбалла;Хизбалла", "Народный фронт освобождения палестины;НФОП", "Организация освобождения палестины;ООП", "Союз исламского джихада;Исламский джихад", "Аль-Джихад;Египетский исламский джихад", "Братья-мусульмане;Аль Ихван альМуслимун", "ХАМАС", "Движение за освобождение Палестины;ФАТХ", "Фронт Аль Нусра;Аль Нусра", "Джабхат ан Нусра"]: 
            ...
            t = Termin._new118(pp[0], OrgProfile.POLICY)
            i = 0
            while i < len(pp): 
                ...
                i += 1
            OrganizationAnalyzer.__m_politic_names.add(t)
    
    __m_politic_prefs = None
    
    __m_politic_suffs = None
    
    __m_politic_names = None
    ...
    def __attach_global_org(self, t : 'Token', attach_typ : 'AttachType', ad : 'AnalyzerData', ext_geo : object=None) -> 'ReferentToken':
        ...
        if (isinstance(t, TextToken) and t.chars.is_latin_letter): 
            if (MiscHelper.is_eng_article(t)): 
                res11 = self.__attach_global_org(t.next0, attach_typ, ad, ext_geo)
                if (res11 is not None): 
                    res11.begin_token = t
                    return res11
        rt00 = self.__try_attach_politic_party(t, (ad if isinstance(ad, OrganizationAnalyzer.OrgAnalyzerData) else None), True)
        if (rt00 is not None): 
            return rt00
        if (not ((isinstance(t, TextToken)))): 
            if (t is not None and t.get_referent() is not None and t.get_referent().type_name == "URI"): 
                rt = self.__attach_global_org((t if isinstance(t, ReferentToken) else None).begin_token, attach_typ, ad, None)
                if (rt is not None and rt.end_char == t.end_char): 
                    rt.end_token = t
                    rt.begin_token = rt.end_token
                    return rt
            return None
        term = (t if isinstance(t, TextToken) else None).term
        if (t.chars.is_all_upper and term == "ВС"): 
            if (t.previous is not None): 
                if (t.previous.is_value("ПРЕЗИДИУМ", None) or t.previous.is_value("ПЛЕНУМ", None) or t.previous.is_value("СЕССИЯ", None)): 
                    ...
                    te = self.__attach_tail_attributes(org00, t.next0, None, False, OrganizationAnalyzer.AttachType.NORMAL, True)
                    return ReferentToken(org00, t, Utils.ifNotNull(te, t))
            if (t.next0 is not None and isinstance(t.next0.get_referent(), GeoReferent)): 
                is_vc = False
                if (t.previous is not None and isinstance(t.previous.get_referent(), OrganizationReferent) and (t.previous.get_referent() if isinstance(t.previous.get_referent(), OrganizationReferent) else None).kind == OrganizationKind.MILITARY): 
                    is_vc = True
                elif (ad is not None): 
                ...
                if (is_vc): 
                    ...
                    te = self.__attach_tail_attributes(org00, t.next0, None, False, OrganizationAnalyzer.AttachType.NORMAL, True)
                    return ReferentToken(org00, t, Utils.ifNotNull(te, t))
        if ((t.chars.is_all_upper and ((term == "АН" or term == "ВАС")) and t.next0 is not None) and isinstance(t.next0.get_referent(), GeoReferent)): 
            org00 = OrganizationReferent()
            if (term == "АН"): 
        ...
        if (t.chars.is_all_upper and term == "МЮ"): 
            ...
            if ((t.previous is not None and t.previous.is_value("В", None) and t.previous.previous is not None) and t.previous.previous.is_value("ЗАРЕГИСТРИРОВАТЬ", None)): 
                ok = True
            elif (t.next0 is not None and isinstance(t.next0.get_referent(), GeoReferent)): 
                ok = True
            if (ok): 
                org00 = OrganizationReferent()
        ...
        if (t.chars.is_all_upper and term == "ФС"): 
            if (t.next0 is not None and isinstance(t.next0.get_referent(), GeoReferent)): 
                ...
                org00._add_geo_object(t.next0.get_referent())
                return ReferentToken(org00, t, t.next0)
        if (t.chars.is_all_upper and term == "МП"): 
            ...
            prev = False
            if (tt0 is not None): 
                org0 = (tt0.get_referent() if isinstance(tt0.get_referent(), OrganizationReferent) else None)
                if (org0 is not None): 
                    prev = True
            if (t.next0 is not None and org0 is None): 
                org0 = (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None)
            if (org0 is not None and org0.kind == OrganizationKind.CHURCH): 
                ...
                if (not prev): 
                    ...
                    res.begin_token = tt0
                    if (tt0 != t.previous and res.end_token.next0 is not None and res.end_token.next0.is_char(')')): 
                        res.end_token = res.end_token.next0
                return res
        if (t.chars.is_all_upper and term == "ГШ"): 
            if (t.next0 is not None and isinstance(t.next0.get_referent(), OrganizationReferent) and (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None).kind == OrganizationKind.MILITARY): 
                org00 = OrganizationReferent()
                org00.add_type_str("генеральный штаб")
                org00.add_profile(OrgProfile.ARMY)
                org00.higher = (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None)
                return ReferentToken(org00, t, t.next0)
        ...
        if (t.chars.is_all_upper and term == "СФ"): 
            ...
            if (t.next0 is not None): 
                if (t.next0.is_value("ФС", None) or ((isinstance(t.next0.get_referent(), OrganizationReferent) and t.next0.get_referent().find_slot(OrganizationReferent.ATTR_NAME, "ФЕДЕРАЛЬНОЕ СОБРАНИЕ", True) is not None))): 
                    org00 = OrganizationReferent()
                    org00.add_type_str("совет")
                    org00.add_profile(OrgProfile.STATE)
                    org00.add_name("СОВЕТ ФЕДЕРАЦИИ", True, None)
        ...
        if (t.chars.is_all_upper and ((term == "СК" or term == "CK"))): 
            if (t.next0 is not None and isinstance(t.next0.get_referent(), GeoReferent)): 
                ...
                org00._add_geo_object(t.next0.get_referent())
                return ReferentToken(org00, t, t.next0)
            gt1 = OrgGlobal.GLOBAL_ORGS.try_attach(t.next0, None, False)
            if (gt1 is None and t.next0 is not None and t.kit.base_language.is_ua): 
                gt1 = OrgGlobal.GLOBAL_ORGS_UA.try_attach(t.next0, None, False)
            ok = False
        ...
        if (gt is None): 
            return None
        for ot in gt: 
            org0 = (ot.item.referent if isinstance(ot.item.referent, OrganizationReferent) else None)
            if (org0 is None): 
                continue
            if (ot.begin_token == ot.end_token): 
                if (len(gt) == 1): 
                    if (isinstance(ot.begin_token, TextToken) and (ot.begin_token if isinstance(ot.begin_token, TextToken) else None).term == "МГТУ"): 
            ...
            if ((ot.begin_token == ot.end_token and (t.length_char < 6) and not t.chars.is_all_upper) and not t.chars.is_last_lower): 
                if (org0.find_slot(OrganizationReferent.ATTR_NAME, (t if isinstance(t, TextToken) else None).term, True) is None): 
                    ...
                    has_vow = False
                    for ch in (t if isinstance(t, TextToken) else None).term: 
            ...
            if (ot.begin_token == ot.end_token): 
                if (t.previous is None or t.is_whitespace_before): 
                    ...
                    continue
                if (t.next0 is None or t.is_whitespace_after): 
                    pass
                elif (isinstance(t.next0, TextToken) and ((t.next0.is_char_of(",.") or BracketHelper.can_be_end_of_sequence(t.next0, False, None, False)))): 
                    pass
                else: 
                    continue
            ...
            org_ = None
            if (isinstance(t, TextToken)): 
                if (t.is_value("ДЕПАРТАМЕНТ", None) or t.is_value("КОМИТЕТ", None)): 
                    nnn = OrgItemNameToken.try_attach(t.next0, None, True, True)
                    if (nnn is not None and nnn.end_char > ot.end_char): 
                        org_ = OrganizationReferent()
                        org_.add_type_str((t if isinstance(t, TextToken) else None).lemma.lower())
                        org_.add_name(MiscHelper.get_text_value(t, nnn.end_token, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVESINGLE), True, None)
                        ot.end_token = nnn.end_token
            if (org_ is None): 
                org_ = (org0.clone() if isinstance(org0.clone(), OrganizationReferent) else None)
                if (len(org_._geo_objects) > 0): 
                    for s in org_.slots: 
                        if (s.type_name == OrganizationReferent.ATTR_GEO and isinstance(s.value, GeoReferent)): 
                            gg = (s.value if isinstance(s.value, GeoReferent) else None).clone()
                            gg.occurrence.clear()
                            ...
                            org_._add_geo_object(rtg)
                            break
                org_.add_name(ot.termin.canonic_text, True, None)
            if (ext_geo is not None): 
                org_._add_geo_object(ext_geo)
            org_._is_from_global_ontos = True
            tt = ot.begin_token
            while tt is not None and (tt.end_char < ot.end_char): 
                if (isinstance(tt.get_referent(), GeoReferent)): 
                    org_._add_geo_object(tt)
                    break
                tt = tt.next0
            if (isinstance(t.previous, TextToken) and (t.whitespaces_before_count < 2) and t.previous.morph.class0.is_adjective): 
                gg = t.kit.process_referent("GEO", t.previous)
                if (gg is not None and gg.morph.class0.is_adjective): 
                    t = t.previous
                    org_._add_geo_object(gg)
            t1 = None
            if (not "академия" in org0.types and attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                t1 = self.__attach_tail_attributes(org_, ot.end_token.next0, None, False, OrganizationAnalyzer.AttachType.NORMAL, True)
            elif ((((("министерство" in org0.types or "парламент" in org0.types or "совет" in org0.types) or org0.kind == OrganizationKind.SCIENCE or org0.kind == OrganizationKind.STUDY) or org0.kind == OrganizationKind.JUSTICE or org0.kind == OrganizationKind.MILITARY)) and isinstance(ot.end_token.next0, ReferentToken)): 
                geo_ = (ot.end_token.next0.get_referent() if isinstance(ot.end_token.next0.get_referent(), GeoReferent) else None)
                if (geo_ is not None and geo_.is_state): 
                    org_._add_geo_object(geo_)
                    t1 = ot.end_token.next0
            if (t1 is None): 
                t1 = ot.end_token
            epp = OrgItemEponymToken.try_attach(t1.next0, False)
            if (epp is not None): 
                exi = False
                for v in epp.eponyms: 
                    if (org_.find_slot(OrganizationReferent.ATTR_EPONYM, v, True) is not None): 
                        exi = True
                        break
                if (not exi): 
                    for i in range(len(org_.slots) - 1, -1, -1):
                        if (org_.slots[i].type_name == OrganizationReferent.ATTR_EPONYM): 
                            del org_.slots[i]
                    for vv in epp.eponyms: 
                        org_.add_eponym(vv)
                t1 = epp.end_token
            if (t1.whitespaces_after_count < 2): 
                typ = OrgItemTypeToken.try_attach(t1.next0, False, None)
                if (typ is not None): 
                    if (OrgItemTypeToken.is_type_accords(org_, typ)): 
                        if (typ.chars.is_latin_letter and typ.root is not None and typ.root.can_be_normal_dep): 
                            pass
                        else: 
                            org_.add_type(typ, False)
                            t1 = typ.end_token
            if (len(org_._geo_objects) == 0 and t.previous is not None and t.previous.morph.class0.is_adjective): 
                grt = t.kit.process_referent("GEO", t.previous)
                if (grt is not None and grt.end_token.next0 == t): 
                    org_._add_geo_object(grt)
                    t = t.previous
            if (org_.find_slot(OrganizationReferent.ATTR_NAME, "ВТБ", True) is not None and t1.next0 is not None): 
                tt = t1.next0
                if (tt.is_hiphen and tt.next0 is not None): 
                    tt = tt.next0
                if (isinstance(tt, NumberToken)): 
                    org_.number = str((tt if isinstance(tt, NumberToken) else None).value)
                    t1 = tt
            if (not t.is_whitespace_before and not t1.is_whitespace_after): 
                if (BracketHelper.can_be_start_of_sequence(t.previous, True, False) and BracketHelper.can_be_end_of_sequence(t1.next0, True, None, False)): 
                    t = t.previous
                    t1 = t1.next0
            return ReferentToken(org_, t, t1)
        return None
    ...
    def __try_attach_org_med_typ(t : 'Token') -> 'MetaToken':
        ...
        if (((t is not None and s == "Г" and t.next0 is not None) and t.next0.is_char_of("\\/.") and t.next0.next0 is not None) and t.next0.next0.is_value("Б", None)): 
            t1 = t.next0.next0
            if (t.next0.is_char('.') and t1.next0 is not None and t1.next0.is_char('.')): 
                t1 = t1.next0
            return MetaToken._new2026(t, t1, "городская больница", MorphCollection._new2025(MorphGender.FEMINIE))
        if ((s == "ИН" and t.next0 is not None and t.next0.is_hiphen) and t.next0.next0 is not None and t.next0.next0.is_value("Т", None)): 
            return MetaToken._new2026(t, t.next0.next0, "институт", MorphCollection._new2025(MorphGender.MASCULINE))
        if ((s == "Б" and t.next0 is not None and t.next0.is_hiphen) and isinstance(t.next0.next0, TextToken) and ((t.next0.next0.is_value("ЦА", None) or t.next0.next0.is_value("ЦУ", None)))): 
            return MetaToken._new2026(t, t.next0.next0, "больница", MorphCollection._new2025(MorphGender.FEMINIE))
        if (s == "ГКБ"): 
            return MetaToken._new2026(t, t, "городская клиническая больница", MorphCollection._new2025(MorphGender.FEMINIE))
        if (t.is_value("ПОЛИКЛИНИКА", None)): 
            return MetaToken._new2026(t, t, "поликлиника", MorphCollection._new2025(MorphGender.FEMINIE))
        if (t.is_value("БОЛЬНИЦА", None)): 
            return MetaToken._new2026(t, t, "больница", MorphCollection._new2025(MorphGender.FEMINIE))
        if (t.is_value("ДЕТСКИЙ", None)): 
            mt = OrganizationAnalyzer.__try_attach_org_med_typ(t.next0)
            if (mt is not None): 
                mt.begin_token = t
    ...
    def __try_attach_org_med(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        if (t.previous is None or t.previous.previous is None): 
            return None
        if ((t.previous.morph.class0.is_preposition and t.previous.previous.is_value("ДОСТАВИТЬ", None)) or t.previous.previous.is_value("ПОСТУПИТЬ", None)): 
            pass
        else: 
            return None
        if (t.is_value("ТРАВМПУНКТ", None)): 
            ...
            if ((t.next0 is not None and t.next0.is_char('.') and t.next0.next0 is not None) and t.next0.next0.is_value("ПУНКТ", None)): 
                t = t.next0.next0.next0
        if (isinstance(t, NumberToken)): 
            tt = OrganizationAnalyzer.__try_attach_org_med_typ(t.next0)
            if (tt is not None): 
                org1 = OrganizationReferent()
                org1.add_type_str((tt.tag if isinstance(tt.tag, str) else None).lower())
                org1.number = str((t if isinstance(t, NumberToken) else None).value)
                return ReferentToken(org1, t, tt.end_token)
        typ = OrganizationAnalyzer.__try_attach_org_med_typ(t)
        adj = None
        if (typ is None and t.chars.is_capital_upper and t.morph.class0.is_adjective): 
            typ = OrganizationAnalyzer.__try_attach_org_med_typ(t.next0)
            if (typ is not None): 
                adj = t.get_normal_case_text(MorphClass.ADJECTIVE, True, typ.morph.gender, False)
        if (typ is None): 
            return None
        org_ = OrganizationReferent()
        s = (typ.tag if isinstance(typ.tag, str) else None)
        org_.add_type_str(s.lower())
        if (adj is not None): 
            org_.add_name("{0} {1}".format(adj, s.upper()), True, None)
        t1 = typ.end_token
        epo = OrgItemEponymToken.try_attach(t1.next0, False)
        if (epo is not None): 
            for v in epo.eponyms: 
                org_.add_eponym(v)
            t1 = epo.end_token
        if (isinstance(t1.next0, TextToken)): 
            if (t1.next0.is_value("СКЛИФОСОФСКОГО", None) or t1.next0.is_value("СЕРБСКОГО", None) or t1.next0.is_value("БОТКИНА", None)): 
                org_.add_eponym((t1.next0 if isinstance(t1.next0, TextToken) else None).term)
                t1 = t1.next0
        num = OrgItemNumberToken.try_attach(t1.next0, False, None)
        if (num is not None): 
            org_.number = num.number
            t1 = num.end_token
        if (len(org_.slots) > 1): 
            return ReferentToken(org_, t, t1)
        return None
    ...
    def __try_attach_prop_names(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        if (isinstance(t0, TextToken) and (t0.whitespaces_after_count < 2) and t0.morph.class0.is_adjective): 
            rt0 = t0.kit.process_referent("GEO", t0)
            if (rt0 is not None and rt0.morph.class0.is_adjective): 
                rt.begin_token = rt0.begin_token
                (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)._add_geo_object(rt0)
        if (rt.end_token.whitespaces_after_count < 2): 
            tt1 = self.__attach_tail_attributes(rt.referent if isinstance(rt.referent, OrganizationReferent) else None, rt.end_token.next0, ad, True, OrganizationAnalyzer.AttachType.NORMAL, True)
            if (tt1 is not None): 
                rt.end_token = tt1
        return rt
    ...
    def __try_attach_org_names(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        if (not ((isinstance(t, TextToken))) or not t.chars.is_letter): 
            if (BracketHelper.can_be_start_of_sequence(t, True, False)): 
                ...
                if ((t.next0 is not None and (t.next0.whitespaces_after_count < 3) and t.next0.chars.is_latin_letter) and ((t.next0.is_value("POST", None) or t.next0.is_value("TODAY", None)))): 
                    tname1 = t.next0
                ...
                return None
        elif (t.chars.is_all_upper and (t if isinstance(t, TextToken) else None).term == "ИА"): 
            prof = OrgProfile.MEDIA
            t = t.next0
            ...
            if (re is not None): 
                re.begin_token = t0
                (re.referent if isinstance(re.referent, OrganizationReferent) else None).add_type_str(typ)
                return re
            if (t.chars.is_latin_letter): 
        ...
        elif (((t.chars.is_latin_letter and t.next0 is not None and t.next0.is_char('.')) and not t.next0.is_whitespace_after and t.next0.next0 is not None) and t.next0.next0.chars.is_latin_letter): 
            tname1 = t.next0.next0
            prof = OrgProfile.MEDIA
            if (tname1.next0 is None): 
                pass
            elif (tname1.whitespaces_after_count > 0): 
                pass
            elif (tname1.next0.is_char(',')): 
                pass
            elif (tname1.length_char > 1 and tname1.next0.is_char_of(".") and tname1.next0.is_whitespace_after): 
                pass
            ...
            return None
        t00 = t0.previous
        if (t00 is not None and t00.morph.class0.is_adjective): 
            t00 = t00.previous
        if (t00 is not None and t00.morph.class0.is_preposition): 
            t00 = t00.previous
        tok = OrganizationAnalyzer.__m_prop_names.try_parse(t, TerminParseAttr.NO)
        if (tok is None and t.chars.is_latin_letter and t.is_value("THE", None)): 
            tok = OrganizationAnalyzer.__m_prop_names.try_parse(t.next0, TerminParseAttr.NO)
        if (tok is not None): 
        ...
        if (br is not None): 
            ...
            while tt is not None and tt.end_char <= br.end_char: 
                ...
                if (tt.is_char_of("?:")): 
                    return None
                if (tt == br.begin_token.next0 or tt == br.end_token.previous): 
            ...
            if (not ok and OrganizationAnalyzer.__is_std_press_end(t1)): 
                if (br.begin_token.next0.chars.is_capital_upper and (br.length_char < 15)): 
                    ok = True
                    prof = OrgProfile.MEDIA
                    prof2 = OrgProfile.PRESS
        ...
        if ((tok is None and not ok and tname1 is None) and prof == OrgProfile.UNDEFINED): 
            if (br is None or not t.chars.is_capital_upper): 
                return None
            tok1 = OrganizationAnalyzer.__m_prop_pref.try_parse(t00, TerminParseAttr.NO)
            if (tok1 is not None): 
        ...
        if (not ok): 
            ...
            while True:
                ...
                if (MiscHelper.can_be_start_of_sentence(tt.next0)): 
                    break
                tok1 = OrganizationAnalyzer.__m_prop_pref.try_parse(tt, TerminParseAttr.NO)
                if (tok1 is not None): 
                    pr = Utils.valToEnum(tok1.termin.tag, OrgProfile)
                    if (prof != OrgProfile.UNDEFINED and prof != pr): 
                        continue
        ...
        if (prof == OrgProfile.UNDEFINED): 
            return None
        org_ = OrganizationReferent()
        org_.add_profile(prof)
        if (prof2 != OrgProfile.UNDEFINED): 
            org_.add_profile(prof2)
        if (prof == OrgProfile.SPORT): 
            org_.add_type_str("спортивный клуб")
        if (typ is not None): 
            org_.add_type_str(typ)
        if (br is not None and ((tok is None or tok.end_token != br.end_token.previous))): 
            if (tok is not None): 
                nam = MiscHelper.get_text_value(tok.end_token.next0, br.end_token, GetTextAttr.NO)
                if (nam is not None): 
            ...
            org_.add_name(tok.termin.canonic_text, True, None)
            if (tok.termin.acronym is not None): 
                org_.add_name(tok.termin.acronym, True, None)
        ...
        if (((((prof & OrgProfile.MEDIA)) != OrgProfile.UNDEFINED)) and t0.previous is not None): 
            if ((t0.previous.is_value("ЖУРНАЛ", None) or t0.previous.is_value("ИЗДАНИЕ", None) or t0.previous.is_value("ИЗДАТЕЛЬСТВО", None)) or t0.previous.is_value("АГЕНТСТВО", None)): 
                t0 = t0.previous
                org_.add_type_str(t0.get_normal_case_text(MorphClass.NOUN, True, MorphGender.UNDEFINED, False).lower())
                if (not t0.previous.is_value("АГЕНТСТВО", None)): 
                    org_.add_profile(OrgProfile.PRESS)
        res = ReferentToken(org_, t0, t)
        if (br is not None): 
    ...
    def __is_std_press_end(t : 'Token') -> bool:
        ...
        if ((((((((str0 == "NEWS" or str0 == "PRESS" or str0 == "PRESSE") or str0 == "ПРЕСС" or str0 == "НЬЮС") or str0 == "TIMES" or str0 == "TIME") or str0 == "ТАЙМС" or str0 == "POST") or str0 == "ПОСТ" or str0 == "TODAY") or str0 == "ТУДЕЙ" or str0 == "DAILY") or str0 == "ДЕЙЛИ" or str0 == "ИНФОРМ") or str0 == "INFORM"): 
            return True
        return False
    ...
    def __try_attach_org_sport_associations(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        if (isinstance(t.get_referent(), GeoReferent)): 
            ...
            if (rt.end_token.is_value("ФЕДЕРАЦИЯ", None) or rt.begin_token.is_value("ФЕДЕРАЦИЯ", None)): 
                typ = "федерация"
                geo_ = (t.get_referent() if isinstance(t.get_referent(), GeoReferent) else None)
            t1 = t
            if (t.previous is not None and t.previous.morph.class0.is_adjective): 
                if (OrganizationAnalyzer.__m_sports.try_parse(t.previous, TerminParseAttr.NO) is not None): 
                    cou += 1
                    t = t.previous
        else: 
            ...
            if (((npt.noun.is_value("АССОЦИАЦИЯ", None) or npt.noun.is_value("ФЕДЕРАЦИЯ", None) or npt.noun.is_value("СОЮЗ", None)) or npt.noun.is_value("СБОРНАЯ", None) or npt.noun.is_value("КОМАНДА", None)) or npt.noun.is_value("КЛУБ", None)): 
                typ = npt.noun.get_normal_case_text(MorphClass.NOUN, True, MorphGender.UNDEFINED, False).lower()
            elif (isinstance(t, TextToken) and t.chars.is_all_upper and (t if isinstance(t, TextToken) else None).term == "ФК"): 
                typ = "команда"
            else: 
                return None
            if (typ == "команда"): 
            ...
            for a in npt.adjectives: 
                tok = OrganizationAnalyzer.__m_sports.try_parse(a.begin_token, TerminParseAttr.NO)
                if (tok is not None): 
                    cou += 1
        ...
        while True:
            ...
            if (tt.is_comma_and): 
                continue
            if (tt.morph.class0.is_preposition and not tt.morph.class0.is_adverb and not tt.morph.class0.is_verb): 
                continue
            if (isinstance(tt.get_referent(), GeoReferent)): 
                t1 = tt
                geo_ = (tt.get_referent() if isinstance(tt.get_referent(), GeoReferent) else None)
                if (typ == "сборная"): 
                    cou += 1
                continue
            if (tt.is_value("СТРАНА", None) and isinstance(tt, TextToken)): 
                t11 = tt
                t1 = t11
                del_word = (tt if isinstance(tt, TextToken) else None).term
                continue
            tok = OrganizationAnalyzer.__m_sports.try_parse(tt, TerminParseAttr.NO)
            if (tok is not None): 
                cou += 1
                tt = tok.end_token
                t11 = tt
            ...
            if (tt.chars.is_all_lower or tt.get_morph_class_in_dictionary().is_verb): 
                ...
                tok = OrganizationAnalyzer.__m_prop_names.try_parse(tt, TerminParseAttr.NO)
            if (tok is not None): 
            ...
            if (BracketHelper.can_be_start_of_sequence(tt, True, False)): 
                ...
                if (br is None): 
                    break
                tok = OrganizationAnalyzer.__m_prop_names.try_parse(tt.next0, TerminParseAttr.NO)
                if (tok is not None or cou > 0): 
                    propname = MiscHelper.get_text_value(tt.next0, br.end_token, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
                    cou += 1
                    t1 = br.end_token
                    tt = t1
                    continue
            ...
            if (npt1 is None): 
                break
            tok = OrganizationAnalyzer.__m_sports.try_parse(npt1.noun.begin_token, TerminParseAttr.NO)
            if (tok is None): 
                break
            cou += 1
            tt = tok.end_token
        ...
        if (propname is not None): 
            org_.add_name(propname, True, None)
            if (name_ is not None): 
                org_.add_type_str(name_.lower())
        elif (name_ is not None): 
            org_.add_name(name_, True, None)
        if (geo_ is not None): 
            org_._add_geo_object(geo_)
        org_.add_profile(OrgProfile.SPORT)
        return ReferentToken(org_, t, t1)
    ...
    def __init_sport() -> None:
        ...
        OrganizationAnalyzer.__m_sports = TerminCollection()
        for s in ["акробатика;акробатический;акробат", "бадминтон;бадминтонный;бадминтонист", "баскетбол;баскетбольный;баскетболист", "бейсбол;бейсбольный;бейсболист", "биатлон;биатлонный;биатлонист", "бильярд;бильярдный;бильярдист", "бобслей;бобслейный;бобслеист", "боулинг", "боевое искуство", "бокс;боксерский;боксер", "борьба;борец", "водное поло", "волейбол;волейбольный;волейболист", "гандбол;гандбольный;гандболист", "гольф;гольфный;гольфист", "горнолыжный спорт", "слалом;;слаломист", "сквош", "гребля", "дзюдо;дзюдоистский;дзюдоист", "карате;;каратист", "керлинг;;керлингист", "коньки;конькобежный;конькобежец", "легкая атлетика;легкоатлетический;легкоатлет", "лыжных гонок", "мотоцикл;мотоциклетный;мотоциклист", "тяжелая атлетика;тяжелоатлетический;тяжелоатлет", "ориентирование", "плавание;;пловец", "прыжки", "регби;;регбист", "пятиборье", "гимнастика;гимнастический;гимнаст", "самбо;;самбист", "сумо;;сумист", "сноуборд;сноубордический;сноубордист", "софтбол;софтбольный;софтболист", "стрельба;стрелковый", "спорт;спортивный", "теннис;теннисный;теннисист", "триатлон", "тхэквондо", "ушу;;ушуист", "фехтование;фехтовальный;фехтовальщик", "фигурное катание;;фигурист", "фристайл;фристальный", "футбол;футбольный;футболист", "мини-футбол", "хоккей;хоккейный;хоккеист", "хоккей на траве", "шахматы;шахматный;шахматист", "шашки;шашечный"]: 
            ...
            OrganizationAnalyzer.__m_sports.add(t)
        for s in ["байдарка", "каноэ", "лук", "трава", "коньки", "трамплин", "двоеборье", "батут", "вода", "шпага", "сабля", "лыжи", "скелетон"]: 
            OrganizationAnalyzer.__m_sports.add(Termin._new2020(s.upper(), s))
        OrganizationAnalyzer.__m_prop_names = TerminCollection()
        for s in ["СПАРТАК", "ЦСКА", "ЗЕНИТ!", "ТЕРЕК", "КРЫЛЬЯ СОВЕТОВ", "ДИНАМО", "АНЖИ", "КУБАНЬ", "АЛАНИЯ", "ТОРПЕДО", "АРСЕНАЛ!", "ЛОКОМОТИВ", "МЕТАЛЛУРГ!", "РОТОР", "СКА", "СОКОЛ!", "ХИМИК!", "ШИННИК", "РУБИН", "ШАХТЕР", "САЛАВАТ ЮЛАЕВ", "ТРАКТОР!", "АВАНГАРД!", "АВТОМОБИЛИСТ!", "АТЛАНТ!", "ВИТЯЗЬ!", "НАЦИОНАЛЬНАЯ ХОККЕЙНАЯ ЛИГА;НХЛ", "КОНТИНЕНТАЛЬНАЯ ХОККЕЙНАЯ ЛИГА;КХЛ", "СОЮЗ ЕВРОПЕЙСКИХ ФУТБОЛЬНЫХ АССОЦИАЦИЙ;УЕФА;UEFA", "Женская теннисная ассоциация;WTA", "Международная федерация бокса;IBF", "Всемирная боксерская организация;WBO", "РЕАЛ", "МАНЧЕСТЕР ЮНАЙТЕД", "манчестер сити", "БАРСЕЛОНА!", "БАВАРИЯ!", "ЧЕЛСИ", "ЛИВЕРПУЛЬ!", "ЮВЕНТУС", "НАПОЛИ", "БОЛОНЬЯ", "ФУЛХЭМ", "ЭВЕРТОН", "ФИЛАДЕЛЬФИЯ", "ПИТТСБУРГ", "ИНТЕР!", "Аякс", "ФЕРРАРИ;FERRARI", "РЕД БУЛЛ;RED BULL", "МАКЛАРЕН;MCLAREN", "МАКЛАРЕН-МЕРСЕДЕС;MCLAREN-MERCEDES"]: 
            ...
            t = Termin._new118(pp[0], OrgProfile.SPORT)
            if (not is_bad): 
                t.tag2 = ss
            if (len(pp) > 1): 
                if (len(pp[1]) < 4): 
            ...
            OrganizationAnalyzer.__m_prop_names.add(t)
        for s in ["ИТАР ТАСС;ТАСС;Телеграфное агентство советского союза", "Интерфакс;Interfax", "REGNUM", "ЛЕНТА.РУ;Lenta.ru", "Частный корреспондент;ЧасКор", "РИА Новости;Новости!;АПН", "Росбалт;RosBalt", "УНИАН", "ИНФОРОС;inforos", "Эхо Москвы", "Сноб!", "Серебряный дождь", "Вечерняя Москва;Вечерка", "Московский Комсомолец;Комсомолка", "Коммерсантъ;Коммерсант", "Афиша", "Аргументы и факты;АИФ", "Викиновости", "РосБизнесКонсалтинг;РБК", "Газета.ру", "Русский Репортер!", "Ведомости", "Вести!", "Рамблер Новости", "Живой Журнал;ЖЖ;livejournal;livejournal.ru", "Новый Мир", "Новая газета", "Правда!", "Известия!", "Бизнес!", "Русская жизнь!", "НТВ Плюс", "НТВ", "ВГТРК", "ТНТ", "Муз ТВ;МузТВ", "АСТ", "Эксмо", "Астрель", "Терра!", "Финанс!", "Собеседник!", "Newsru.com", "Nature!", "Россия сегодня;Russia Today;RT!", "БЕЛТА", "Ассошиэйтед Пресс;Associated Press", "France Press;France Presse;Франс пресс;Agence France Presse;AFP", "СИНЬХУА", "Gallup", "Cable News Network;CNN", "CBS News", "ABC News", "GoogleNews;Google News", "FoxNews;Fox News", "Reuters;Рейтер", "British Broadcasting Corporation;BBC;БиБиСи;BBC News", "MSNBC", "Голос Америки", "Аль Джазира;Al Jazeera", "Радио Свобода", "Радио Свободная Европа", "Guardian;Гардиан", "Daily Telegraph", "Times;Таймс!", "Independent!", "Financial Times", "Die Welt", "Bild!", "La Pepublica;Република!", "Le Monde", "People Daily", "BusinessWeek", "Economist!", "Forbes;Форбс", "Los Angeles Times", "New York Times", "Wall Street Journal;WSJ", "Washington Post", "Le Figaro;Фигаро", "Bloomberg", "DELFI!"]: 
            ...
            if (ss.endswith("!")): 
                ...
                ss = ss[0 : (len(ss) - 1)]
            pp = Utils.splitString(ss, ';', False)
            t = Termin._new118(pp[0], OrgProfile.MEDIA)
            if (not is_bad): 
                t.tag2 = ss
            ii = 1
            while ii < len(pp): 
        ...
        for s in ["Машина времени!", "ДДТ", "Биттлз;Bittles", "ABBA;АББА", "Океан Эльзы;Океан Эльзи", "Аквариум!", "Крематорий!", "Наутилус;Наутилус Помпилиус!", "Пусси Райот;Пусси Риот;Pussy Riot", "Кино!", "Алиса!", "Агата Кристи!", "Чайф", "Ария!", "Земфира!", "Браво!", "Черный кофе!", "Воскресение!", "Урфин Джюс", "Сплин!", "Пикник!", "Мумий Троль", "Коррозия металла", "Арсенал!", "Ночные снайперы!", "Любэ", "Ласковый май!", "Noize MC", "Linkin Park", "ac dc", "green day!", "Pink Floyd;Пинк Флойд", "Depeche Mode", "Bon Jovi", "Nirvana;Нирвана!", "Queen;Квин!", "Nine Inch Nails", "Radioheads", "Pet Shop Boys", "Buggles"]: 
            ...
            if (ss.endswith("!")): 
                ...
                ss = ss[0 : (len(ss) - 1)]
            pp = Utils.splitString(ss, ';', False)
            t = Termin._new118(pp[0], OrgProfile.MUSIC)
            if (not is_bad): 
                t.tag2 = ss
            ii = 1
            while ii < len(pp): 
                ...
                ii += 1
            OrganizationAnalyzer.__m_prop_names.add(t)
        OrganizationAnalyzer.__m_prop_pref = TerminCollection()
        for s in ["ФАНАТ", "БОЛЕЛЬЩИК", "гонщик", "вратарь", "нападающий", "голкипер", "полузащитник", "полу-защитник", "центрфорвард", "центр-форвард", "форвард", "игрок", "легионер", "спортсмен"]: 
            OrganizationAnalyzer.__m_prop_pref.add(Termin._new118(s.upper(), OrgProfile.SPORT))
        for s in ["защитник", "капитан", "пилот", "игра", "поле", "стадион", "гонка", "чемпионат", "турнир", "заезд", "матч", "кубок", "олипмиада", "финал", "полуфинал", "победа", "поражение", "разгром", "дивизион", "олипмиада", "финал", "полуфинал", "играть", "выигрывать", "выиграть", "проигрывать", "проиграть", "съиграть"]: 
            OrganizationAnalyzer.__m_prop_pref.add(Termin._new120(s.upper(), OrgProfile.SPORT, s))
        for s in ["корреспондент", "фотокорреспондент", "репортер", "журналист", "тележурналист", "телеоператор", "главный редактор", "главред", "телеведущий", "редколлегия", "обозреватель", "сообщать", "сообщить", "передавать", "передать", "писать", "написать", "издавать", "пояснить", "пояснять", "разъяснить", "разъяснять", "сказать", "говорить", "спрашивать", "спросить", "отвечать", "ответить", "выяснять", "выяснить", "цитировать", "процитировать", "рассказать", "рассказывать", "информировать", "проинформировать", "поведать", "напечатать", "напоминать", "напомнить", "узнать", "узнавать", "репортаж", "интервью", "информации", "сведение", "ИА", "информагенство", "информагентство", "информационный", "газета", "журнал"]: 
            OrganizationAnalyzer.__m_prop_pref.add(Termin._new118(s.upper(), OrgProfile.MEDIA))
        for s in ["сообщение", "статья", "номер", "журнал", "издание", "издательство", "агентство", "цитата", "редактор", "комментатор", "по данным", "оператор", "вышедший", "отчет", "вопрос", "читатель", "слушатель", "телезритель", "источник", "собеедник"]: 
            OrganizationAnalyzer.__m_prop_pref.add(Termin._new120(s.upper(), OrgProfile.MEDIA, s))
    ...
    def __try_attach_army(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        if (not ((isinstance(t, NumberToken))) or t.whitespaces_after_count > 2): 
            return None
        typ = OrgItemTypeToken.try_attach(t.next0, True, ad)
        if (typ is None): 
            return None
        if (typ.root is not None and OrgProfile.ARMY in typ.root.profiles): 
            rt = self.__try_attach_org(t.next0, ad, OrganizationAnalyzer.AttachType.HIGH, None, False, 0, -1)
            if (rt is not None): 
                if (rt.begin_token == typ.begin_token): 
                    rt.begin_token = t
                    (rt.referent if isinstance(rt.referent, OrganizationReferent) else None).number = str((t if isinstance(t, NumberToken) else None).value)
                return rt
            org_ = OrganizationReferent()
            org_.add_type(typ, False)
            org_.number = str((t if isinstance(t, NumberToken) else None).value)
            return ReferentToken(org_, t, typ.end_token)
        return None
    ...
    def __try_attach_dep_before_org(self, typ : 'OrgItemTypeToken', rt_org : 'ReferentToken') -> 'ReferentToken':
        ...
        if (typ is None): 
            return None
        org_ = (None if rt_org is None else (rt_org.referent if isinstance(rt_org.referent, OrganizationReferent) else None))
        t = typ.end_token
        if (org_ is None): 
            t = t.next0
        ...
        if (org_ is None): 
            return None
        t1 = t
        if (isinstance(t1.next0, ReferentToken)): 
            geo0 = (t1.next0.get_referent() if isinstance(t1.next0.get_referent(), GeoReferent) else None)
        ...
        if ("главное управление" in dep.types or "головне управління" in dep.types or "пограничное управление" in dep.type_name): 
            if (typ.begin_token == typ.end_token): 
                if (org_.kind != OrganizationKind.GOVENMENT and org_.kind != OrganizationKind.BANK): 
                    return None
        if (not OrgOwnershipHelper.can_be_higher(org_, dep, False) and ((typ.root is None or not typ.root.can_be_normal_dep))): 
            if (len(dep.types) > 0 and dep.types[0] in org_.types and dep.can_be_equals(org_, Referent.EqualType.FORMERGING)): 
                dep.merge_slots(org_, False)
        ...
        self.__correct_dep_attrs(res, typ)
    ...
    def __try_attach_dep_after_org(self, typ : 'OrgItemTypeToken') -> 'ReferentToken':
        ...
        if (OrgOwnershipHelper.can_be_higher(org_, dep, False)): 
            dep.higher = org_
        elif (OrgOwnershipHelper.can_be_higher(dep, org_, False) and org_.higher is None): 
            org_.higher = dep
            t = t.next0
        else: 
            t = t.next0
    ...
    def __try_attach_dep(self, typ : 'OrgItemTypeToken', attach_typ : 'AttachType', spec_word_before : bool) -> 'ReferentToken':
        ...
        if ((typ.is_newline_after and typ.name is None and typ.typ != "курс") and ((typ.root is None or not typ.root.can_be_normal_dep))): 
            ...
            if (BracketHelper.can_be_start_of_sequence(tt2, False, False)): 
                ...
                return None
        if (not spec_word_before and typ.end_token.next0 is not None and (typ.end_token.whitespaces_after_count < 2)): 
            na0 = OrgItemNameToken.try_attach(typ.end_token.next0, None, False, True)
            if (na0 is not None and ((na0.std_org_name_nouns > 0 or na0.is_std_name))): 
                spec_word_before = True
            else: 
                rt00 = self.__try_attach_org(typ.end_token.next0, None, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, 0, -1)
                if (rt00 is not None): 
                    after_org = (rt00.referent if isinstance(rt00.referent, OrganizationReferent) else None)
                    spec_word_before = True
                    after_org_temp = True
                elif (isinstance(typ.end_token.next0, TextToken) and typ.end_token.next0.chars.is_all_upper): 
                    rrr = self.__try_attach_orgs(typ.end_token.next0, None, 0)
                    if (rrr is not None and len(rrr) == 1): 
                        after_org = (rrr[0].referent if isinstance(rrr[0].referent, OrganizationReferent) else None)
                        spec_word_before = True
                        after_org_temp = True
        if (((((((typ.root is not None and typ.root.can_be_normal_dep and not spec_word_before) and typ.typ != "отделение" and typ.typ != "инспекция") and typ.typ != "филиал" and typ.typ != "аппарат") and typ.typ != "відділення" and typ.typ != "інспекція") and typ.typ != "філія" and typ.typ != "апарат") and typ.typ != "совет" and typ.typ != "рада") and ((' ') not in typ.typ) and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
            return None
        ...
        if (typ.number is not None): 
            ...
            if (nnn is not None): 
                if (nnn.value >= 1 and nnn.value <= 6): 
        ...
        while True:
            ...
            if (t.is_char(':')): 
                ...
                if (names is not None or typ.name is not None): 
                    break
                continue
            num = OrgItemNumberToken.try_attach(t, False, None)
            if ((num) is not None): 
                ...
                if ((ty0) is not None and ty0.morph.number != MorphNumber.PLURAL and not ty0.is_doubt_root_word): 
                    ...
                    br = BracketHelper.try_parse(t, BracketParseAttr.NO, 100)
                    if (names is None and (br) is not None): 
                        ...
                        r = t.get_referent()
                        if ((r is None and t.morph.class0.is_preposition and t.next0 is not None) and isinstance(t.next0.get_referent(), GeoReferent)): 
                            dep._add_geo_object(t.next0.get_referent())
                            t = t.next0
                            break
                        if (r is not None): 
                            if (isinstance(r, OrganizationReferent)): 
                        ...
                        if (isinstance(t, NumberToken) and typ.root is not None and dep.number is None): 
                            if (t.whitespaces_before_count > 1): 
                                break
                            dep.number = str((t if isinstance(t, NumberToken) else None).value)
                            t1 = t
                            continue
                        if (is_pure_dep): 
                            break
                        if (not t.chars.is_all_lower): 
                            ...
                            if (rtp is not None and rtp.referent.type_name == "PERSONPROPERTY"): 
                                if (rtp.morph.case.is_genitive and t == typ.end_token.next0 and (t.whitespaces_before_count < 4)): 
                                    rtp = None
                            if (rtp is not None): 
                                break
                        ...
                        if (t.is_value("ПО", None) and t.next0 is not None and t.next0.is_value("РАЙОН", None)): 
                            na = OrgItemNameToken.try_attach(t.next0.next0, pr, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, True)
                        if (t.morph.class0.is_preposition and ((t.is_value("ПРИ", None) or t.is_value("OF", None) or t.is_value("AT", None)))): 
                            if (isinstance(t.next0, ReferentToken) and isinstance(t.next0.get_referent(), OrganizationReferent)): 
                                after_org = (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None)
                                break
                            rt0 = self.__try_attach_org(t.next0, None, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, 0, -1)
                            if (rt0 is not None): 
                                after_org = (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)
                                after_org_temp = True
                                break
                        if (na is None): 
                            break
                        if (names is None): 
                            ...
                            if (rt0 is not None): 
                                ...
                                break
                            names = list()
                        elif (t.whitespaces_before_count > 2 and na.chars != pr.chars): 
                            break
                        names.append(na)
                        pr = na
                        t = na.end_token
        ...
        if (br is not None and names is None): 
            ...
            if (not Utils.isNullOrEmpty(nam)): 
                if (len(nam) > 100): 
                    return None
                coef += 3
                na = OrgItemNameToken.try_attach(br.begin_token.next0, None, False, True)
            ...
            if (after_org is not None or attach_typ == OrganizationAnalyzer.AttachType.HIGH): 
                ...
                for j in range(len(names)):
                    if (((names[j].is_newline_before and not names[j].is_after_conjunction)) or ((names[j].chars != names[0].chars and names[j].std_org_name_nouns == 0))): 
                        break
                    else: 
            ...
            if (typ.typ == "курс" and typ.name[0].isdigit()): 
                dep.number = typ.name[0 : (typ.name.find(' '))]
            else: 
                dep.add_name_str(None, typ, 1)
            ...
            if (t is not None and t.is_char(':')): 
                t = t.next0
            if ((t is not None and isinstance(t, TextToken) and not t.is_newline_before) and t.morph.class0.is_adjective): 
                if (typ.morph.gender == t.morph.gender): 
                    s = t.get_normal_case_text(MorphClass.ADJECTIVE, False, MorphGender.UNDEFINED, False)
                    if (s is not None): 
            ...
            if (t is not None and not t.is_newline_before): 
                val = 0
                if (isinstance(t, NumberToken)): 
                    if (not t.morph.class0.is_noun): 
                        if (t.is_whitespace_after or t.next0.is_char_of(";,")): 
            ...
            if (not after_org_temp): 
                dep.higher = (after_org if isinstance(after_org, OrganizationReferent) else None)
        ...
        if (after_org is not None): 
            coef += 2
            if (typ.name is not None and OrgOwnershipHelper.can_be_higher(after_org if isinstance(after_org, OrganizationReferent) else None, dep, False)): 
                coef += 1
                if (not typ.chars.is_all_lower): 
                    coef += 0.5
        if (typ.typ == "курс" or typ.typ == "группа" or typ.typ == "група"): 
            if (dep.number is None): 
                ...
                inoutarg2046 = RefOutArgWrapper(None)
                inoutres2047 = Utils.tryParseInt(dep.number, inoutarg2046)
                n = inoutarg2046.value
                if (inoutres2047): 
                    if (n > 0 and (n < 9)): 
                        coef += 2
        if (t1.next0 is not None and t1.next0.is_char('(')): 
            ttt = t1.next0.next0
            if ((ttt is not None and ttt.next0 is not None and ttt.next0.is_char(')')) and isinstance(ttt, TextToken)): 
                if ((ttt if isinstance(ttt, TextToken) else None).term in dep._name_vars): 
                    coef += 2
                    dep.add_name((ttt if isinstance(ttt, TextToken) else None).term, True, ttt)
                    t1 = ttt.next0
        ep_ = OrgItemEponymToken.try_attach(t1.next0, False)
        if (ep_ is not None): 
            coef += 2
            for e0 in ep_.eponyms: 
                dep.add_eponym(e0)
            t1 = ep_.end_token
        if (br_name is not None): 
            str1 = MiscHelper.get_text_value(br_name.begin_token.next0, br_name.end_token.previous, GetTextAttr.NO)
            if (str1 is not None): 
    ...
    def __correct_dep_attrs(self, res : 'ReferentToken', typ : 'OrgItemTypeToken') -> None:
        from pullenti.ner.Referent import Referent
        from pullenti.ner.org.OrganizationReferent import OrganizationReferent
        from pullenti.ner.NumberToken import NumberToken
        from pullenti.ner.geo.GeoReferent import GeoReferent
        ...
        from pullenti.ner.core.MiscHelper import MiscHelper
        t0 = res.begin_token
        dep = (res.referent if isinstance(res.referent, OrganizationReferent) else None)
        if ((((((typ is not None and typ.root is not None and typ.root.can_has_number)) or "отдел" in dep.types or "отделение" in dep.types) or "инспекция" in dep.types or "лаборатория" in dep.types) or "відділ" in dep.types or "відділення" in dep.types) or "інспекція" in dep.types or "лабораторія" in dep.types): 
            if ((isinstance(t0.previous, NumberToken) and (t0.whitespaces_before_count < 2) and not t0.previous.morph.class0.is_noun) and t0.previous.is_whitespace_before): 
                nn = str((t0.previous if isinstance(t0.previous, NumberToken) else None).value)
        ...
        if ("управление" in dep.types or "департамент" in dep.types or "управління" in dep.types): 
            for s in dep.slots: 
                if (s.type_name == OrganizationReferent.ATTR_GEO and isinstance(s.value, GeoReferent)): 
                    g = (s.value if isinstance(s.value, GeoReferent) else None)
        ...
        if (br is not None and (t1.whitespaces_after_count < 2) and br.is_quote_type): 
            g = self.__is_geo(br.begin_token.next0, False)
            if (isinstance(g, ReferentToken)): 
                if ((g if isinstance(g, ReferentToken) else None).end_token.next0 == br.end_token): 
                    dep._add_geo_object(g)
                    res.end_token = br.end_token
                    t1 = res.end_token
            elif (isinstance(g, Referent) and br.begin_token.next0.next0 == br.end_token): 
                dep._add_geo_object(g)
                res.end_token = br.end_token
                t1 = res.end_token
            elif (br.begin_token.next0.is_value("О", None) or br.begin_token.next0.is_value("ОБ", None)): 
                pass
            else: 
                nam = MiscHelper.get_text_value_of_meta_token(br, GetTextAttr.NO)
                if (nam is not None): 
                    dep.add_name(nam, True, br.begin_token.next0)
                    res.end_token = br.end_token
                    t1 = res.end_token
        prep = False
        if (t1.next0 is not None): 
            if (t1.next0.morph.class0.is_preposition): 
                if (t1.next0.is_value("В", None) or t1.next0.is_value("ПО", None)): 
                    t1 = t1.next0
                    prep = True
        for k in range(2):
            if (t1.next0 is None): 
                return
            geo_ = (t1.next0.get_referent() if isinstance(t1.next0.get_referent(), GeoReferent) else None)
            ge = False
            if (geo_ is not None): 
                ...
                rgeo = t1.kit.process_referent("GEO", t1.next0)
                if (rgeo is not None): 
                    if (not rgeo.morph.class0.is_adjective): 
    ...
    def __try_attach_org(self, t : 'Token', ad : 'OrgAnalyzerData', attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken'=None, is_additional_attach : bool=False, level : int=0, step : int=-1) -> 'ReferentToken':
        ...
        if (level > 2 or t is None): 
            return None
        if (t.chars.is_latin_letter and MiscHelper.is_eng_article(t)): 
            re = self.__try_attach_org(t.next0, ad, attach_typ, mult_typ, is_additional_attach, level, step)
            if (re is not None): 
                re.begin_token = t
                return re
        org_ = None
        ...
        while True:
            ...
            if ((rt0 is None and typ is not None and typ.geo is not None) and typ.begin_token.next0 == typ.end_token): 
            ...
            if (rt0 is not None): 
                if (attach_typ == OrganizationAnalyzer.AttachType.MULTIPLE): 
                    if (types is None or len(types) == 0): 
                        return None
                    if (not OrgItemTypeToken.is_type_accords(rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None, types[0])): 
                        return None
                    (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None).add_type(types[0], False)
                    if ((rt0.begin_token.begin_char - types[0].end_token.next0.end_char) < 3): 
                        rt0.begin_token = types[0].begin_token
                    break
                if (typ is not None and not typ.end_token.morph.class0.is_verb): 
                    if (OrgItemTypeToken.is_type_accords(rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None, typ)): 
                        rt0.begin_token = typ.begin_token
                        (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None).add_type(typ, False)
                break
            if (t.is_hiphen): 
                if (t == t0 or types is None): 
                    if (ot_ex_li is not None): 
                        break
                    return None
                if ((typ is not None and typ.root is not None and typ.root.can_has_number) and isinstance(t.next0, NumberToken)): 
                    pass
                else: 
                    hiph = True
                continue
            if (ad is not None and ot_ex_li is None): 
                ...
                if (t.inner_bool): 
                    ...
                    ok1 = True
                elif (BracketHelper.can_be_start_of_sequence(t, True, False) and t.next0 is not None): 
                    ok1 = True
                    tt = t.next0
            ...
            if ((step >= 0 and not t.inner_bool and t == t0) and isinstance(t, TextToken)): 
                ...
                if (typ is None and BracketHelper.can_be_start_of_sequence(t, False, False)): 
                    br = BracketHelper.try_parse(t, BracketParseAttr.NO, 100)
                    if (br is not None): 
                        typ = OrgItemTypeToken.try_attach(t.next0, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, ad)
                        if (typ is not None and typ.end_token == br.end_token.previous and ((BracketHelper.can_be_start_of_sequence(br.end_token.next0, True, False) or t.is_char('(')))): 
            ...
            if (types is None): 
                ...
                types = list()
                t0 = typ.begin_token
                if (typ.is_not_typ and typ.end_token.next0 is not None): 
                    t0 = typ.end_token.next0
                if (OrgItemTypeToken.check_org_special_word_before(typ.begin_token.previous)): 
                    spec_word_before = True
            else: 
                ok = True
                ...
                if (not pref0 and not pref): 
                    if (typ0.name is not None and len(typ0.name) != len(typ0.typ)): 
                        if (t.whitespaces_before_count > 1): 
                            break
                    if (not typ0.morph.case.is_undefined and not typ.morph.case.is_undefined): 
                        if (not (typ0.morph.case & typ.morph.case).is_nominative and not hiph): 
                            if (not typ.morph.case.is_nominative): 
                                break
                ...
                if (not pref0 and pref and not hiph): 
                    ...
                    for m in typ.morph.items: 
                        if (m.number == MorphNumber.SINGULAR and m.case.is_nominative): 
                            nom = True
                            break
                    if (not nom): 
            ...
            in_brackets = False
            if (typ.name is not None): 
                if (BracketHelper.can_be_start_of_sequence(typ.begin_token.previous, True, False) and BracketHelper.can_be_end_of_sequence(typ.end_token.next0, False, None, False)): 
                    typ.begin_token = typ.begin_token.previous
                    typ.end_token = typ.end_token.next0
        ...
        if ((types is None and ot_ex_li is None and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))) and rt0 is None): 
            ok = False
            if (not ok): 
                if (t0 is not None and t0.morph.class0.is_adjective and t0.next0 is not None): 
                    rt0 = self.__try_attach_org(t0.next0, ad, attach_typ, mult_typ, is_additional_attach, level + 1, step)
                    if ((rt0) is not None): 
                ...
                if ((((t0.kit.recurse_level < 4) and isinstance(t0, TextToken) and t0.previous is not None) and t0.length_char > 2 and not t0.chars.is_all_lower) and not t0.is_newline_after and not MiscHelper.can_be_start_of_sentence(t0)): 
                    typ = OrgItemTypeToken.try_attach(t0.next0, False, None)
                    if (typ is not None): 
                        t0.kit.recurse_level += 1
                        rrr = self.__try_attach_org(t0.next0, ad, attach_typ, mult_typ, is_additional_attach, level + 1, step)
                        t0.kit.recurse_level -= 1
                        if (rrr is None): 
                            if (spec_word_before or t0.previous.is_value("ТЕРРИТОРИЯ", None)): 
                                org0 = OrganizationReferent()
                                org0.add_type(typ, False)
                                org0.add_name((t0 if isinstance(t0, TextToken) else None).term, False, t0)
                                t1 = typ.end_token
                                t1 = (Utils.ifNotNull(self.__attach_tail_attributes(org0, t1.next0, ad, False, OrganizationAnalyzer.AttachType.NORMAL, False), t1))
                ...
                while True:
                    ...
                    if (((isinstance(tt, TextToken) and tt.chars.is_letter and not tt.chars.is_all_lower) and not tt.chars.is_capital_upper and tt.length_char > 1) and (tt.whitespaces_after_count < 2)): 
                        ...
                        break
                    if (isinstance(tt.next0, ReferentToken) and isinstance(tt.next0.get_referent(), OrganizationReferent)): 
                        ttt = t.previous
                        ...
                        if (nam == "СЭД"): 
                            break
                        own = (tt.next0.get_referent() if isinstance(tt.next0.get_referent(), OrganizationReferent) else None)
                        if (OrgProfile.UNIT in own.profiles): 
                            break
                        if (nam == "НК" or nam == "ГК"): 
                            return ReferentToken(own, t, tt.next0)
                        org0 = OrganizationReferent()
                        org0.add_profile(OrgProfile.UNIT)
        ...
        if (rt0 is None): 
            ...
            if (rt0 is not None): 
                if (types is not None and rt0.begin_token == types[0].begin_token): 
                    for ty in types: 
                        (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None).add_type(ty, True)
                if ((rt0.begin_token == t0 and t0.previous is not None and t0.previous.morph.class0.is_adjective) and (t0.whitespaces_before_count < 2)): 
                    if (len((rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)._geo_objects) == 0): 
        ...
        if (ot_ex_li is not None and rt0 is None and (len(ot_ex_li) < 10)): 
            for ot in ot_ex_li: 
                ...
                if (BracketHelper.can_be_start_of_sequence(ts.previous, False, False) and BracketHelper.is_bracket(ts.previous, False)): 
                    if (BracketHelper.can_be_end_of_sequence(te.next0, False, None, False)): 
                        if (ot.length_char < 2): 
                            continue
                ...
                if (ot.end_token.next0 is not None and isinstance(ot.end_token.next0.get_referent(), OrganizationReferent)): 
                    ok = True
                    ...
                    ok = False
                    len0 = ot.begin_token.length_char
                    if (not ot.chars.is_all_lower): 
                        if (not ot.chars.is_all_upper and ot.morph.class0.is_preposition): 
                            continue
                        name_eq = False
                        for n in org0.names: 
                        ...
                        if (ano is None): 
                            ...
                            if (d > 2000): 
                                ...
                                if (d > 100 or not ot.begin_token.chars.is_all_upper): 
                                    is_very_doubt = True
                        if (((ot.begin_token.chars.is_all_upper or ot.begin_token.chars.is_last_lower)) and ((len0 > 3 or ((len0 == 3 and ((name_eq or ano is not None))))))): 
                            ok = True
                        elif (spec_word_before or types is not None or is_quots): 
                            ok = True
                        elif (ot.length_char < 3): 
                            continue
                        elif (ot.item.owner.is_ext_ontology and ot.begin_token.get_morph_class_in_dictionary().is_undefined and ((len0 > 3 or ((len0 == 3 and ((name_eq or ano is not None))))))): 
                            ok = True
                ...
                if (ok): 
                    ...
                    if (types is not None): 
                        for ty in types: 
                            org_.add_type(ty, False)
                        if (not org_.can_be_equals(org0, Referent.EqualType.FORMERGING)): 
                            continue
                    else: 
                        for ty in org0.types: 
                            org_.add_type_str(ty)
                    if (org0.number is not None and isinstance(ot.begin_token.previous, NumberToken) and org_.number is None): 
                        if (org0.number != str((ot.begin_token.previous if isinstance(ot.begin_token.previous, NumberToken) else None).value) and (ot.begin_token.whitespaces_before_count < 2)): 
                            if (len(org_.names) > 0 or org_.higher is not None): 
                                is_very_doubt = False
                                ok = True
                                org_.number = str((ot.begin_token.previous if isinstance(ot.begin_token.previous, NumberToken) else None).value)
                                if (org0.higher is not None): 
                                    org_.higher = org0.higher
                                t0 = ot.begin_token.previous
                    if (org_.number is None): 
                        ttt = ot.end_token.next0
                        ...
                        if (nnn is None and not ot.is_whitespace_after and ttt is not None): 
                            if (ttt.is_hiphen and ttt.next0 is not None): 
                                ttt = ttt.next0
                            if (isinstance(ttt, NumberToken)): 
                                nnn = OrgItemNumberToken._new1539(ot.end_token.next0, ttt, str((ttt if isinstance(ttt, NumberToken) else None).value))
                        if (nnn is not None): 
                            org_.number = nnn.number
                            te = nnn.end_token
                    ...
                    org_.add_name(s, True, (None if norm else ot.begin_token))
                    if (types is None or len(types) == 0): 
                        s1 = MiscHelper.get_text_value_of_meta_token(ot, GetTextAttr.IGNOREARTICLES)
                        if (s1 != s and norm): 
                            org_.add_name(s1, True, ot.begin_token)
                    t1 = te
                    if (t1.is_char(')') and t1.is_newline_after): 
                        pass
                    else: 
                        t1 = (Utils.ifNotNull(self.__attach_middle_attributes(org_, t1.next0), t1))
                        if (attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                            t1 = (Utils.ifNotNull(self.__attach_tail_attributes(org_, t1.next0, ad, False, OrganizationAnalyzer.AttachType.NORMAL, False), t1))
                    hi = None
                    if (t1.next0 is not None): 
                        hi = (t1.next0.get_referent() if isinstance(t1.next0.get_referent(), OrganizationReferent) else None)
                    if (org0.higher is not None and hi is not None and len(ot_ex_li) == 1): 
                        if (hi.can_be_equals(org0.higher, Referent.EqualType.WITHINONETEXT)): 
                            org_.higher = hi
                            t1 = t1.next0
                    if ((len(org_.eponyms) == 0 and org_.number is None and is_very_doubt) and types is None): 
                        continue
                    if (not org_.can_be_equals_ex(org0, True, Referent.EqualType.WITHINONETEXT)): 
                        if (t is not None and OrgItemTypeToken.check_org_special_word_before(t.previous)): 
                            ...
                            if (not is_very_doubt): 
                                if (len(org_.eponyms) > 0 or org_.number is not None or org_.higher is not None): 
                                    ok = True
                            ok = False
                    elif (org_.can_be_equals(org0, Referent.EqualType.DIFFERENTTEXTS)): 
                        org_.merge_slots(org0, False)
                        ok = True
                    elif (org0.higher is None or org_.higher is not None or ot.item.owner.is_ext_ontology): 
                        ok = True
                        org_.merge_slots(org0, False)
                    elif (not ot.item.owner.is_ext_ontology and org_.can_be_equals(org0, Referent.EqualType.WITHINONETEXT)): 
                    ...
                    if (ts.begin_char < t0.begin_char): 
                        t0 = ts
                    rt0 = ReferentToken(org_, t0, t1)
                    if (org_.kind == OrganizationKind.DEPARTMENT): 
                        self.__correct_dep_attrs(rt0, typ)
                    if (ot.item.owner.is_ext_ontology): 
                        for sl in org_.slots: 
                            if (isinstance(sl.value, Referent)): 
                                ext = False
        ...
        if ((rt0 is None and types is not None and len(types) == 1) and types[0].name is None): 
            ...
            if (MiscHelper.is_eng_article(types[0].begin_token)): 
            ...
            elif (MiscHelper.is_eng_adj_suffix(types[0].end_token.next0)): 
                tt0 = types[0].begin_token
            else: 
                ...
                if (tt00 is not None and (tt00.whitespaces_after_count < 2) and tt00.chars.is_latin_letter == types[0].chars.is_latin_letter): 
                    if (MiscHelper.is_eng_article(tt00)): 
                        tt0 = tt00
                    elif (tt00.morph.class0.is_preposition or tt00.morph.class0.is_pronoun): 
                        tt0 = tt00
            cou = 100
            if (tt0 is not None): 
                tt00 = tt0.previous
                while tt00 is not None and cou > 0: 
                    if (isinstance(tt00.get_referent(), OrganizationReferent)): 
                        if (OrgItemTypeToken.is_type_accords((tt00.get_referent() if isinstance(tt00.get_referent(), OrganizationReferent) else None), types[0])): 
        ...
        if (hiph and not in_brackets and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))): 
            ...
            if (rt0 is not None and BracketHelper.can_be_end_of_sequence(rt0.end_token, True, None, False)): 
                if (len(types) > 0): 
                    ...
                    if (ty.end_token.next0 is not None and ty.end_token.next0.is_hiphen and BracketHelper.can_be_start_of_sequence(ty.end_token.next0.next0, True, False)): 
                        ok1 = True
            elif (rt0 is not None and rt0.end_token.next0 is not None and rt0.end_token.next0.is_hiphen): 
                ty = OrgItemTypeToken.try_attach(rt0.end_token.next0.next0, False, None)
                if (ty is None): 
                    ok1 = True
            if (not ok1): 
                return None
        ...
        org_ = (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)
        if (doubt and ad is not None): 
            rli = ad.local_ontology.try_attach_by_referent(org_, None, True)
            if (rli is not None and len(rli) > 0): 
                doubt = False
            else: 
                for it in ad.local_ontology.items: 
                    if (it.referent is not None): 
                        if (it.referent.can_be_equals(org_, Referent.EqualType.WITHINONETEXT)): 
                            doubt = False
                            break
        if ((ad is not None and t is not None and t.kit.ontology is not None) and attach_typ == OrganizationAnalyzer.AttachType.NORMAL and doubt): 
            rli = t.kit.ontology.attach_referent(org_)
            if (rli is not None): 
                if (len(rli) >= 1): 
                    doubt = False
        if (doubt): 
    ...
    def __correct_after(self, rt0 : 'ReferentToken') -> None:
        ...
        if (rt0 is None): 
            return
        if (not rt0.is_newline_after and rt0.end_token.next0 is not None and rt0.end_token.next0.is_char('(')): 
            tt = rt0.end_token.next0.next0
            if (isinstance(tt, TextToken)): 
                if (tt.is_char(')')): 
                    rt0.end_token = tt
                    ...
                    if (isinstance(tt.next0, NumberToken) and not tt.is_whitespace_after and (tt.next0 if isinstance(tt.next0, NumberToken) else None).typ == NumberSpellingType.DIGIT): 
                        tt = tt.next0
                    ...
                    if (tt.next0 is not None and tt.next0.is_char(')')): 
                        ...
                        rt0.end_token = tt.next0
                else: 
                    org_ = (rt0.referent if isinstance(rt0.referent, OrganizationReferent) else None)
                    if (org_.kind == OrganizationKind.BANK and tt.chars.is_latin_letter): 
                        pass
    ...
    def __try_attach_org_(self, t0 : 'Token', t : 'Token', ad : 'OrgAnalyzerData', types : typing.List['OrgItemTypeToken'], spec_word_before : bool, attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken', is_additional_attach : bool, level : int) -> 'ReferentToken':
        ...
        if (typ is not None): 
            ...
            if (typ.is_newline_after and typ.name is None): 
                if (t1 is not None and isinstance(t1.get_referent(), GeoReferent) and OrgProfile.STATE in typ.profiles): 
                    ...
                    pass
                elif ((typ.coef >= 3 and (typ.newlines_after_count < 2) and typ.end_token.next0 is not None) and typ.end_token.next0.morph.class0.is_preposition): 
                    pass
                else: 
                    return None
            if (typ != mult_typ and ((typ.morph.number == MorphNumber.PLURAL and not typ.typ[0].isupper()))): 
                if (BracketHelper.can_be_start_of_sequence(t, True, False)): 
                    pass
        ...
        if (typ is not None and typ.root is not None and typ.root.is_pure_prefix): 
            if (isinstance(t, TextToken) and t.chars.is_all_upper and not t.is_newline_after): 
                b = BracketHelper.try_parse(t.next0, BracketParseAttr.NO, 100)
                if (b is not None and b.is_quote_type): 
                    org_.add_type_str((t if isinstance(t, TextToken) else None).term)
                    t = t.next0
                else: 
                    s = (t if isinstance(t, TextToken) else None).term
                    if (len(s) == 2 and s[len(s) - 1] == 'К'): 
                        org_.add_type_str(s)
                        t = t.next0
                    elif (((t.get_morph_class_in_dictionary().is_undefined and t.next0 is not None and isinstance(t.next0, TextToken)) and t.next0.chars.is_capital_upper and t.next0.next0 is not None) and not t.next0.is_newline_after): 
                        if (t.next0.next0.is_char_of(",.;") or BracketHelper.can_be_end_of_sequence(t.next0.next0, False, None, False)): 
                            org_.add_type_str(s)
                            t = t.next0
            elif (isinstance(t, TextToken) and t.morph.class0.is_adjective and not t.chars.is_all_lower): 
                rtg = (self.__is_geo(t, True) if isinstance(self.__is_geo(t, True), ReferentToken) else None)
                if (rtg is not None and BracketHelper.can_be_start_of_sequence(rtg.end_token.next0, False, False)): 
                    org_._add_geo_object(rtg)
                    t = rtg.end_token.next0
            elif ((t is not None and isinstance(t.get_referent(), GeoReferent) and t.next0 is not None) and BracketHelper.can_be_start_of_sequence(t.next0, True, False)): 
                org_._add_geo_object(t.get_referent())
                t = t.next0
        te = None
        ki0 = org_.kind
        if (((((ki0 == OrganizationKind.GOVENMENT or ki0 == OrganizationKind.AIRPORT or ki0 == OrganizationKind.FACTORY) or ki0 == OrganizationKind.PARTY or ki0 == OrganizationKind.JUSTICE) or ki0 == OrganizationKind.MILITARY)) and t is not None): 
            ...
            if (g is None and t.morph.class0.is_preposition and t.next0 is not None): 
                g = self.__is_geo(t.next0, False)
            if (g is not None): 
                if (org_._add_geo_object(g)): 
                    ...
                    if (gt is not None and len(gt) == 1): 
                        if (org_.can_be_equals(gt[0].item.referent, Referent.EqualType.FORMERGING)): 
                            org_.merge_slots(gt[0].item.referent, False)
                            return ReferentToken(org_, t0, gt[0].end_token)
        if (typ is not None and typ.root is not None and ((typ.root.can_be_single_geo and not typ.root.can_has_single_name))): 
            if (len(org_._geo_objects) > 0 and te is not None): 
                return ReferentToken(org_, t0, te)
            r = None
            t1 = (typ.end_token if typ != mult_typ else t0.previous)
            te = t1
            if (t is not None and t1.next0 is not None): 
                r = self.__is_geo(t1.next0, False)
                if (r is None and t1.next0.morph.class0.is_preposition): 
                    r = self.__is_geo(t1.next0.next0, False)
            if (r is not None): 
                if (not org_._add_geo_object(r)): 
                    return None
                te = self.__get_geo_end_token(r, t1.next0)
            if (len(org_._geo_objects) > 0 and te is not None): 
                npt11 = NounPhraseHelper.try_parse(te.next0, NounPhraseParseAttr.NO, 0)
        ...
        if (typ is not None and (((typ.typ == "милиция" or typ.typ == "полиция" or typ.typ == "міліція") or typ.typ == "поліція"))): 
            if (len(org_._geo_objects) > 0 and te is not None): 
                ...
                return None
        if (t is not None and t.morph.class0.is_proper_name): 
            rt1 = t.kit.process_referent("PERSON", t)
            if (rt1 is not None and (rt1.whitespaces_after_count < 2)): 
                if (BracketHelper.can_be_start_of_sequence(rt1.end_token.next0, True, False)): 
                    t = rt1.end_token.next0
                elif (rt1.end_token.next0 is not None and rt1.end_token.next0.is_hiphen and BracketHelper.can_be_start_of_sequence(rt1.end_token.next0.next0, True, False)): 
                    t = rt1.end_token.next0.next0
        elif ((t is not None and t.chars.is_capital_upper and t.morph.class0.is_proper_surname) and t.next0 is not None and (t.whitespaces_after_count < 2)): 
            if (BracketHelper.can_be_start_of_sequence(t.next0, True, False)): 
                t = t.next0
            elif (((t.next0.is_char_of(":") or t.next0.is_hiphen)) and BracketHelper.can_be_start_of_sequence(t.next0.next0, True, False)): 
                t = t.next0.next0
        tmax = None
        br = None
        if (t is not None): 
            ...
            if (typ is not None and br is None and BracketHelper.can_be_start_of_sequence(t, False, False)): 
                if (t.next0 is not None and isinstance(t.next0.get_referent(), OrganizationReferent)): 
                    org0 = (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None)
                    if (not OrgItemTypeToken.is_types_antagonisticoo(org_, org0)): 
                        org0.merge_slots(org_, False)
                        return ReferentToken(org0, t0, t.next0)
                if (((typ.typ == "компания" or typ.typ == "предприятие" or typ.typ == "организация") or typ.typ == "компанія" or typ.typ == "підприємство") or typ.typ == "організація"): 
                    if (OrgItemTypeToken.is_decree_keyword(t0.previous, 1)): 
                        return None
                ty2 = OrgItemTypeToken.try_attach(t.next0, False, None)
                if (ty2 is not None): 
                    typs2 = list()
                    typs2.append(ty2)
                    rt2 = self.__try_attach_org_(t.next0, ty2.end_token.next0, ad, typs2, True, OrganizationAnalyzer.AttachType.HIGH, None, is_additional_attach, level + 1)
                    if (rt2 is not None): 
                        org0 = (rt2.referent if isinstance(rt2.referent, OrganizationReferent) else None)
                        if (not OrgItemTypeToken.is_types_antagonisticoo(org_, org0)): 
                            org0.merge_slots(org_, False)
                            rt2.begin_token = t0
                            if (BracketHelper.can_be_end_of_sequence(rt2.end_token.next0, False, None, False)): 
                                rt2.end_token = rt2.end_token.next0
                            return rt2
        if (br is not None and typ is not None and org_.kind == OrganizationKind.GOVENMENT): 
            if (typ.root is not None and not typ.root.can_has_single_name): 
                br = None
        if (br is not None and br.is_quote_type): 
            if (br.begin_token.next0.is_value("О", None) or br.begin_token.next0.is_value("ОБ", None)): 
                br = None
        ...
        if (br is not None and br.is_quote_type and ((br.open_char != '<' or ((typ is not None and typ.root is not None and typ.root.is_pure_prefix))))): 
            if (t.is_newline_before and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))): 
                if (not br.is_newline_after): 
                    return None
            if (org_.find_slot(OrganizationReferent.ATTR_TYPE, "организация", True) is not None or org_.find_slot(OrganizationReferent.ATTR_TYPE, "організація", True) is not None): 
                return None
            if (typ is not None and ((((typ.typ == "компания" or typ.typ == "предприятие" or typ.typ == "организация") or typ.typ == "компанія" or typ.typ == "підприємство") or typ.typ == "організація"))): 
                if (OrgItemTypeToken.is_decree_keyword(t0.previous, 1)): 
                    return None
            nn = OrgItemNameToken.try_attach(t.next0, None, False, True)
            if (nn is not None and nn.is_ignored_part): 
                t = nn.end_token
            org0 = (t.next0.get_referent() if isinstance(t.next0.get_referent(), OrganizationReferent) else None)
            if (org0 is not None): 
                if (not OrgItemTypeToken.is_types_antagonisticoo(org_, org0) and t.next0.next0 is not None): 
                    if (BracketHelper.can_be_end_of_sequence(t.next0.next0, False, None, False)): 
                        org0.merge_slots(org_, False)
                        return ReferentToken(org0, t0, t.next0.next0)
                    if (isinstance(t.next0.next0.get_referent(), OrganizationReferent) and BracketHelper.can_be_end_of_sequence(t.next0.next0.next0, False, None, False)): 
                        org0.merge_slots(org_, False)
                        return ReferentToken(org0, t0, t.next0)
                return None
            if (len(br.internal) > 1): 
                return None
            na0 = OrgItemNameToken.try_attach(br.begin_token.next0, None, False, True)
            if (na0 is not None and na0.is_empty_word and na0.end_token.next0 == br.end_token): 
                return None
            rt0 = self.__try_attach_org(t.next0, None, attach_typ, None, is_additional_attach, level + 1, -1)
            abbr = None
            tt00 = (None if rt0 is None else rt0.begin_token)
            if (((rt0 is None and t.next0 is not None and isinstance(t.next0, TextToken)) and t.next0.chars.is_all_upper and t.next0.length_char > 2) and t.next0.chars.is_cyrillic_letter): 
                rt0 = self.__try_attach_org(t.next0.next0, None, attach_typ, None, is_additional_attach, level + 1, -1)
                if (rt0 is not None and rt0.begin_token == t.next0.next0): 
                    tt00 = t.next0
                    abbr = t.next0.get_source_text()
                else: 
                    rt0 = None
            ok2 = False
            if (rt0 is not None): 
                if (rt0.end_token == br.end_token.previous or rt0.end_token == br.end_token): 
                    ...
                    br2 = BracketHelper.try_parse(br.end_token.next0, BracketParseAttr.NO, 100)
            ...
            if (ok2): 
                ...
                typ1 = None
                if (tt00 != t.next0): 
                    typ1 = OrgItemTypeToken.try_attach(t.next0, False, ad)
                    if (typ1 is not None and typ1.end_token.next0 == tt00): 
                        org_.add_type(typ1, False)
                hi = False
                if (OrgOwnershipHelper.can_be_higher(org0, org_, True)): 
                    if (OrgItemTypeToken.is_types_antagonisticoo(org0, org_)): 
                        hi = True
                if (hi): 
                    org_.higher = org0
                    rt0.set_default_local_onto(t.kit.processor)
                    org_.add_ext_referent(rt0)
                    if (typ1 is not None): 
                        org_.add_type(typ1, True)
                    if (abbr is not None): 
                        org_.add_name(abbr, True, None)
                elif (not OrgItemTypeToken.is_types_antagonisticoo(org0, org_)): 
                    org_.merge_slots(org0, True)
                    if (abbr is not None): 
                        for s in org_.slots: 
                            if (s.type_name == OrganizationReferent.ATTR_NAME): 
                                org_.upload_slot(s, "{0} {1}".format(abbr, s.value))
                else: 
                    rt0 = None
                if (rt0 is not None): 
                    t11 = br.end_token
                    if (rt0.end_char > t11.end_char): 
                        t11 = rt0.end_token
                    ep11 = OrgItemEponymToken.try_attach(t11.next0, True)
                    if (ep11 is not None): 
                        t11 = ep11.end_token
                        for e0 in ep11.eponyms: 
                            org_.add_eponym(e0)
                    t1 = self.__attach_tail_attributes(org_, t11.next0, None, True, attach_typ, False)
                    if (t1 is None): 
                        t1 = t11
                    if (typ is not None): 
                        if ((typ.name is not None and typ.geo is None and len(org_.names) > 0) and not typ.name in org_.names): 
                            org_.add_type_str(typ.name.lower())
                    return ReferentToken(org_, t0, t1)
            if (rt0 is not None and (rt0.end_char < br.end_token.previous.end_char)): 
                rt1 = self.__try_attach_org(rt0.end_token.next0, None, attach_typ, None, is_additional_attach, level + 1, -1)
                if (rt1 is not None and rt1.end_token.next0 == br.end_token): 
                    return rt1
                org1 = (rt0.end_token.next0.get_referent() if isinstance(rt0.end_token.next0.get_referent(), OrganizationReferent) else None)
            ...
            for step in range(2):
                ...
                while True:
                    ...
                    if (t1.is_char('(')): 
                        ...
                        r = None
                        if (t1.next0 is not None): 
                            r = t1.next0.get_referent()
                        if (r is not None and t1.next0.next0 is not None and t1.next0.next0.is_char(')')): 
                            if (r.type_name == OrganizationAnalyzer.__geoname): 
                                org_._add_geo_object(r)
                                break
                        if (level == 0): 
                            rt = self.__try_attach_org(t1.next0, None, OrganizationAnalyzer.AttachType.HIGH, None, False, level + 1, -1)
                            if (rt is not None and rt.end_token.next0 is not None and rt.end_token.next0.is_char(')')): 
                                if (not OrganizationReferent.can_be_second_definition(org_, (rt.referent if isinstance(rt.referent, OrganizationReferent) else None))): 
                                    break
                                org_.merge_slots(rt.referent, False)
                        break
                    else: 
                        org0 = (t1.get_referent() if isinstance(t1.get_referent(), OrganizationReferent) else None)
                        if ((org0) is not None): 
                            if ((isinstance(t1.previous, NumberToken) and t1.previous.previous == br.begin_token and not OrgItemTypeToken.is_types_antagonisticoo(org_, org0)) and org0.number is None): 
                                org0.number = str((t1.previous if isinstance(t1.previous, NumberToken) else None).value)
                                org0.merge_slots(org_, False)
                                if (BracketHelper.can_be_end_of_sequence(t1.next0, False, None, False)): 
                                    t1 = t1.next0
                                return ReferentToken(org0, t0, t1)
                            ne = OrgItemNameToken.try_attach(br.begin_token.next0, None, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, True)
                            if (ne is not None and ne.is_ignored_part and ne.end_token.next0 == t1): 
                                org0.merge_slots(org_, False)
                                if (BracketHelper.can_be_end_of_sequence(t1.next0, False, None, False)): 
                                    t1 = t1.next0
                                return ReferentToken(org0, t0, t1)
                            ...
                            if (typ is not None and types is not None): 
                                for ty in types: 
                                    if (OrgItemTypeToken.is_types_antagonistictt(ty, typ)): 
                                        ...
                                        break
                            if (typ is not None): 
                                if (typ.is_doubt_root_word and ((typ.end_token.next0 == br.end_token or ((typ.end_token.next0 is not None and typ.end_token.next0.is_hiphen))))): 
                                    typ = None
                                elif (typ.morph.number == MorphNumber.PLURAL): 
                                    typ = None
                                elif (not typ.morph.case.is_undefined and not typ.morph.case.is_nominative): 
                                    typ = None
                                elif (typ.begin_token == typ.end_token): 
                                    ttt = typ.end_token.next0
                            ...
                            if (nu is not None and not ((isinstance(t1, NumberToken)))): 
                                org_.number = nu.number
                                tt1 = t1.previous
                                t1 = nu.end_token
                                not_empty += 2
                                continue
                            br_spec = False
                            if ((len(br.internal) == 0 and isinstance(br.end_token.next0, TextToken) and ((not br.end_token.next0.chars.is_all_lower))) and BracketHelper.can_be_end_of_sequence(br.end_token.next0.next0, True, None, False)): 
                                br_spec = True
                            if (typ is not None and ((pref or not typ.is_dep))): 
                                ...
                                if (((attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY or attach_typ == OrganizationAnalyzer.AttachType.HIGH)) and ((typ.root is None or not typ.root.is_pure_prefix))): 
                                    ...
                                    org_.add_type(typ, False)
                                    if (pref): 
                                        tt0 = typ.end_token.next0
                                    elif (typ.root is not None and typ.root.is_pure_prefix): 
                                        tt1 = typ.begin_token.previous
                                        break
                                elif (typ.end_token.next0 != br.end_token): 
                                    org_.add_type(typ, False)
                                    if (typ.typ == "банк"): 
                                        pref = False
                                    else: 
                                        org_.add_type_str(typ.name.lower())
                                        org_.add_type_str(typ.alt_typ)
                                        if (pref): 
                                            tt0 = typ.end_token.next0
                                elif (br_spec): 
                                    org_.add_type(typ, False)
                        ...
                        if (oin is not None): 
                            ...
                            if (oin.is_std_tail): 
                                ...
                                if (ei is None and oin.begin_token.is_comma): 
                                    ei = OrgItemEngItem.try_attach(oin.begin_token.next0, False)
                                if (ei is not None): 
                                    org_.add_type_str(ei.full_value)
                                    if (ei.short_value is not None): 
                                        org_.add_type_str(ei.short_value)
                                te = ttt.previous
                                break
                        if ((ttt != tt0 and isinstance(ttt, ReferentToken) and ttt.next0 == te) and isinstance(ttt.get_referent(), GeoReferent)): 
                            if (ttt.previous is not None and ttt.previous.get_morph_class_in_dictionary().is_adjective): 
                                continue
                            npt = NounPhraseHelper.try_parse(ttt.previous, NounPhraseParseAttr.REFERENTCANBENOUN, 0)
                            if (npt is not None and npt.end_token == ttt): 
                                pass
                            else: 
                                te = ttt.previous
                                if (te.morph.class0.is_preposition and te.previous is not None): 
                                    te = te.previous
                            org_._add_geo_object(ttt.get_referent())
                            break
                if (te is not None and tt0 is not None and (tt0.begin_char < te.begin_char)): 
                    if (isinstance(te.previous, NumberToken) and can_has_num): 
                        err = False
                        num1 = (te.previous if isinstance(te.previous, NumberToken) else None)
                        if (org_.number is not None and org_.number != str(num1.value)): 
                            err = True
                ...
                if ((te is not None and isinstance(te.previous, NumberToken) and can_has_num) and org_.number is None): 
                    org_.number = str((te.previous if isinstance(te.previous, NumberToken) else None).value)
                    tt11 = te.previous
                ...
                if (not Utils.isNullOrEmpty(s)): 
                    if (tt0.morph.class0.is_preposition and tt0 != br.begin_token.next0): 
                        for ty in org_.types: 
                            if (not " " in ty and ty[0].islower()): 
                                s = "{0} {1}".format(ty.upper(), s)
                                s1 = None
                                break
                    if (len(s) > OrganizationAnalyzer.__max_org_name): 
                        return None
                    if (s1 is not None and s1 != s and len(s1) <= len(s)): 
                        org_.add_name(s1, True, None)
                    org_.add_name(s, True, tt0)
                    ok1 = False
                    for c in s: 
                        if (c.isalnum()): 
                            ok1 = True
                    ...
                    if (not ok1): 
                        return None
                    if (br.begin_token.next0.chars.is_all_lower): 
                        return None
                    if (len(org_.types) == 0): 
                        ...
                        if (ty is not None and ty.coef >= 4): 
                            ...
                            if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL): 
                                return None
                            if (len(org_.names) == 1 and (len(org_.names[0]) < 2) and (br.length_char < 5)): 
                                return None
                    ...
                    br = br1
                    continue
                elif (((org_.number is not None or len(org_.eponyms) > 0)) and t1 == br.end_token): 
                    pass
                elif (len(org_._geo_objects) > 0 and len(org_.types) > 2): 
                    pass
                else: 
                    return None
                t1 = br.end_token
                if (org_.number is None and t1.next0 is not None and (t1.whitespaces_after_count < 2)): 
                    num1 = (None if OrgItemTypeToken.is_decree_keyword(t0.previous, 1) else OrgItemNumberToken.try_attach(t1.next0, False, typ))
                    if (num1 is not None): 
                        org_.number = num1.number
                        t1 = num1.end_token
                    else: 
                        t1 = self.__attach_tail_attributes(org_, t1.next0, None, True, attach_typ, False)
                else: 
                    t1 = self.__attach_tail_attributes(org_, t1.next0, None, True, attach_typ, False)
                if (t1 is None): 
                    t1 = br.end_token
                ...
                if (types is not None): 
                    for ty in types: 
                        if (ty.name is not None): 
                            org_.add_type_str(ty.name.lower())
                        ...
                        if (not ty.is_doubt_root_word or ty.coef > 0 or ty.geo is not None): 
                            ok0 = True
                        elif (ty.typ == "движение" and ((not br.begin_token.next0.chars.is_all_lower or not ty.chars.is_all_lower))): 
                            if (not br.begin_token.next0.morph.case.is_genitive): 
                                ok0 = True
                        elif (ty.typ == "АО"): 
                            if (ty.begin_token.chars.is_all_upper and (ty.whitespaces_after_count < 2) and BracketHelper.is_bracket(ty.end_token.next0, True)): 
                                ok0 = True
                            else: 
                                tt2 = t1.next0
                                first_pass2845 = True
                                while True:
                                    ...
                                    if (tt2.is_value("ИМЕНОВАТЬ", None)): 
                                        ok0 = True
                                    if (tt2.is_value("В", None) and tt2.next0 is not None): 
                                        if (tt2.next0.is_value("ЛИЦО", None) or tt2.next0.is_value("ДАЛЬШЕЙШЕМ", None) or tt2.next0.is_value("ДАЛЕЕ", None)): 
                                            ok0 = True
                                    break
                if (len(org_.eponyms) == 0 and (t1.whitespaces_after_count < 2)): 
                    ep_ = OrgItemEponymToken.try_attach(t1.next0, False)
                    if (ep_ is not None): 
                        for e0 in ep_.eponyms: 
                            org_.add_eponym(e0)
                ...
                if (len(org_.names) == 0): 
                    ...
                    org_.add_name(s, True, br.begin_token.next0)
                    org_.add_name(s1, True, None)
                if (not ok0): 
                ...
                if (ok0): 
                    ...
                    return ReferentToken._new697(org_, t0, t1, org_)
        num = None
        epon = None
        names = None
        pr = None
        ...
        while True:
            ...
            if ((_num) is not None): 
                ...
                if (t.whitespaces_before_count > 2): 
                    if (typ.end_token.next0 == t and MiscHelper.check_number_prefix(t) is not None): 
                        pass
                    else: 
                        break
            ...
            if ((typ) is not None): 
                if (typ.morph.case.is_genitive): 
                    if (typ.end_token.is_value("СЛУЖБА", None) or typ.end_token.is_value("УПРАВЛЕНИЕ", "УПРАВЛІННЯ")): 
                        typ = None
                if (typ is not None): 
                    ...
                    if (OrganizationAnalyzer.__last_typ(types) is not None and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                        if (OrgItemTypeToken.is_types_antagonistictt(typ, OrganizationAnalyzer.__last_typ(types))): 
                            if (names is not None and ((typ.morph.case.is_genitive or typ.morph.case.is_instrumental)) and (t.whitespaces_before_count < 2)): 
                                pass
                            else: 
                                break
            br = BracketHelper.try_parse(t, BracketParseAttr.NO, 100)
            if ((br) is not None): 
                if (own_org is not None and not (own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None)._is_from_global_ontos): 
                    break
                if (t.is_newline_before and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))): 
                    break
                typ = OrganizationAnalyzer.__last_typ(types)
                if ((org_.find_slot(OrganizationReferent.ATTR_TYPE, "организация", True) is not None or org_.find_slot(OrganizationReferent.ATTR_TYPE, "движение", True) is not None or org_.find_slot(OrganizationReferent.ATTR_TYPE, "організація", True) is not None) or org_.find_slot(OrganizationReferent.ATTR_TYPE, "рух", True) is not None): 
                ...
                if (br.is_quote_type): 
                    ...
                    if (rt is None): 
                        break
                    org0 = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
                    if (names is not None and len(names) == 1): 
                        if (((not names[0].is_noun_phrase and names[0].chars.is_all_upper)) or len(org0.names) > 0): 
                            if (not names[0].begin_token.morph.class0.is_preposition): 
                                if (len(org0.names) == 0): 
                                    org_.add_type_str(names[0].value)
                                else: 
                                    for n in org0.names: 
                                        org_.add_name("{0} {1}".format(names[0].value, n), True, None)
                                        if (typ is not None and typ.root is not None and typ.root.typ != OrgItemTermin.Types.PREFIX): 
                                            org_.add_name("{0} {1} {2}".format(typ.typ.upper(), MiscHelper.get_text_value_of_meta_token(names[0], GetTextAttr.NO), n), True, None)
                                    if (typ is not None): 
                                        typ.coef = 4
                                names = None
                    ...
                    if (t.next0.get_referent() is not None and t.next0.next0 == br.end_token): 
                        r = t.next0.get_referent()
                        if (r.type_name == OrganizationAnalyzer.__geoname): 
                            org_._add_geo_object(r)
                            t = br.end_token
                            ...
                            tmax = t1
                            continue
                    elif ((isinstance(t.next0, TextToken) and t.next0.chars.is_letter and not t.next0.chars.is_all_lower) and t.next0.next0 == br.end_token): 
                        typ = OrgItemTypeToken.try_attach(t.next0, True, None)
                        if (typ is not None): 
                            or0 = OrganizationReferent()
                            or0.add_type(typ, False)
                            if (or0.kind != OrganizationKind.UNDEFINED and org_.kind != OrganizationKind.UNDEFINED): 
                                if (org_.kind != or0.kind): 
                                    break
                            if (MiscHelper.test_acronym(t.next0, t0, t.previous)): 
                                org_.add_name(t.next0.get_source_text(), True, None)
                            else: 
                                org_.add_type(typ, False)
                            tmax = br.end_token
                            t = tmax
                            ...
                            nam = OrgItemNameToken.try_attach(t.next0, None, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, True)
                            if (nam is not None and nam.is_empty_word): 
                                break
                            if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL): 
                                org0 = OrganizationReferent()
                                org0.add_name((t.next0 if isinstance(t.next0, TextToken) else None).term, True, t.next0)
                                if (not OrganizationReferent.can_be_second_definition(org_, org0)): 
                                    break
                            org_.add_name((t.next0 if isinstance(t.next0, TextToken) else None).term, True, t.next0)
                            t = br.end_token
                            t1 = t
                            tmax = t1
                            continue
            ...
            if (typ is not None and typ.root is not None and typ.root.is_pure_prefix): 
                if (pr is None and names is None): 
                    ...
                    pr.morph.case = MorphCase.NOMINATIVE
            na = OrgItemNameToken.try_attach(t, pr, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, names is None)
            if (na is None and t is not None): 
                if (org_.kind == OrganizationKind.CHURCH or ((typ is not None and typ.typ is not None and "фермер" in typ.typ))): 
                    prt = t.kit.process_referent("PERSON", t)
                    if (prt is not None): 
                        na = OrgItemNameToken._new2050(t, prt.end_token, True)
                        na.value = MiscHelper.get_text_value_of_meta_token(na, GetTextAttr.NO)
                        na.chars = CharsInfo._new2051(True)
                        na.morph = prt.morph
            ...
            if (na is None): 
                if (attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                    if (t.is_char(',') or t.is_and): 
                        continue
                if (isinstance(t.get_referent(), OrganizationReferent)): 
                    own_org = (t if isinstance(t, ReferentToken) else None)
                    continue
                if (t.is_value("ПРИ", None) and isinstance(t.next0, ReferentToken) and isinstance(t.next0.get_referent(), OrganizationReferent)): 
                    t = t.next0
                    own_org = (t if isinstance(t, ReferentToken) else None)
                    continue
                if ((((names is None and t.is_char('/') and isinstance(t.next0, TextToken)) and not t.is_whitespace_after and t.next0.chars.is_all_upper) and t.next0.length_char >= 3 and isinstance(t.next0.next0, TextToken)) and not t.next0.is_whitespace_after and t.next0.next0.is_char('/')): 
                    na = OrgItemNameToken._new2052(t, t.next0.next0, t.next0.get_source_text().upper(), t.next0.chars)
                elif (names is None and typ is not None and ((typ.typ == "движение" or org_.kind == OrganizationKind.PARTY))): 
                    tt1 = None
                    if (t.is_value("ЗА", None) or t.is_value("ПРОТИВ", None)): 
                        tt1 = t.next0
                    elif (t.is_value("В", None) and t.next0 is not None): 
                        if (t.next0.is_value("ЗАЩИТА", None) or t.next0.is_value("ПОДДЕРЖКА", None)): 
                            tt1 = t.next0
            ...
            if (attach_typ == OrganizationAnalyzer.AttachType.MULTIPLE or attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                if (not na.is_std_tail and not na.chars.is_latin_letter and na.std_org_name_nouns == 0): 
                    ...
                    if (cla.is_proper_surname or ((t.morph.language.is_ua and t.morph.class0.is_proper_surname))): 
                        if (names is None and org_.kind == OrganizationKind.AIRPORT): 
                            pass
                    ...
                    if (cla.is_undefined and na.chars.is_cyrillic_letter and na.chars.is_capital_upper): 
                        if ((t.previous is not None and not t.previous.morph.class0.is_preposition and not t.previous.morph.class0.is_conjunction) and t.previous.chars.is_all_lower): 
                            if ((t.next0 is not None and isinstance(t.next0, TextToken) and t.next0.chars.is_letter) and not t.next0.chars.is_all_lower): 
                                break
                    if (typ is not None and typ.typ == "союз" and not t.morph.case.is_genitive): 
                        break
            ...
            if (names is None): 
                ...
                if (t.previous is not None and t.is_newline_before and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                    if (t.newlines_after_count > 1 or not t.chars.is_all_lower): 
                        break
                    if (t.morph.class0.is_preposition and typ is not None and (((typ.typ == "комитет" or typ.typ == "комиссия" or typ.typ == "комітет") or typ.typ == "комісія"))): 
                        pass
                    ...
                    if (t.chars != t.previous.chars): 
                        break
                if (t.chars.is_all_lower and org_.kind == OrganizationKind.JUSTICE): 
                    if (t.is_value("ПО", None) and t.next0 is not None and t.next0.is_value("ПРАВО", None)): 
                        pass
                    elif (t.is_value("З", None) and t.next0 is not None and t.next0.is_value("ПРАВ", None)): 
                        pass
                    else: 
                        break
                if (org_.kind == OrganizationKind.FEDERATION): 
                    if (t.morph.class0.is_preposition or t.morph.class0.is_conjunction): 
                        break
                if (t.chars.is_all_lower and ((org_.kind == OrganizationKind.AIRPORT or org_.kind == OrganizationKind.HOTEL))): 
                    break
                ...
                if (typ is not None and ((typ.root is None or not typ.root.is_pure_prefix))): 
                    if (typ.chars.is_latin_letter and na.chars.is_latin_letter): 
                        if (not t.is_value("OF", None)): 
                            break
                    if ((na.is_in_dictionary and na.morph.language.is_cyrillic and na.chars.is_all_lower) and not na.morph.case.is_undefined): 
                        if (na.preposition is None): 
                            if (not na.morph.case.is_genitive): 
                                break
                            if (org_.kind == OrganizationKind.PARTY and not spec_word_before): 
                                if (typ.typ == "лига"): 
                                    pass
                                else: 
                                    break
                            if (na.morph.number != MorphNumber.PLURAL): 
                                ...
                                if (prr is not None): 
                                    if (OrgItemEponymToken.try_attach(na.end_token.next0, False) is not None): 
                                        pass
                                    else: 
                                        break
                    if (na.preposition is not None): 
                        if (org_.kind == OrganizationKind.PARTY): 
                        ...
                        if (prt is not None): 
                            if (org_.kind == OrganizationKind.CHURCH): 
                ...
                if (na0.is_std_tail): 
                    break
                if (na.preposition is None): 
                    if ((not na.chars.is_latin_letter and na.chars.is_all_lower and not na.is_after_conjunction) and not na.morph.case.is_genitive): 
                        break
            names.append(na)
            t = na.end_token
            t1 = t
        ...
        if (typ is not None): 
            ...
            if (LanguageHelper.ends_with_ex(typ.typ, "комитет", "комиссия", "комітет", "комісія")): 
                ...
                ki = (own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None).kind
                ...
                own_org = None
            if (org_.kind == OrganizationKind.GOVENMENT): 
                if (names is None and ((typ.name is None or Utils.compareStrings(typ.name, typ.typ, True) == 0))): 
                    if ((attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY and typ.typ != "следственный комитет" and typ.typ != "кабинет министров") and typ.typ != "слідчий комітет"): 
                        if (((typ.typ == "администрация" or typ.typ == "адміністрація")) and isinstance(typ.end_token.next0, TextToken)): 
                            rt1 = typ.kit.process_referent("PERSONPROPERTY", typ.end_token.next0)
                            if (rt1 is not None and typ.end_token.next0.morph.case.is_genitive): 
                                geo_ = (rt1.referent.get_value("REF") if isinstance(rt1.referent.get_value("REF"), GeoReferent) else None)
                                if (geo_ is not None): 
                                    org_.add_name("АДМИНИСТРАЦИЯ " + (typ.end_token.next0 if isinstance(typ.end_token.next0, TextToken) else None).term, True, None)
                                    org_._add_geo_object(geo_)
                                    return ReferentToken(org_, typ.begin_token, rt1.end_token)
        ...
        always = False
        name_ = None
        if (((num is not None or org_.number is not None or epon is not None) or attach_typ == OrganizationAnalyzer.AttachType.HIGH or attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY) or own_org is not None): 
            if (names is not None): 
                if ((len(names) == 1 and names[0].chars.is_all_upper and attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY) and is_additional_attach): 
                    org_.add_name(MiscHelper.get_text_value(names[0].begin_token, names[len(names) - 1].end_token, GetTextAttr.NO), True, names[0].begin_token)
                else: 
                ...
                if (typ != mult_typ): 
                    if (t1.end_char < typ.end_token.end_char): 
                        t1 = typ.end_token
            if (name_ is not None): 
                if (len(name_) > OrganizationAnalyzer.__max_org_name): 
                    return None
                org_.add_name(name_, True, None)
            if (num is not None): 
                org_.number = num.number
            if (epon is not None): 
                for e0 in epon.eponyms: 
                    org_.add_eponym(e0)
            ...
            if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL): 
                if (typ is None): 
                    ...
                    if (num is None and epon is None): 
                        ...
                        if (t1.is_whitespace_after or t1.next0 is None): 
                            pass
                        elif (t1.next0.is_char_of(".,;") and t1.next0.is_whitespace_after): 
                            pass
                        else: 
                            ok = False
            if ((not ok and typ is not None and typ.can_be_dep_before_organization) and own_org is not None): 
                org_.add_type_str(("підрозділ" if own_org.kit.base_language.is_ua else "подразделение"))
                org_.higher = (own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None)
                t1 = own_org
                ok = True
            elif (typ is not None and own_org is not None and OrgOwnershipHelper.can_be_higher((own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None), org_, True)): 
                if (OrgItemTypeToken.is_types_antagonisticoo(own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None, org_)): 
                    if (org_.kind == OrganizationKind.DEPARTMENT and not typ.can_be_dep_before_organization): 
                        pass
                    else: 
                        org_.higher = (own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None)
            ...
            if (typ is None): 
                if (names[0].is_std_name and spec_word_before): 
                    org_.add_name(names[0].value, True, None)
                    t1 = names[0].end_token
                    t = self.__attach_tail_attributes(org_, t1.next0, None, True, attach_typ, False)
                    if (t is not None): 
                        t1 = t
                    return ReferentToken(org_, t0, t1)
                return None
            ...
            if (names[0].chars.is_latin_letter): 
                ...
                for j in range(1, len(names), 1):
                    if (not names[j].is_std_tail and ((names[j].is_newline_before or not names[j].chars.is_latin_letter))): 
                        ...
                        if (typ.geo is None and org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is not None): 
                            org_.slots.remove(org_.find_slot(OrganizationReferent.ATTR_GEO, None, True))
                        break
                    else: 
                        ...
                        if (names[j - 1].end_token.is_char('.') and not names[j - 1].value.endswith(".")): 
                            ...
                            print(" {0}".format(names[j].value), end="", file=tmp, flush=True)
                if (tmp.tell() > OrganizationAnalyzer.__max_org_name): 
                    return None
                ...
                if (tmp.tell() < 3): 
                    ...
                    if (types is not None and names[0].chars.is_all_upper): 
                        ...
                        return None
                ok = True
                org_.add_name(Utils.toStringStringIO(tmp), True, None)
            elif (typ.root is not None and typ.root.is_pure_prefix): 
                tt = (typ.end_token if isinstance(typ.end_token, TextToken) else None)
                if (tt is None): 
                    return None
                if (tt.is_newline_after): 
                    return None
                if (typ.begin_token == typ.end_token and tt.chars.is_all_lower): 
                    return None
                if (names[0].chars.is_all_lower): 
                    if (not names[0].morph.case.is_genitive): 
                        return None
                t1 = names[0].end_token
                for j in range(1, len(names), 1):
                ...
                if (num is None and isinstance(t1, NumberToken) and (t1 if isinstance(t1, NumberToken) else None).typ == NumberSpellingType.DIGIT): 
                    tt1 = t1.previous
                    ...
                    if (tt1 is not None and tt1.end_char > names[0].begin_char and isinstance(tt1, TextToken)): 
                        ...
                        org_.number = str((t1 if isinstance(t1, NumberToken) else None).value)
                if (len(name_) > OrganizationAnalyzer.__max_org_name): 
                    return None
                org_.add_name(name_, True, names[0].begin_token)
            else: 
                if (typ.is_dep): 
                    return None
                ...
                if (koef >= 4): 
                    always = True
                if (org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is not None): 
                    koef += 1
                if (spec_word_before): 
                    koef += 1
                ...
                for j in range(len(names)):
                    if (((names[j].is_newline_before and j > 0)) or names[j].is_noun_phrase != names[0].is_noun_phrase): 
                        ...
                        if (j == 0 and names[j].preposition is None and names[j].is_in_dictionary): 
                            if (not names[j].morph.case.is_genitive and ((typ.root is not None and not typ.root.can_has_single_name))): 
                                break
                        ...
                        if (names[j].is_noun_phrase): 
                            if (not names[j].chars.is_all_lower): 
                                ca = names[j].morph.case
                                if ((ca.is_dative or ca.is_genitive or ca.is_instrumental) or ca.is_prepositional): 
                                    koef += 0.5
                                else: 
                                    continue
                            elif (((j == 0 or names[j].is_after_conjunction)) and names[j].morph.case.is_genitive and names[j].preposition is None): 
                                koef += 0.5
                            if (j == (len(names) - 1)): 
                                if (isinstance(names[j].end_token.next0, TextToken)): 
                                    if (names[j].end_token.next0.get_morph_class_in_dictionary().is_verb): 
                                        koef += 0.5
                ...
                if (attach_typ != OrganizationAnalyzer.AttachType.NORMAL and attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                    koef += 3
                br1 = None
                if ((t1.whitespaces_after_count < 2) and BracketHelper.can_be_start_of_sequence(t1.next0, True, False)): 
                    ...
                    if (br1 is not None and (br1.length_char < 30)): 
                        ...
                        if (sss is not None and len(sss) > 2): 
                            org_.add_name(sss, True, br1.begin_token.next0)
                            koef += 1
                            t1 = br1.end_token
                        else: 
                            br1 = None
                if (koef >= 3 and t1.next0 is not None): 
                    r = t1.next0.get_referent()
                    if (r is not None and ((r.type_name == OrganizationAnalyzer.__geoname or r.type_name == OrganizationReferent.OBJ_TYPENAME))): 
                        koef += 1
                    elif (self.__is_geo(t1.next0, False) is not None): 
                        koef += 1
                    elif (t1.next0.is_char('(') and self.__is_geo(t1.next0.next0, False) is not None): 
                        koef += 1
                    elif (spec_word_before and t1.kit.process_referent("PERSON", t1.next0) is not None): 
                        koef += 1
                if (koef >= 4): 
                    ok = True
                if (not ok): 
                    if ((oki == OrganizationKind.PRESS or oki == OrganizationKind.FEDERATION or "агентство" in org_.types) or ((oki == OrganizationKind.PARTY and OrgItemTypeToken.check_org_special_word_before(t0.previous)))): 
                        if (not names[0].is_newline_before and not names[0].morph.class0.is_proper): 
                            if (names[0].morph.case.is_genitive and names[0].is_in_dictionary): 
                                ...
                                t1 = names[0].end_token
                if ((not ok and oki == OrganizationKind.FEDERATION and names[0].morph.case.is_genitive) and koef > 0): 
                    if (self.__is_geo(names[len(names) - 1].end_token.next0, False) is not None): 
                        ok = True
                ...
                if (ok): 
                    ...
                    if (isinstance(tt1.get_referent(), GeoReferent) and (tt1.get_referent() if isinstance(tt1.get_referent(), GeoReferent) else None).is_state): 
                    ...
                    if ((names[0].begin_token == t1 and s is not None and t1.morph.case.is_genitive) and t1.chars.is_capital_upper): 
                        if (cla.is_undefined or cla.is_proper_geo): 
                            if (ki == OrganizationKind.MEDICAL or ki == OrganizationKind.JUSTICE): 
                                geo_ = GeoReferent()
                                geo_.add_slot(GeoReferent.ATTR_NAME, t1.get_normal_case_text(MorphClass(), False, MorphGender.UNDEFINED, False), False, 0)
                                geo_.add_slot(GeoReferent.ATTR_TYPE, ("місто" if t1.kit.base_language.is_ua else "город"), False, 0)
                                rt = ReferentToken(geo_, t1, t1)
                                rt.data = ad
                                org_._add_geo_object(rt)
                                s = None
                    if (s is not None): 
                        if (tmp.tell() == 0): 
                            if (names[0].morph.case.is_genitive or names[0].preposition is not None): 
                                if (names[0].chars.is_all_lower): 
                    ...
                    if (tmp.tell() > OrganizationAnalyzer.__max_org_name): 
                        return None
                    org_.add_name(Utils.toStringStringIO(tmp), True, names[0].begin_token)
                    if (len(types) > 1 and types[0].name is not None): 
                        org_.add_type_str(types[0].name.lower())
        else: 
            if (typ is None): 
                return None
            ...
            if ((typ.typ == "банк" and isinstance(t, ReferentToken) and not t.is_newline_before) and typ.morph.number == MorphNumber.SINGULAR): 
                if (typ.name is not None): 
                    if (typ.begin_token.chars.is_all_lower): 
                        ...
                        org_.add_name(typ.name, True, None)
                        s0 = MiscHelper.get_text_value_of_meta_token(typ, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
                        if (s0 != typ.name): 
                            org_.add_name(s0, True, None)
                r = t.get_referent()
                if (r.type_name == OrganizationAnalyzer.__geoname and t.morph.case != MorphCase.NOMINATIVE): 
                    org_._add_geo_object(r)
                    return ReferentToken(org_, t0, t)
            if (((typ.root is not None and typ.root.is_pure_prefix)) and (typ.coef < 4)): 
                return None
            if (typ.root is not None and typ.root.must_has_capital_name): 
                return None
            if (typ.name is None): 
                if (((typ.typ.endswith("университет") or typ.typ.endswith("університет"))) and self.__is_geo(typ.end_token.next0, False) is not None): 
                    always = True
                elif (((org_.kind == OrganizationKind.JUSTICE or org_.kind == OrganizationKind.AIRPORT)) and org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is not None): 
                    pass
                elif (typ.coef >= 4): 
                    always = True
                elif (typ.chars.is_capital_upper): 
                    if (typ.end_token.next0 is not None and ((typ.end_token.next0.is_hiphen or typ.end_token.next0.is_char_of(":")))): 
                        pass
                    else: 
                        li = (None if ad is None else ad.local_ontology.try_attach_by_item(org_.create_ontology_item()))
                        if (li is not None and len(li) > 0): 
                            for ll in li: 
                                r = Utils.ifNotNull(ll.referent, (ll.tag if isinstance(ll.tag, Referent) else None))
                                if (r is not None): 
                                    if (org_.can_be_equals(r, Referent.EqualType.FORMERGING)): 
                                        ttt = typ.end_token
                                        nu = OrgItemNumberToken.try_attach(ttt.next0, True, None)
                                        if (nu is not None): 
                                            if ((r if isinstance(r, OrganizationReferent) else None).number != nu.number): 
                                                ttt = None
                                            else: 
                                                org_.number = nu.number
                                                ttt = nu.end_token
                    ...
                    while True:
                        ...
                        if (org0 is None): 
                            continue
                        if (not org0.can_be_equals(org_, Referent.EqualType.WITHINONETEXT)): 
                            continue
                        tt = (Utils.ifNotNull(self.__attach_tail_attributes(org_, typ.end_token.next0, ad, False, attach_typ, False), (typ.end_token if typ is not None else None)))
                        if (not org0.can_be_equals(org_, Referent.EqualType.WITHINONETEXT)): 
                            break
                        org_.merge_slots(org0, True)
                        return ReferentToken(org_, typ.begin_token, tt)
                    if (typ.root is not None and typ.root.can_be_single_geo and t1.next0 is not None): 
                        ggg = self.__is_geo(t1.next0, False)
                        if (ggg is not None): 
                            org_._add_geo_object(ggg)
                            t1 = self.__get_geo_end_token(ggg, t1.next0)
                            return ReferentToken(org_, t0, t1)
                    return None
            ...
            while to != typ.end_token and to is not None: 
                ...
                if (to.chars.is_capital_upper and not to.morph.class0.is_pronoun): 
                    koef += 0.5
                to = to.next0
            if (not has_nonstd_words and org_.kind == OrganizationKind.GOVENMENT): 
                koef -= 2
            ...
            if (typ.can_be_organization): 
                koef += 3
            org_.add_type(typ, False)
            if (((org_.kind == OrganizationKind.BANK or org_.kind == OrganizationKind.JUSTICE)) and typ.name is not None and len(typ.name) > len(typ.typ)): 
                koef += 1
            if (org_.kind == OrganizationKind.JUSTICE and len(org_._geo_objects) > 0): 
                always = True
            if (org_.kind == OrganizationKind.AIRPORT): 
                for g in org_._geo_objects: 
                    if (g.is_city): 
                        always = True
            if (koef > 3 or always): 
                ok = True
            if (((org_.kind == OrganizationKind.PARTY or org_.kind == OrganizationKind.JUSTICE)) and typ.morph.number == MorphNumber.SINGULAR): 
                if (org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is not None and typ.name is not None and len(typ.name) > len(typ.typ)): 
                    ok = True
                elif (typ.coef >= 4): 
                    ok = True
            ...
            if (ok): 
                if (typ.name is not None and not typ.is_not_typ): 
                    if (len(typ.name) > OrganizationAnalyzer.__max_org_name or Utils.compareStrings(typ.name, typ.typ, True) == 0): 
                        return None
                    org_.add_name(typ.name, True, None)
                t1 = typ.end_token
        if (not ok or len(org_.slots) == 0): 
            return None
        if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
            ok = always
            for s in org_.slots: 
        ...
        if (t is not None): 
            t1 = t
        if (own_org is not None and org_.higher is None): 
            if (do_higher_always or OrgOwnershipHelper.can_be_higher(own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None, org_, False)): 
                org_.higher = (own_org.referent if isinstance(own_org.referent, OrganizationReferent) else None)
        ...
        if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL and ((typ is None or (typ.coef < 4)))): 
            if (org_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is None or ((typ is not None and typ.geo is not None))): 
                is_all_low = True
                t = t0
                while t != t1.next0: 
                ...
                if (is_all_low and not spec_word_before): 
                    return None
        res = ReferentToken(org_, t0, t1)
        if (types is not None and len(types) > 0): 
            res.morph = types[0].morph
            if (types[0].is_not_typ and types[0].begin_token == t0 and (types[0].end_char < t1.end_char)): 
                res.begin_token = types[0].end_token.next0
        else: 
            res.morph = t0.morph
        if ((org_.number is None and t1.next0 is not None and (t1.whitespaces_after_count < 2)) and typ is not None and ((typ.root is None or typ.root.can_has_number))): 
            num1 = OrgItemNumberToken.try_attach(t1.next0, False, typ)
            if (num1 is None and t1.next0.is_hiphen): 
                num1 = OrgItemNumberToken.try_attach(t1.next0.next0, False, typ)
            if (num1 is not None): 
    ...
    def __try_attach_org_before(self, t : 'Token', ad : 'OrgAnalyzerData') -> 'ReferentToken':
        ...
        while t0 is not None: 
            ...
            while True:
                ...
                if (isinstance(ttt, NumberToken)): 
                    if (num is not None): 
                        break
                    num = str((ttt if isinstance(ttt, NumberToken) else None).value)
                    num_et = ttt
                    tt0 = ttt.previous
                    continue
                ...
                if (ttt.is_value("НАПРАВЛЕНИЕ", "НАПРЯМОК")): 
                    if (num is not None or ((isinstance(ttt.previous, NumberToken) and (ttt.whitespaces_before_count < 3)))): 
                        ...
                        if (isinstance(ttt.previous, NumberToken)): 
                            ...
                            oo.add_slot(OrganizationReferent.ATTR_NUMBER, str((ttt.previous if isinstance(ttt.previous, NumberToken) else None).value), False, 0)
                            return rt0
            ...
            if (rt is not None): 
                if (rt.end_char >= min_end_char and rt.end_char <= max_end_char): 
                    oo = (rt.referent if isinstance(rt.referent, OrganizationReferent) else None)
        ...
        while True:
            ...
            if (ty is not None): 
                nn = OrgItemNumberToken.try_attach(ty.end_token.next0, True, ty)
                if (nn is not None): 
                    ty.end_token = nn.end_token
                    ty.number = nn.number
                elif (isinstance(ty.end_token.next0, NumberToken) and (ty.whitespaces_after_count < 2)): 
                    ty.end_token = ty.end_token.next0
                    ty.number = str((ty.end_token if isinstance(ty.end_token, NumberToken) else None).value)
