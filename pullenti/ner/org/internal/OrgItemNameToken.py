...
class OrgItemNameToken(MetaToken):
    ...
    def try_attach(t : 'Token', prev : 'OrgItemNameToken', ext_onto : bool, first : bool) -> 'OrgItemNameToken':
        ...
        if (t is None): 
            return None
        if (t.is_value("ОРДЕНА", None) and t.next0 is not None): 
            ...
            if (npt is not None): 
                ...
                if (((t1.is_value("ЗНАК", None) or t1.is_value("ДРУЖБА", None))) and (t1.whitespaces_after_count < 2)): 
                    npt = NounPhraseHelper.try_parse(t1.next0, NounPhraseParseAttr.NO, 0)
                    if (npt is not None): 
                        t1 = npt.end_token
                return OrgItemNameToken._new1519(t, t1, True)
            if (t.next0.get_morph_class_in_dictionary().is_proper_surname): 
                return OrgItemNameToken._new1519(t, t.next0, True)
            ppp = t.kit.process_referent("PERSON", t.next0)
            if (ppp is not None): 
                return OrgItemNameToken._new1519(t, ppp.end_token, True)
            if ((t.whitespaces_after_count < 2) and BracketHelper.can_be_start_of_sequence(t.next0, True, False)): 
                br = BracketHelper.try_parse(t.next0, BracketParseAttr.NEARCLOSEBRACKET, 10)
                if (br is not None and (br.length_char < 40)): 
                    return OrgItemNameToken._new1519(t, br.end_token, True)
        if (first and t.chars.is_cyrillic_letter and t.morph.class0.is_preposition): 
        ...
        if (prev is None and not ext_onto): 
            if (t.kit.ontology is not None): 
                ad = (t.kit.ontology._get_analyzer_data(OrganizationAnalyzer.ANALYZER_NAME) if isinstance(t.kit.ontology._get_analyzer_data(OrganizationAnalyzer.ANALYZER_NAME), OrganizationAnalyzer.OrgAnalyzerData) else None)
                if (ad is not None): 
        ...
        if (prev is not None and not ext_onto): 
            if ((prev.chars.is_all_lower and not res.chars.is_all_lower and not res.is_std_tail) and not res.is_std_name): 
                if (prev.chars.is_latin_letter and res.chars.is_latin_letter): 
                    ...
                    return None
        if ((res.end_token.next0 is not None and not res.end_token.is_whitespace_after and res.end_token.next0.is_hiphen) and not res.end_token.next0.is_whitespace_after): 
            tt = (res.end_token.next0.next0 if isinstance(res.end_token.next0.next0, TextToken) else None)
            if (tt is not None): 
                if (tt.chars == res.chars or tt.chars.is_all_upper): 
                    res.end_token = tt
                    res.value = "{0}-{1}".format(res.value, tt.term)
        if ((res.end_token.next0 is not None and res.end_token.next0.is_and and res.end_token.whitespaces_after_count == 1) and res.end_token.next0.whitespaces_after_count == 1): 
            res1 = OrgItemNameToken.__try_attach(res.end_token.next0.next0, prev, ext_onto)
            if (res1 is not None and res1.chars == res.chars and OrgItemTypeToken.try_attach(res.end_token.next0.next0, False, None) is None): 
                if (not (res1.morph.case & res.morph.case).is_undefined): 
    ...
    def __is_not_term_noun(t : 'Token') -> bool:
        ...
        if (not ((isinstance(t.previous, TextToken)))): 
            return False
        if ((t.previous if isinstance(t.previous, TextToken) else None).term != "ПО"): 
            return False
    ...
    def __try_attach(t : 'Token', prev : 'OrgItemNameToken', ext_onto : bool) -> 'OrgItemNameToken':
        ...
        if (r is not None): 
            if (r.type_name == "DENOMINATION"): 
                return OrgItemNameToken._new1524(t, t, r.to_string(True, t.kit.base_language, 0), True)
            if (isinstance(r, GeoReferent) and t.chars.is_latin_letter): 
                res2 = OrgItemNameToken.__try_attach(t.next0, prev, ext_onto)
                if (res2 is not None and res2.chars.is_latin_letter): 
                    res2.begin_token = t
                    res2.value = "{0} {1}".format(MiscHelper.get_text_value_of_meta_token(t if isinstance(t, MetaToken) else None, GetTextAttr.NO), res2.value)
        ...
        if (tok is None and t.is_char(',')): 
            tok = OrgItemNameToken.__m_std_tails.try_parse(t.next0, TerminParseAttr.NO)
        if (tok is not None): 
        ...
        if (eng is None and t.is_char(',')): 
            eng = OrgItemEngItem.try_attach(t.next0, False)
        if (eng is not None): 
        ...
        if (tt.is_char(',') and prev is not None): 
            npt1 = NounPhraseHelper.try_parse(t.next0, NounPhraseParseAttr.NO, 0)
            if (npt1 is None or npt1.chars != prev.chars or (npt1.morph.case & prev.morph.case).is_undefined): 
                return None
            ty = OrgItemTypeToken.try_attach(t.next0, False, None)
            if (ty is not None): 
                return None
            if (npt1.end_token.next0 is None or not npt1.end_token.next0.is_value("И", None)): 
                return None
            t1 = npt1.end_token.next0
            npt2 = NounPhraseHelper.try_parse(t1.next0, NounPhraseParseAttr.NO, 0)
            if (npt2 is None or npt2.chars != prev.chars or (npt2.morph.case & npt1.morph.case & prev.morph.case).is_undefined): 
                return None
            ty = OrgItemTypeToken.try_attach(t1.next0, False, None)
            if (ty is not None): 
                return None
            res = OrgItemNameToken._new1528(npt1.begin_token, npt1.end_token, npt1.morph, npt1.get_normal_case_text(MorphClass(), False, MorphGender.UNDEFINED, False))
        ...
        if (((tt.is_char('&') or tt.is_value("AND", None) or tt.is_value("UND", None))) and prev is not None): 
            if (isinstance(tt.next0, TextToken) and tt.length_char == 1 and tt.next0.chars.is_latin_letter): 
                res = OrgItemNameToken._new1529(tt, tt.next0, tt.next0.chars)
                res.is_after_conjunction = True
                res.value = ("& " + (tt.next0 if isinstance(tt.next0, TextToken) else None).term)
                return res
            res = OrgItemNameToken.try_attach(tt.next0, None, ext_onto, False)
        ...
        if (prev is not None and prev.end_token.get_morph_class_in_dictionary().is_noun): 
            ...
            expinf = Explanatory.find_words(wo, prev.end_token.morph.language)
        ...
        if (npt is not None and expinf is not None): 
            for ei in expinf: 
                if (ei.nexts is not None and "" in ei.nexts): 
                    mc = ei.nexts[""]
                    if (not (mc & npt.morph.case).is_undefined): 
                        expl_ok = True
                        break
        if (npt is not None and ((expl_ok or npt.morph.case.is_genitive or ((prev is not None and not (prev.morph.case & npt.morph.case).is_undefined))))): 
            mc = npt.begin_token.get_morph_class_in_dictionary()
            if (mc.is_verb or mc.is_pronoun): 
                return None
            if (mc.is_adverb): 
                if (npt.begin_token.next0 is not None and npt.begin_token.next0.is_hiphen): 
                    pass
                else: 
                    return None
            ...
            if (mc.is_noun and npt.chars.is_all_lower): 
                ...
                if ((not ca.is_dative and not ca.is_genitive and not ca.is_instrumental) and not ca.is_prepositional): 
                    return None
            res = OrgItemNameToken._new1528(npt.begin_token, npt.end_token, npt.morph, npt.get_normal_case_text(MorphClass(), False, MorphGender.UNDEFINED, False))
            res.is_noun_phrase = True
            if ((npt.end_token.whitespaces_after_count < 2) and isinstance(npt.end_token.next0, TextToken)): 
                npt2 = NounPhraseHelper.try_parse(npt.end_token.next0, NounPhraseParseAttr.NO, 0)
                if (npt2 is not None and npt2.morph.case.is_genitive and npt2.chars.is_all_lower): 
                    typ = OrgItemTypeToken.try_attach(npt.end_token.next0, True, None)
                    epo = OrgItemEponymToken.try_attach(npt.end_token.next0, False)
                    rtt = t.kit.process_referent("PERSONPROPERTY", npt.end_token.next0)
                ...
                elif (npt.end_token.next0.is_comma and isinstance(npt.end_token.next0.next0, TextToken)): 
                    tt2 = npt.end_token.next0.next0
                    mv2 = tt2.get_morph_class_in_dictionary()
                    if (mv2.is_adjective and mv2.is_verb): 
                        bi = MorphBaseInfo._new1531(npt.morph.case, npt.morph.gender, npt.morph.number)
                        if (tt2.morph.check_accord(bi, False)): 
                            npt2 = NounPhraseHelper.try_parse(tt2.next0, NounPhraseParseAttr.NO, 0)
                            if (npt2 is not None and ((npt2.morph.case.is_dative or npt2.morph.case.is_genitive)) and npt2.chars.is_all_lower): 
                                res.end_token = npt2.end_token
                                res.value = "{0} {1}".format(res.value, MiscHelper.get_text_value(npt.end_token.next0, res.end_token, GetTextAttr.NO))
            if (expl_ok): 
                res.is_after_conjunction = True
        elif (npt is not None and ((((prev is not None and prev.is_noun_phrase and npt.morph.case.is_instrumental)) or ext_onto))): 
            res = OrgItemNameToken._new1528(npt.begin_token, npt.end_token, npt.morph, npt.get_normal_case_text(MorphClass(), False, MorphGender.UNDEFINED, False))
            res.is_noun_phrase = True
        elif (tt.is_and): 
            res = OrgItemNameToken.try_attach(tt.next0, prev, ext_onto, False)
            if (res is None or not res.is_noun_phrase or prev is None): 
                return None
            if ((prev.morph.case & res.morph.case).is_undefined): 
                return None
            if (prev.morph.number != MorphNumber.UNDEFINED and res.morph.number != MorphNumber.UNDEFINED): 
                if (((prev.morph.number & res.morph.number)) == MorphNumber.UNDEFINED): 
                    if (prev.chars != res.chars): 
                        return None
                    ty = OrgItemTypeToken.try_attach(res.end_token.next0, False, None)
                    if (ty is not None): 
                        return None
            ci = res.chars
            res.chars = ci
            ...
            if (npt is not None): 
                ...
                ok = False
                if (tt.term == "ПО"): 
                    ok = npt.morph.case.is_dative
                elif (tt.term == "С"): 
                    ok = npt.morph.case.is_instrumental
                elif (tt.term == "ЗА"): 
                    ok = (npt.morph.case.is_genitive | npt.morph.case.is_instrumental)
                elif (tt.term == "НА"): 
                    ok = npt.morph.case.is_prepositional
                elif (tt.term == "В"): 
                    ok = (npt.morph.case.is_dative | npt.morph.case.is_prepositional)
                    if (ok): 
                        ok = False
                        if (t.next0.is_value("СФЕРА", None) or t.next0.is_value("ОБЛАСТЬ", None)): 
                            ok = True
                elif (tt.term == "ПРИ"): 
                    ok = npt.morph.case.is_prepositional
                    if (ok): 
                        if (OrgItemTypeToken.try_attach(tt.next0, True, None) is not None): 
                            ok = False
                        else: 
                            rt = tt.kit.process_referent(OrganizationAnalyzer.ANALYZER_NAME, tt.next0)
                            if (rt is not None): 
                                ok = False
                    s = npt.noun.get_normal_case_text(MorphClass(), False, MorphGender.UNDEFINED, False)
                    if (s == "ПОДДЕРЖКА" or s == "УЧАСТИЕ"): 
                        ok = False
                else: 
                    ok = npt.morph.case.is_prepositional
                if (ok): 
                    res = OrgItemNameToken._new1533(t, npt.end_token, npt.morph, npt.get_normal_case_text(MorphClass(), True, MorphGender.UNDEFINED, False), npt.chars)
                    res.is_noun_phrase = True
                    res.preposition = tt.term
                    if (((res.value == "ДЕЛО" or res.value == "ВОПРОС")) and not res.is_newline_after): 
                        res2 = OrgItemNameToken.__try_attach(res.end_token.next0, res, ext_onto)
                        if (res2 is not None and res2.morph.case.is_genitive): 
                            res.value = "{0} {1}".format(res.value, res2.value)
                            res.end_token = res2.end_token
                            ttt = res2.end_token.next0
                            while ttt is not None: 
                                if (not ttt.is_comma_and): 
                                    break
                                res3 = OrgItemNameToken.__try_attach(ttt.next0, res2, ext_onto)
                                if (res3 is None): 
                                    break
        ...
        if (res is None): 
            ...
            t = tt.next0
            while t is not None: 
                if ((((t.is_hiphen or t.is_char_of("\\/"))) and t.next0 is not None and isinstance(t.next0, TextToken)) and not t.is_whitespace_before and not t.is_whitespace_after): 
                    t = t.next0
                    res.end_token = t
                    res.value = "{0}{1}{2}".format(res.value, ('.' if t.previous.is_char('.') else '-'), (t if isinstance(t, TextToken) else None).term)
                elif (t.is_char('.')): 
                    if (not t.is_whitespace_after and not t.is_whitespace_before and isinstance(t.next0, TextToken)): 
                        res.end_token = t.next0
                        t = t.next0
                        res.value = "{0}.{1}".format(res.value, (t if isinstance(t, TextToken) else None).term)
                    elif ((t.next0 is not None and not t.is_newline_after and t.next0.chars.is_latin_letter) and tt.chars.is_latin_letter): 
                        res.end_token = t
                    else: 
                        break
                else: 
        ...
        while t0 is not None: 
            ...
            if ((tt) is not None and tt.is_letters): 
                if (not tt.morph.class0.is_conjunction and not tt.morph.class0.is_preposition): 
                    for mf in tt.morph.items: 
                        if ((mf if isinstance(mf, MorphWordForm) else None).is_in_dictionary): 
                            res.is_in_dictionary = True
            if (t0 == res.end_token): 
                break
            t0 = t0.next0
        if (res.begin_token == res.end_token and res.begin_token.chars.is_all_upper): 
            if (res.end_token.next0 is not None and not res.end_token.is_whitespace_after): 
                t1 = res.end_token.next0
                if (t1.next0 is not None and not t1.is_whitespace_after and t1.is_hiphen): 
                    t1 = t1.next0
                if (isinstance(t1, NumberToken)): 
                    res.value += str((t1 if isinstance(t1, NumberToken) else None).value)
                    res.end_token = t1
    ...
    def initialize() -> None:
        ...
        for s in ["ЗАМОВНИК", "ВИКОНАВЕЦЬ", "РОЗРОБНИК", "БЕНЕФІЦІАР", "ОДЕРЖУВАЧ", "ВІДПРАВНИК", "ВИРОБНИК", "ВИРОБНИК", "ПОСТАЧАЛЬНИК", "АБОНЕНТ", "КЛІЄНТ", "ВКЛАДНИК", "СУБ'ЄКТ", "ПРОДАВЕЦЬ", "ПОКУПЕЦЬ", "ОРЕНДОДАВЕЦЬ", "ОРЕНДАР", "СУБОРЕНДАР", "НАЙМОДАВЕЦЬ", "НАЙМАЧ", "АГЕНТ", "ПРИНЦИПАЛ", "ПРОДАВЕЦЬ", "ПОСТАЧАЛЬНИК", "ПІДРЯДНИК", "СУБПІДРЯДНИК"]: 
            OrgItemNameToken.__m_std_tails.add(Termin._new459(s, MorphLang.UA, s))
        ...
        for k in range(2):
            ...
            str0 = OrgItemTypeToken._deflate(dat).decode('utf-8', 'ignore')
            with io.StringIO(str0) as tr: 
                while True:
                    ...
                    if (k == 0): 
                        ...
                        OrgItemNameToken.__m_std_nouns.add(Termin._new844(line, MorphLang.UA))
    ...
    @staticmethod
    def _new2050(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.is_std_name = _arg3
        return res
    
    @staticmethod
    def _new2052(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'CharsInfo') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
        res.chars = _arg4
        return res
