...
class UnitToken(MetaToken):
    ...
    def try_parse(t : 'Token', add_units : 'TerminCollection', prev : 'UnitToken', parse_unknown_units : bool=False) -> 'UnitToken':
        ...
        if (toks is not None): 
            ...
            if (toks[0].begin_token == toks[0].end_token and ((toks[0].begin_token.is_value("М", None) or toks[0].begin_token.is_value("M", None))) and toks[0].begin_token.chars.is_all_lower): 
                if (prev is not None and prev.unit is not None and prev.unit.kind == MeasureKind.LENGTH): 
                    res = UnitToken._new1626(t0, toks[0].end_token, UnitsHelper.UMINUTE)
                    res.pow0_ = pow0__
                    if (is_neg): 
                        res.pow0_ = (- pow0__)
                    return res
            uts = list()
            for tok in toks: 
                res = UnitToken._new1626(t0, tok.end_token, Utils.asObjectOrNull(tok.termin.tag, Unit))
                res.pow0_ = pow0__
                if (is_neg): 
                    res.pow0_ = (- pow0__)
        ...
        if (add_units is not None): 
            ...
            if (tok is not None): 
                res = UnitToken._new1741(t0, tok.end_token, Utils.asObjectOrNull(tok.termin.tag, UnitReferent))
    ...
    @staticmethod
    def _new1626(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Unit') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.unit = _arg3
        return res
    
    @staticmethod
    def _new1738(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Unit', _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.unit = _arg3
        res.is_doubt = _arg4
        return res
    ...
    @staticmethod
    def _new1741(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'UnitReferent') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.ext_onto = _arg3
        return res
    
    @staticmethod
    def _new1742(_arg1 : 'Token', _arg2 : 'Token', _arg3 : int, _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.pow0_ = _arg3
        res.is_doubt = _arg4
        return res