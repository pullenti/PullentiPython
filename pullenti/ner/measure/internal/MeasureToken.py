...
class MeasureToken(MetaToken):
    ...
    def try_parse_minimal(t : 'Token', add_units : 'TerminCollection', can_omit_number : bool=False) -> 'MeasureToken':
        if (t is None or (isinstance(t, ReferentToken))): 
            return None
        mt = NumbersWithUnitToken.try_parse_multi(t, add_units, can_omit_number, False, False)
        if (mt is None): 
            return None
        if (len(mt[0].units) == 0): 
            return None
        if ((len(mt) == 1 and len(mt[0].units) == 1 and mt[0].units[0].is_doubt) and not mt[0].is_newline_before): 
            return None
        if (len(mt) == 1): 
            res = MeasureToken._new1590(mt[0].begin_token, mt[len(mt) - 1].end_token, mt[0])
            res.__parse_internals(add_units)
            return res
        res = MeasureToken(mt[0].begin_token, mt[len(mt) - 1].end_token)
        for m in mt: 
            res.internals.append(MeasureToken._new1590(m.begin_token, m.end_token, m))
        return res
    ...
    def __parse_internals(self, add_units : 'TerminCollection') -> None:
        if (self.end_token.next0_ is not None and ((self.end_token.next0_.is_char_of("\\/") or self.end_token.next0_.is_value("ПРИ", None)))): 
            mt1 = MeasureToken.try_parse(self.end_token.next0_.next0_, add_units, True, False)
            if (mt1 is not None): 
                self.internals.append(mt1)
                self.end_token = mt1.end_token
            else: 
                mt = NumbersWithUnitToken.try_parse(self.end_token.next0_.next0_, add_units, False, False, False)
                if (mt is not None and len(mt.units) > 0 and not UnitToken.can_be_equals(self.nums.units, mt.units)): 
                    self.internals.append(MeasureToken._new1590(mt.begin_token, mt.end_token, mt))
                    self.end_token = mt.end_token
    
    @staticmethod
    def try_parse(t : 'Token', add_units : 'TerminCollection', can_be_set : bool=True, can_units_absent : bool=False) -> 'MeasureToken':
        ...
        whd = None
        minmax = 0
        wrapminmax1601 = RefOutArgWrapper(minmax)
        tt = NumbersWithUnitToken._is_min_or_max(t0, wrapminmax1601)
        minmax = wrapminmax1601.value
        if (tt is not None): 
            t = tt.next0_
        ...
        if (npt is None): 
            ...
            elif ((isinstance(t0, TextToken)) and t0.length_char > 3 and t0.get_morph_class_in_dictionary().is_undefined0): 
                npt = NounPhraseToken(t0, t0)
            else: 
                return None
        elif (NumberHelper.try_parse_real_number(t, True) is not None): 
            return None
        else: 
        ...
        t1 = npt.end_token
        t = npt.end_token
        name_ = MetaToken._new572(npt.begin_token, npt.end_token, npt.morph)
        units = None
        units2 = None
        internals_ = list()
        not0_ = False
        ...
        while True:
            ...
            if (tt.is_table_control_char0): 
                break
            wrapminmax1594 = RefOutArgWrapper(minmax)
            tt2 = NumbersWithUnitToken._is_min_or_max(tt, wrapminmax1594)
            minmax = wrapminmax1594.value
            if (tt2 is not None): 
                tt = tt2
                t = tt
                t1 = t
                continue
            if ((tt.is_value("БЫТЬ", None) or tt.is_value("ДОЛЖЕН", None) or tt.is_value("ДОЛЖНЫЙ", None)) or tt.is_value("МОЖЕТ", None) or ((tt.is_value("СОСТАВЛЯТЬ", None) and not tt.get_morph_class_in_dictionary().is_adjective0))): 
                t = tt
                t1 = t
            ...
            if (www is not None): 
                ...
                t1 = t
                continue
            if (len(internals_) > 0 and tt.is_comma_and0): 
                continue
            if (tt.is_value("ПРИ", None) or len(internals_) > 0): 
                mt1 = MeasureToken.try_parse(tt.next0_, add_units, False, False)
            ...
            if ((isinstance(tt, NumberToken)) and (tt).typ == NumberSpellingType.WORDS): 
                npt3 = NounPhraseHelper.try_parse(tt, NounPhraseParseAttr.PARSENUMERICASADJECTIVE, 0)
                if (npt3 is not None): 
                    tt = npt3.end_token
                    t1 = tt
                    if (len(internals_) == 0): 
                        name_.end_token = t1
                    continue
            mt0 = NumbersWithUnitToken.try_parse(tt, add_units, False, False, False)
            if (mt0 is not None): 
                break
            if (((tt.is_comma0 or tt.is_char('('))) and tt.next0_ is not None): 
            ...
            if (tt.is_value("НЕ", None) and tt.next0_ is not None): 
                ...
                if (mc.is_adverb0 or mc.is_misc0): 
                    break
                continue
            if (tt.is_value("ЯМЗ", None)): 
                pass
            npt2 = NounPhraseHelper.try_parse(tt, Utils.valToEnum((NounPhraseParseAttr.PARSEPREPOSITION) | (NounPhraseParseAttr.IGNOREBRACKETS), NounPhraseParseAttr), 0)
            if (npt2 is None): 
                if (tt.morph.class0_.is_preposition0 or tt.morph.class0_.is_conjunction0): 
                    ...
                    if (to is not None): 
                        if ((isinstance(to.end_token.next0_, TextToken)) and to.end_token.next0_.is_letters0): 
                            pass
                        else: 
                            break
                    t1 = tt
                ...
                if (((isinstance(tt, TextToken)) and tt.chars.is_letter0 and tt.length_char > 1) and (((tt.chars.is_all_upper0 or mc.is_adverb0 or mc.is_undefined0) or mc.is_adjective0))): 
        ...
        while True:
            ...
            if (t1.is_table_control_char0): 
                ...
                if (www is not None): 
                    ...
                    t1 = t
                    continue
            elif (t1.is_hiphen0 and t1.is_whitespace_after0 and t1.is_whitespace_before0): 
                pass
            else: 
                break
        if (t1 is None): 
            return None
        mts = NumbersWithUnitToken.try_parse_multi(t1, add_units, False, not0_, True)
        if (mts is None): 
            if (units is not None and len(units) > 0): 
                if (t1 is None or t1.previous.is_char(':')): 
                    ...
                    if (t1 is None): 
                        ...
                        t1 = t1.previous
                    mts.append(NumbersWithUnitToken._new1595(t0, t1, math.nan))
            if (mts is None): 
                return None
        mt = mts[0]
        if (name_.begin_token.morph.class0_.is_preposition0): 
        ...
        if (len(mts) > 1 and len(internals_) == 0): 
            if (len(mt.units) == 0): 
                if (units is not None): 
                    for m in mts: 
                        m.units = units
            res1 = MeasureToken._new1596(t0, mts[len(mts) - 1].end_token, name_.morph, True)
            res1.name = MiscHelper.get_text_value_of_meta_token(name_, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
            k = 0
            while k < len(mts): 
                ttt = MeasureToken._new1590(mts[k].begin_token, mts[k].end_token, mts[k])
                if (whd is not None): 
        ...
        if (not mt.is_whitespace_before): 
            if (mt.begin_token.previous is None): 
                return None
            if (mt.begin_token.previous.is_char_of(":),") or mt.begin_token.previous.is_table_control_char0): 
                pass
            else: 
                return None
        ...
        if (len(mt.units) == 0): 
            ...
            if (units is None): 
                ...
                mt.units = units
        res = MeasureToken._new1598(t0, mt.end_token, name_.morph, internals_)
        if (((not t0.is_whitespace_before0 and t0.previous is not None and t0 == name_.begin_token) and t0.previous.is_hiphen0 and not t0.previous.is_whitespace_before0) and (isinstance(t0.previous.previous, TextToken))): 
        ...
        if (t1 is not None and t1.is_comma_and0): 
            t1 = t1.next0_
        mts1 = NumbersWithUnitToken.try_parse_multi(t1, add_units, False, False, False)
        if ((mts1 is not None and len(mts1) == 1 and (t1.whitespaces_before_count < 3)) and len(mts1[0].units) > 0 and not UnitToken.can_be_equals(mts[0].units, mts1[0].units)): 
            res.is_set = True
            res.nums = (None)
            res.internals.append(MeasureToken._new1590(mt.begin_token, mt.end_token, mt))
            res.internals.append(MeasureToken._new1590(mts1[0].begin_token, mts1[0].end_token, mts1[0]))
            res.end_token = mts1[0].end_token
        return res
    
    @staticmethod
    def _new1590(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'NumbersWithUnitToken') -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.nums = _arg3
        return res
    
    @staticmethod
    def _new1596(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : bool) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.reliable = _arg4
        return res
    ...
    @staticmethod
    def _new1598(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : typing.List['MeasureToken']) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.internals = _arg4
        return res